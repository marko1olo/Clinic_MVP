...
function xrayIntFromEnv(name, fallback, min, max) {
 ...
 */
function xrayAnalysisDeadlineMs() {
 ...
 */
async function persistXrayAnalysisOutcome(job, patch) {
 ...
 */
async function analyzeWithDeadline(imageDataUri, deadlineMs) {
    let timer = null;
    try {
 ...
 * из-за одного снимка.
 */
async function executeXrayAnalysisJob(job) {
    try {
        ...
        try {
            await persistXrayAnalysisOutcome(job, {
                status: "error",
                aiError: error instanceof XrayAnalysisDeadlineError
            ...
            console.error("[XRay AI] Состояние разбора не записано", job.scanId, persistError instanceof Error ? persistError.message : String(persistError));
        }
    }
}
/**
 ...
 */
function startDetachedXrayAnalysis(job) {
    setImmediate(() => {
