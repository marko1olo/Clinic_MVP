...
export async function consumeDenteTelegramLinkCode(
	organizationId,
	codeFingerprint,
) {
	const [updated] = await db
		...
		.where(
			and(
				eq(denteTelegramLinkCodes.organizationId, organizationId),
				eq(denteTelegramLinkCodes.codeFingerprint, codeFingerprint),
				eq(denteTelegramLinkCodes.status, "pending"),
			),
		)
		.returning();
