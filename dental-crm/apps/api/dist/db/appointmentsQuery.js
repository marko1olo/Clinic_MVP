...
import { createAppointment as createAppointmentInMemory, appointments as inMemoryAppointments, updateAppointment as updateAppointmentInMemory, } from "../sampleData.js";
...
async function assertNoResourceOverlap(executor, organizationId, candidate) {
    ...
    if (candidate.doctorUserId)
        matchConditions.push(eq(schema.appointments.doctorUserId, candidate.doctorUserId));
    if (candidate.patientId)
        matchConditions.push(eq(schema.appointments.patientId, candidate.patientId));
...
export async function updateAppointmentInDb(organizationId, appointmentId, input) {
    ...
    const updated = await db.transaction(async (tx) => {
        ...
        const [row] = await tx
            ...
            .where(and(eq(schema.appointments.organizationId, organizationId), eq(schema.appointments.id, appointmentId)))
            .returning();
        return row;
    });
    if (!updated)
