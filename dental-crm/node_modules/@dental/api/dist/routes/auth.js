...
import { configuredAuthTokenSecret, requireAuthTokenSecret, } from "../accessGuard.js";
...
function requireAdminSetupKey(reply, providedKey) {
    ...
    if (!adminKey) {
        reply.code(503).send({
            error: "AdminSetupKeyMissing",
            message: "На сервере не задан ADMIN_SETUP_KEY.",
        });
        return false;
    }
    if (typeof providedKey !== "string" || providedKey !== adminKey) {
        reply
            .code(403)
            .send({ error: "Forbidden", message: "Неверный admin key." });
        return false;
    }
    return true;
}
...
export async function requireClinicToken(request, reply) {
    ...
    if (!token) {
        return void reply.code(401).send({
            error: "AuthRequired",
            message: "Необходима авторизация рабочего кабинета клиники.",
        });
    }
    ...
    if (!payload || !payload.organizationId) {
        return void reply.code(401).send({
            error: "TokenExpired",
            message: "Сессия истекла. Войдите в кабинет заново.",
        });
    }
...
export async function registerAuthRoutes(app) {
    ...
    app.post("/api/auth/clinic/login", async (request, reply) => {
        ...
        if (!email || !password) {
            return reply.code(400).send({
                ...
                message: "Введите логин и пароль клиники.",
            });
        }
    ...
    app.post("/api/auth/staff/unlock", async (request, reply) => {
        ...
        if (!userId || !pinCode) {
            return reply.code(400).send({
                error: "ValidationError",
                message: "Необходимо указать сотрудника и ввести PIN-код.",
            });
        }
        ...
        const clinicPayload = clinicToken
            ? verifySessionToken(clinicToken)
            : null;
        ...
        if (!user) {
            ...
            return reply.code(404).send({
                error: "UserNotFound",
                message: "Сотрудник не найден или заблокирован.",
            });
        }
    ...
    app.post("/api/auth/register", async (request, reply) => {
        const { clinicName, ownerName, email, password } = request.body ?? {};
    ...
    app.post("/api/auth/user/update-pin", async (request, reply) => {
        ...
        if (!oldPin || !newPin)
            return reply.code(400).send({
                error: "ValidationError",
                message: "Введите старый и новый PIN-код.",
            });
        const [user] = await db
            .select()
            .from(users)
    ...
    app.post("/api/auth/dev-login", async (request, reply) => {
        ...
        if (!user) {
            return reply.code(500).send({
                error: "NoUsers",
                message: "No seed users found in the database. Please run seed script first.",
            });
        }
        const [org] = await db
            .select()
        ...
        if (!org) {
            return reply.code(500).send({
                error: "NoOrgs",
                message: "User exists but organization missing.",
            });
        }
        const clinicToken = signToken({ organizationId: org.id, clinicName: org.name }, TOKEN_SECRET(), 60 * 60 * 24 * 7);
        const staffToken = signToken({
