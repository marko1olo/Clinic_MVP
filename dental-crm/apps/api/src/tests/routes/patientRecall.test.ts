...
describe("возврат пациентов", () => {
	...
	before(async () => {
		process.env = { ...originalEnv, NODE_ENV: "test", DENTE_DEV_ALLOW_HEADER_ORG: "1", DENTE_CLINICAL_ALLOW_UNGUARDED_READS: "1", DENTE_CLINICAL_ALLOW_UNGUARDED_MUTATIONS: "1" };
				...
				await db.insert(patients).values([
					{
						id: DUE_PATIENT,
						organizationId: ORG_ID,
						fullName: "Давний Пациент",
						phone: "+7 916 000-09-01",
					},
					{
						id: RECENT_PATIENT,
						organizationId: ORG_ID,
						fullName: "Недавний Пациент",
						phone: "+7 916 000-09-02",
					},
					{
						id: BOOKED_PATIENT,
						organizationId: ORG_ID,
						fullName: "Уже Записан",
						phone: "+7 916 000-09-03",
					},
				]);
	...
	test("зовут давних, не зовут недавних и уже записанных", async (context) => {
		...
		assert.ok(
			ids.includes(DUE_PATIENT),
			`девять месяцев без осмотра — должен быть в списке: ${response.body}`,
		);
		...
		assert.ok(
			!ids.includes(BOOKED_PATIENT),
			"записан на будущее — звать не нужно",
		);
	...
	test("приглашение ставится в очередь как реклама, а не как служебное сообщение", async (context) => {
		...
		const response = await app.inject({
			...
			payload: {
				patientId: DUE_PATIENT,
				channel: "sms",
				body: "Пора на профилактический осмотр.",
			},
		});
		assert.equal(response.statusCode, 200, response.body);
