...
import { doctorCommissions } from "../db/schema.js";
export class CommercialEngine {
	 ...
	 */
	static getSmartBundles(clinicalState, toothNumber) {
		const bundles = [];
		if (clinicalState === "Pulpitis") {
			bundles.push(
				{
					...
					phase: 1,
					isBundle: true,
					name: "Лечение пульпита (механика и обтурация)",
					price: 15000,
				},
				{
					priceId: "ANES_INFIL",
			...
			bundles.push(
				{
					priceId: "SURG_IMPL_PREM",
					quantity: 1,
					phase: 2,
					isBundle: true,
					name: "Установка имплантата (Премиум) с шаблоном",
					price: 45000,
				},
				{
					priceId: "SURG_ANES",
					...
					phase: 2,
					isBundle: true,
					name: "Анестезия хирургическая",
					price: 1200,
				},
			);
			// Prosthetic Bundle
			bundles.push({
				priceId: "PROSTH_CROWN_ZR",
				quantity: 1,
				phase: 3,
				isBundle: true,
			...
			bundles.push(
				{
					...
					phase: 1,
					isBundle: true,
					name: "Восстановление зуба композитом (Кариес)",
					price: 7500,
				},
				{
					priceId: "ANES_INFIL",
	 ...
	 */
	static async calculateDoctorCommission(params) {
		const { doctorId, specialty, serviceCategory, grossPrice, materialCost } =
			params;
		...
		if (!config) {
			return (grossPrice - materialCost) * 0.2;
		}
		// New schema: commissionPct, materialCostDeductionPct
		const deductMaterial =
			materialCost * ((config.materialCostDeductionPct ?? 100) / 100);
