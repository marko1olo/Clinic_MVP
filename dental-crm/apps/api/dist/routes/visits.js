 ...
 */
const visitDraftSignedNoDraftMessage = "Черновик приема не открыт: этот прием уже подписан, поэтому черновика у него нет. Запись приема осталась в карте пациента — " +
    "чтобы дописать лечение, откройте новый прием по записи в расписании.";
const visitDraftVoidedNoDraftMessage = "Черновик приема не открыт: этот прием аннулирован, поэтому черновика у него нет. Аннулированный прием не дописывают — " +
...
const visitDraftAutosaveNoActiveVisitMessage = "Черновик приема не сохранен: в клинике сейчас не открыт ни один прием, поэтому записывать некуда. Набранный текст остался на экране — " +
    "откройте прием по записи в расписании и повторите сохранение.";
const visitDraftAcceptNoActiveVisitMessage = "Черновик приема не принят: в клинике сейчас не открыт ни один прием, поэтому подписывать нечего. Набранный текст остался на экране — " +
    "откройте прием по записи в расписании и повторите сохранение.";
/**
 ...
 */
function sendNoActiveVisitRefusal(reply, operation) {
    reply.code(409);
    return {
        error: "VisitDraftMutationRejected",
        reason: "no_active_visit",
        message: operation === "accept" ? visitDraftAcceptNoActiveVisitMessage : visitDraftAutosaveNoActiveVisitMessage
    };
}
/*
 ...
 */
const visitDraftAcceptResponseIncompleteMessage = "Прием подписан и сохранен в карте пациента, но рабочий экран не получил карточку закрытия приема. " +
...
import { getVisitDraftAutosaveFromDb, upsertVisitDraftAutosaveInDb, acceptVisitDraftInDb, openVisitForAppointmentInDb, VisitSignedResponseIncompleteError } from "../db/visitsQuery.js";
...
const visitOpenAppointmentClosedMessage = "Прием не открыт: запись отменена или отмечена как неявка. Создайте новую запись в расписании.";
 ...
 */
export function sendVisitOpenError(error, reply) {
...
export async function registerVisitRoutes(app) {
    ...
    app.get("/api/visits/:visitId/draft/autosave", async (request, reply) => {
        ...
        if (lookup.outcome === "no_draft") {
            ...
            return {
                ...
                message: lookup.status === "signed" ? visitDraftSignedNoDraftMessage : visitDraftVoidedNoDraftMessage
            };
        }
    ...
    app.put("/api/visits/:visitId/draft/autosave", async (request, reply) => {
        ...
        const outcome = parseVisitPayload(visitDraftAutosaveRequestSchema, { ...visitRequestBody(request.body), visitId }, visitDraftAutosaveValidationMessage, reply);
        if (!outcome.ok)
    ...
    app.post("/api/visits/:visitId/draft/accept", async (request, reply) => {
        ...
        const outcome = parseVisitPayload(acceptVisitDraftSchema, { ...visitRequestBody(request.body), visitId }, visitDraftAcceptValidationMessage, reply);
        if (!outcome.ok)
            return outcome.refusal;
        const input = outcome.data;
        let result;
        try {
            ...
            if (error instanceof VisitSignedResponseIncompleteError) {
                request.log.error({ visitId: error.acceptedVisitId, revision: error.newRevision, cause: error.cause }, "Прием подписан, но слой доступа не собрал ответ по контракту acceptVisitDraftResponseSchema");
                reply.code(500);
                return {
        ...
        if (!response.success) {
            request.log.error({ visitId: result.visit.id, revision: result.visit.revision, issues: response.error.issues }, "Прием подписан, но ответ маршрута не собран по контракту acceptVisitDraftResponseSchema");
            reply.code(500);
            return {
                error: "VisitDraftAcceptResponseIncomplete",
                reason: "visit_signed_response_incomplete",
