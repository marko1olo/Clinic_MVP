...
def create_variations(img_bytes: io.BytesIO) -> list[io.BytesIO]:
    ...
    for i in range(5):
        ...
        # Для человека смысл картинки не меняется, для машины — это совершенно
        # другое фото
        if random.choice([True, False]):
            img = ImageOps.mirror(img)

        # 2. Кроп и Зум (обрезаем случайные куски от 2% до 12% с каждой
        # стороны)
        ...
        # Растягиваем обратно до оригинального размера (или около того), чтобы
        # сместить сетку
        ...
        # 5. Сохранение с рандомным сжатием и удалением EXIF-метаданных (Pillow
        # удаляет их по умолчанию)
        out_bio = io.BytesIO()
...
async def handle_photo(message: Message):
    ...
    # чтобы бот продолжал отвечать другим пользователям, пока процессор
    # считает шум
    ...
    for idx, bio in enumerate(variations):
        ...
        input_file = BufferedInputFile(
            bio.getvalue(), filename=f"variation_{idx}.jpg")
