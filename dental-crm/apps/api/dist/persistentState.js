...
function stateCollectionCounts(state) {
	...
	return {
		...
		denteTelegramOutboxDeliveryReceipts:
			state.denteTelegramOutboxDeliveryReceipts?.length ?? 0,
	};
}
...
function rotateStateBackup() {
	...
	const backupPath = path.join(
		backupDirectoryPath,
		`dental-crm-state-${timestampForFileName()}.json`,
	);
	fs.copyFileSync(stateFilePath, backupPath);
	const maxBackupCount = getMaxBackupCount();
	const backupLimit =
		Number.isFinite(maxBackupCount) && maxBackupCount > 0
...
function readPersistedState() {
	...
	try {
		...
		console.warn(
			`Dental state file ignored: ${error instanceof Error ? error.message : "unknown parse error"}`,
		);
		return null;
	}
}
...
export function savePersistentState(state) {
	...
	try {
		...
		console.warn(
			`Dental state file save failed: ${error instanceof Error ? error.message : "unknown save error"}`,
		);
	}
}
...
export async function getPersistentStateIntegrityReport(limit = 8) {
	...
	const warningCodes = [
		...
		backups.some(
			(backup) =>
				backup.readable === false || backup.checksumVerified === false,
		)
			? "backup_integrity_warning"
			: null,
	];
	const warnings = compactPersistenceWarnings(
		warningCodes.map((warning) =>
			warning ? persistenceWarningText(warning) : null,
		),
	);
	return {
		ok:
