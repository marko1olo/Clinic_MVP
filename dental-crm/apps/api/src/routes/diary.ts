...
export default async function registerDiaryRoutes(app: FastifyInstance) {
  ...
  app.get("/api/diaries/:id/revisions", async (req, reply) => {
    if (!(await requireClinicalReadAccess(req, reply, "read diary revisions"))) return;
  ...
  app.post("/api/diaries", async (req, reply) => {
    ...
    const [existing] = await db
      .select()
      .from(visitDiaries)
      .where(and(eq(visitDiaries.visitId, data.visitId), eq(visitDiaries.organizationId, orgId)));
    ...
    const diaryHash = isSigning
      ? computeDiaryHash(data.visitId, data.patientId, data.anamnesis, data.statusLocalis, data.treatmentDescription)
      : null;

    if (existing) {
      if (existing.isLocked) {
        return reply.code(403).send({ error: "DiaryLocked", message: "Дневник подписан и заблокирован." });
      }
      ...
      await db.update(visitDiaries).set({
        ...
        treatmentDescription: data.treatmentDescription ?? existing.treatmentDescription,
        updatedAt: new Date(),
      ...
      const inserted = await db.insert(visitDiaries).values({
  ...
  app.post("/api/diaries/:id/lock", async (req, reply) => {
    ...
    if (existing.isLocked) return reply.code(409).send({ error: "AlreadyLocked", hash: existing.diaryHash });
  ...
  app.post("/api/diaries/:id/revise", async (req, reply) => {
    if (!(await requireClinicalMutationAccess(req, reply, "revise locked diary"))) return;
    const { id } = req.params as { id: string };
    const userContext = (req as any).user;
    const userId: string | null = userContext?.id ?? null;
    const role: string = userContext?.role ?? "assistant";
    ...
    if (role !== "admin") {
      return reply.code(403).send({ error: "OnlyAdminsCanRevise", message: "Ревизия заблокированного дневника доступна только администратору." });
    }
    ...
    if (!existing) return reply.code(404).send({ error: "NotFound" });
    if (!existing.isLocked) return reply.code(409).send({ error: "NotLocked", message: "Дневник не подписан — просто редактируйте его." });
    ...
    await db.update(visitDiaries).set({
      ...
      treatmentDescription: body.treatmentDescription ?? existing.treatmentDescription,
      diaryHash: newHash,
