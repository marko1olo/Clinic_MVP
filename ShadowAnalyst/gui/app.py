...
GROQ_API_KEYS = [k.strip() for k in env_keys.split(",")] if env_keys else config.get("groq_api_keys", DEFAULT_CONFIG["groq_api_keys"])
GROQ_VISION_MODEL = config.get("groq_vision_model", DEFAULT_CONFIG["groq_vision_model"])

MQTT_HOST = os.getenv("MQTT_HOST", config.get("mqtt_host", DEFAULT_CONFIG["mqtt_host"]))
MQTT_PORT = int(os.getenv("MQTT_PORT", config.get("mqtt_port", DEFAULT_CONFIG["mqtt_port"])))
MQTT_USER = os.getenv("MQTT_USER", config.get("mqtt_user", DEFAULT_CONFIG["mqtt_user"]))
