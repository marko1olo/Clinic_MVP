...
function _sourceHint(source) {
    ...
    if (source === "import")
        return "Административная диктовка для импорта.";
    if (source === "document")
        return "Диктовка для черновика документа.";
    return "Тестовая диктовка в настройках.";
}
import { Buffer } from "node:buffer";
function trimPrompt(value, maxBytes) {
...
export function getDentalSttPromptPolicy() {
    ...
    if (customTerms().length)
        warnings.push("К стандартному словарю распознавания добавлены термины клиники из серверных настроек.");
    return {
