...
async function run() {
  ...
  ws.on('open', () => {
    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
          send('Runtime.evaluate', {
              expression: '(() => { const amount = document.querySelector("#payment-amount-input"); if(amount) { amount.focus(); amount.value = "1200"; amount.dispatchEvent(new Event("input", { bubbles: true })); } const btn = document.querySelector("#payment-capture button.primary-button"); if (btn) { btn.click(); return "CLICKED"; } return "NO_BUTTON"; })()'
          });
          
          setTimeout(() => {
            send('Runtime.evaluate', {
              expression: 'document.querySelector("#finance.finance-panel") ? "YES_FINANCE" : document.querySelector("#root") ? document.querySelector("#root").innerHTML.substring(0, 500) : "NO_FINANCE"'
            });
          }, 1000);
        }, 4000);
    }, 4000);
