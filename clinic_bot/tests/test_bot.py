...
class TestHandleAlertAdmin(unittest.TestCase):
    ...
    def test_handle_alert_admin_with_text(
        self, mock_broadcast, mock_run_coroutine_threadsafe
    ):
        loop = MagicMock()
    ...
    @patch("bot.asyncio.run_coroutine_threadsafe")
    @patch("bot.broadcast", new_callable=MagicMock)
    def test_handle_alert_admin_without_text(
        self, mock_broadcast, mock_run_coroutine_threadsafe
    ):
        loop = MagicMock()
        payload = {"key": "value"}
    ...
    @patch("bot.asyncio.run_coroutine_threadsafe")
    @patch("bot.broadcast", new_callable=MagicMock)
    def test_handle_alert_admin_with_non_dict_payload(
        self, mock_broadcast, mock_run_coroutine_threadsafe
    ):
        loop = MagicMock()
        payload = ["this", "is", "a", "list"]
...
class TestBotMqtt(unittest.TestCase):
    ...
    def test_on_mqtt_message_exception_handling(self):
        ...
        self.assertTrue(
            any(
                "Error processing MQTT message: Simulated decode error" in log_msg
    ...
    def test_on_mqtt_message_json_decode_error(self, mock_handle_default):
        ...
        mock_handle_default.assert_called_once_with(
            "test/topic", {"text": "{invalid_json}"}, loop
        )
...
class TestHandleXrayResult(unittest.TestCase):
    ...
    @patch("bot.broadcast", new_callable=MagicMock)
    def test_handle_xray_result_with_image(
        self, mock_broadcast, mock_broadcast_photo, mock_run_coroutine_threadsafe
    ):
        ...
        mock_broadcast.assert_called_once_with(
            "🔄 *Система*: Снимок xray.jpg (Пациент: Ivan Ivanov) отправлен врачам.",
            role="admin",
        )
    ...
    @patch("bot.broadcast_photo", new_callable=MagicMock)
    @patch("bot.broadcast", new_callable=MagicMock)
    def test_handle_xray_result_without_image(
        self, mock_broadcast, mock_broadcast_photo, mock_run_coroutine_threadsafe
    ):
        mock_broadcast.return_value = "mocked_coro"

        loop = MagicMock()
        payload = {"report": "Test report", "patient_name": "Ivan Ivanov"}
        ...
        mock_broadcast_photo.assert_not_not_called = (
            lambda: mock_broadcast_photo.assert_not_called()
        )
        mock_broadcast_photo.assert_not_called()

    @patch("bot.asyncio.run_coroutine_threadsafe")
    @patch("bot.broadcast", new_callable=MagicMock)
    def test_handle_xray_result_patient_name_formatting(
        self, mock_broadcast, mock_run_coroutine_threadsafe
    ):
        mock_broadcast.return_value = "mocked_coro"
        ...
        payload_unknown = {"report": "Test report", "patient_name": "Неизвестен"}
        ...
        payload_missing = {"report": "Test report"}
        ...
        payload_valid = {"report": "Test report", "patient_name": "Petr Petrov"}
    ...
    @patch("bot.broadcast_photo", new_callable=MagicMock)
    @patch("bot.broadcast", new_callable=MagicMock)
    def test_on_mqtt_message_xray_result(
        self, mock_broadcast, mock_broadcast_photo, mock_run_coroutine_threadsafe
    ):
        import json
...
class TestHandleMarketingSend(unittest.TestCase):
    ...
    @patch("bot.asyncio.run_coroutine_threadsafe")
    @patch("bot.broadcast", new_callable=MagicMock)
    def test_handle_marketing_send_missing_fields(
        self, mock_broadcast, mock_run_coroutine_threadsafe
    ):
        # Setup
        topic = "test/marketing/send"
        ...
        # Assert
        expected_text = f"📣 *Маркетинг — задание*\n\n" f"Пациент: ?\n" f"Черновик: __"
        mock_broadcast.assert_called_once_with(expected_text, role="admin")
        mock_run_coroutine_threadsafe.assert_called_once_with(mock_coroutine, loop)
...
class TestStartMqtt(unittest.TestCase):
    ...
    def test_start_mqtt_exception_handling(self, mock_sleep, mock_mqtt_client_class):
        ...
        mock_client.loop_forever.side_effect = [
