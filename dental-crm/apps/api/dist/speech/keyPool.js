...
const keyHealthByFingerprint = new Map();
...
function normalizePersistedHealth(value) {
    ...
    const lastUsedAt = candidate.lastUsedAt === null || candidate.lastUsedAt === undefined
        ? null
        : Number(candidate.lastUsedAt);
    const lastStatusCode = candidate.lastStatusCode === null || candidate.lastStatusCode === undefined
        ? null
    ...
    if (lastUsedAt !== null && !Number.isFinite(lastUsedAt))
        return null;
    if (lastStatusCode !== null && !Number.isFinite(lastStatusCode))
        return null;
    return {
...
function pruneKeyHealth(now = Date.now()) {
...
function loadKeyHealthFromDisk() {
    if (keyHealthLoadedFromDisk)
        return;
    ...
    try {
        ...
        for (const [key, value] of Object.entries(health)) {
            if (!isPersistedHealthKey(key))
                continue;
            const normalized = normalizePersistedHealth(value);
            if (normalized)
                keyHealthByFingerprint.set(key, normalized);
        }
        pruneKeyHealth();
    }
    catch {
...
export function speechProviderTimeoutMs() {
...
function healthKey(providerId, candidate) {
    return `${providerId}:${candidate.fingerprint}`;
}
function healthFor(providerId, candidate) {
    loadKeyHealthFromDisk();
    return (keyHealthByFingerprint.get(healthKey(providerId, candidate)) ?? {
        cooldownUntil: 0,
        failures: 0,
        successes: 0,
        lastUsedAt: null,
...
function isLikelyTransientProviderError(error) {
...
export function getProviderKeyCandidates(providerId) {
    ...
    for (const baseName of spec.numberedBases) {
        for (let index = 1; index <= maxNumberedKeys(); index += 1) {
            ...
            if (value)
                values.push({ value, source: `${baseName}_${index}` });
        }
    }
    for (const envName of spec.singles) {
        const value = process.env[envName]?.trim();
        if (value)
            values.push({ value, source: envName });
    }
    const seen = new Set();
    return values
        .filter((candidate) => {
...
export function getProviderAcceptedKeyEnvVars(providerId) {
    const spec = providerKeySpecs[providerId];
    return [
        ...spec.lists,
...
export function providerKeyCount(providerId) {
    return getProviderKeyCandidates(providerId).length;
}
export function keyRetryLimit(providerId) {
    const configuredKeyCount = providerKeyCount(providerId);
    if (!configuredKeyCount)
        return 0;
...
export function getProviderKeyPoolSummary(providerId) {
...
export function getProviderKeyHealthSnapshots(providerId) {
    const now = Date.now();
...
export function selectProviderKey(providerId, triedFingerprints = new Set()) {
    const now = Date.now();
...
export function recordProviderKeySuccess(providerId, candidate) {
...
export function recordProviderKeyFailure(providerId, candidate, error) {
    const previous = healthFor(providerId, candidate);
...
export function shouldTryNextProviderKey(error) {
    if (!(error instanceof SpeechProviderRequestError))
        return isLikelyTransientProviderError(error);
    if (error.rateLimited || error.timedOut || error.retryable)
        return true;
    return error.statusCode === 401 || error.statusCode === 403;
}
export function providerHttpError(statusCode, statusText, message) {
...
export async function fetchWithProviderTimeout(input, init = {}, timeoutMs = speechProviderTimeoutMs()) {
    ...
    try {
        ...
        const isNetworkError = error instanceof Error &&
            (/fetch failed|network|econnreset|econnrefused|etimedout|timeout|socket|terminated|dns|enotfound/i.test(error.message) ||
