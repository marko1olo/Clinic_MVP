import { clinicModeSchema, clinicScheduleDefaultsSchema, staffRoleSchema, staffWorkingHoursSchema, } from "@dental/shared";
import { and, asc, desc, eq, isNull, sql } from "drizzle-orm";
import { createChair as createChairInMemory, createStaffMember as createStaffMemberInMemory, deactivateChair as deactivateChairInMemory, deactivateStaffMember as deactivateStaffMemberInMemory, buildClinicSettings as getClinicSettingsInMemory, updateChairProfile as updateChairProfileInMemory, updateChairWorkingHours as updateChairWorkingHoursInMemory, updateClinicMode as updateClinicModeInMemory, updateClinicProfile as updateClinicProfileInMemory, updateStaffMemberProfile as updateStaffMemberProfileInMemory, updateStaffWorkingHours as updateStaffWorkingHoursInMemory, } from "../sampleData.js";
 ...
 */
export function stampedUiPreferencesSavedAt(savedAt) {
 ...
 * правило в памяти.
 */
export function uiPreferencesSaveIsSuperseded(storedSavedAt, incomingSavedAt) {
    if (!storedSavedAt || !incomingSavedAt)
        return false;
    if (!Number.isFinite(Date.parse(storedSavedAt)))
        return false;
    return (Date.parse(stampedUiPreferencesSavedAt(incomingSavedAt)) <
 ...
 */
async function uiPreferencesRow(organizationId) {
    const [row] = await db
...
export async function saveUiPreferencesInDb(organizationId, prefs) {
    ...
    for (let attempt = 1; attempt <= UI_PREFERENCES_SAVE_ATTEMPTS; attempt += 1) {
        ...
        const stored = (row.uiPreferences ?? null);
        if (stored &&
            uiPreferencesSaveIsSuperseded(stored.savedAt, prefs.savedAt)) {
            return { applied: false, stored };
        }
        ...
        const written = await db
            ...
            .where(and(eq(schema.users.id, row.id), stored === null
...
export async function updateClinicProfileInDb(organizationId, input) {
    ...
    if (input.inn !== undefined)
        updateData.inn = input.inn;
    if (input.kpp !== undefined)
        updateData.kpp = input.kpp;
    if (input.ogrn !== undefined)
        updateData.ogrn = input.ogrn;
    if (input.address !== undefined)
        updateData.legalAddress = input.address;
    ...
    if (input.phone !== undefined)
        clinicUpdateData.phone = input.phone;
 ...
 */
export async function updateStaffMemberProfileInDb(organizationId, staffId, input) {
    if (useInMemory()) {
 ...
 */
export async function deactivateStaffMemberInDb(organizationId, staffId) {
    if (useInMemory()) {
...
const COMMISSION_STORAGE_UNAVAILABLE = "Ставка врача не сохранена: хранение отключено (DENTAL_STATE_PERSISTENCE=off), ставки живут только в базе. Включите базу и повторите.";
/**
 ...
 * посчитаются деньги.
 */
export async function listDoctorCommissionRatesInDb(organizationId) {
    if (useInMemory())
        return [];
    const rows = await db
        .select({
        ...
        .where(and(eq(schema.doctorCommissions.organizationId, organizationId), eq(schema.doctorCommissions.isActive, true)))
 ...
 */
export async function setDoctorCommissionRateInDb(organizationId, staffId, commissionPct) {
    if (useInMemory())
        throw new Error(COMMISSION_STORAGE_UNAVAILABLE);
    ...
    return db.transaction(async (tx) => {
        const [previous] = await tx
            ...
            .from(schema.doctorCommissions)
            .where(and(eq(schema.doctorCommissions.organizationId, organizationId), eq(schema.doctorCommissions.userId, staffId), eq(schema.doctorCommissions.isActive, true)))
            .orderBy(desc(schema.doctorCommissions.effectiveFrom), desc(schema.doctorCommissions.createdAt))
            .limit(1);
        await tx
        ...
        if (!inserted)
            throw new Error("Ставка врача не сохранена: строка ставки не записалась.");
        return {
            userId: staffId,
 ...
 */
export async function updateChairProfileInDb(organizationId, chairId, input) {
    if (useInMemory()) {
 ...
 */
export async function deactivateChairInDb(organizationId, chairId) {
    if (useInMemory()) {
