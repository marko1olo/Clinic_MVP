...
import { auditEvents, organizations, userInvitations, users, } from "../db/schema.js";
...
export async function registerAuthRoutes(app) {
    ...
    app.post("/api/auth/clinic/login", {
        ...
        const isDemoClinicLogin = demoLoginAllowed() &&
        ...
        let org;
        try {
            const lookup = await readUnderBypass((tx) => tx
                .select()
                .from(organizations)
    ...
    app.post("/api/auth/clinic/set-password", async (request, reply) => {
        ...
        const hasValidSetupKey = !!setupKey &&
    ...
    app.post("/api/auth/staff/set-pin", async (request, reply) => {
        ...
        if (!targetOrganizationId) {
            const owner = await readUnderBypass((tx) => tx
    ...
    app.post("/api/auth/setup/init", async (request, reply) => {
        ...
        const duplicate = await readUnderBypass((tx) => tx
    ...
    app.post("/api/auth/register", async (request, reply) => {
        ...
        const duplicateOrg = await readUnderBypass((tx) => tx
            .select({ id: organizations.id })
            .from(organizations)
            .where(eq(organizations.loginId, loginId))
            .limit(1));
        ...
        const duplicateUser = await readUnderBypass((tx) => tx
            .select({ id: users.id })
            .from(users)
    ...
    app.post("/api/auth/invites/create", async (request, reply) => {
        ...
        if (!staffPayload?.organizationId ||
            !ADMIN_ROLES.some((allowed) => allowed === invitingRole)) {
            return reply.code(403).send({
                error: "Forbidden",
                message: "Приглашать сотрудников может владелец клиники или администратор.",
            });
        }
    ...
    app.post("/api/auth/invites/accept", async (request, reply) => {
        ...
        const inviteLookup = await readUnderBypass((tx) => tx
            .select()
            .from(userInvitations)
            .where(and(eq(userInvitations.inviteToken, token), eq(userInvitations.status, "pending")))
            .limit(1));
