...
export default async function (fastify) {
	fastify.post("/scheduler/draft-from-plan", async (request, reply) => {
		const { treatmentPlanId } = request.body;
		if (!treatmentPlanId) {
		...
		const items = await db
			...
			.where(eq(treatmentPlanItemsNew.planId, treatmentPlanId));
		if (items.length === 0) {
		...
		if (surgicalItems.length === 0) {
			return reply
				.code(400)
				.send({ error: "No planned items available for scheduling" });
		}
