...
function speechPolishFailureReason(error) {
    if (error instanceof SpeechProviderRequestError) {
        if (error.timedOut)
            return "серверная очистка не ответила вовремя";
        ...
        if (error.statusCode)
            return "сервис очистки отклонил запрос";
    }
...
function createSpeechPolishConfig() {
    ...
    if (!requested) {
        ...
        warnings.push(keyPool && keyPool.configuredKeyCount > 0
...
function safeParseJsonObject(text) {
    try {
        ...
        if (!match?.[0])
            throw new Error("Дополнительная очистка диктовки вернула нечитаемый структурированный ответ.");
...
async function callOpenAiCompatiblePolishWithKeyRotation(input) {
    ...
    for (const fallback of DENTAL_AI_CASCADING_MODELS) {
        ...
        try {
            ...
            if (!fallbackBaseUrl || !fallbackKeyProviderId)
                continue;
            const triedFingerprints = new Set();
