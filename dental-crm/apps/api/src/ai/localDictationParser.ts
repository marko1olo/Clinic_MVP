...
export interface ToothUpdate {
  code: string;
  state: "treatment" | "missing" | "watch" | "planned" | "done" | "prosthetics" | "implant" | "calculus";
}
...
export interface SmartAction {
  action: "update_tooth" | "schedule" | "reschedule" | "cancel_schedule" | "open_card" | "add_implant" | "create_patient" | "complex_llm_fallback";
  payload?: any;
}
...
const WEEKDAYS: Record<string, number> = {
  "понедельник": 1, "вторник": 2, "сред": 3, "четверг": 4, "пятниц": 5, "суббот": 6, "воскресень": 0
};

const TOOTH_REGEX = /(?:^|[^0-9])([1-4][1-8]|[5-8][1-5])(?=[^0-9]|$)(?!\s*[:.\-]\s*\d+)/gi;

const STATE_MAPPING: Record<string, ToothUpdate['state']> = {
  "кариес": "treatment", "пульпит": "treatment", "периодонтит": "treatment", "эндодонтия": "treatment", "лечить": "treatment", "лечение": "treatment", "пломба": "treatment",
  "отсутствует": "missing", "удален": "missing", "удалили": "missing", "удалить": "missing", "нет": "missing", "экстракция": "missing",
  "план на имплант": "planned", "имплантация": "planned", "план имплантации": "planned",
  "наблюдать": "watch", "осмотр": "watch", "наблюдение": "watch", "рентген": "watch", "кт": "watch",
  "вылечен": "done", "здоров": "done", "санирован": "done",
  "коронка": "prosthetics", "коронку": "prosthetics", "винир": "prosthetics", "мост": "prosthetics",
  "имплант": "implant", "имплантат": "implant", "имплантан": "implant",
  "налет": "calculus", "камень": "calculus", "зубной камень": "calculus", "чистка": "calculus", "профгигиена": "calculus"
};

const NUMBER_WORDS: Record<string, number> = {
  "ноль": 0, "один": 1, "одна": 1, "первого": 1, "первый": 1, "первое": 1, 
  "два": 2, "две": 2, "второго": 2, "второй": 2, "второе": 2,
  "три": 3, "третьего": 3, "третий": 3, "третье": 3,
  "четыре": 4, "четвертого": 4, "четвертый": 4, "четвертое": 4,
  "пять": 5, "пятого": 5, "пятый": 5, "пятое": 5,
  "шесть": 6, "шестого": 6, "шестой": 6, "шестое": 6,
  "семь": 7, "седьмого": 7, "седьмой": 7, "седьмое": 7,
  "восемь": 8, "восьмого": 8, "восьмой": 8, "восьмое": 8,
  "девять": 9, "девятого": 9, "девятый": 9, "девятое": 9,
  "десять": 10, "десятого": 10, "десятый": 10, "десятое": 10,
  "одиннадцать": 11, "одиннадцатого": 11, "одиннадцатый": 11, "одиннадцатое": 11,
  "двенадцать": 12, "двенадцатого": 12, "двенадцатый": 12, "двенадцатое": 12,
  "тринадцать": 13, "тринадцатого": 13, "тринадцатый": 13, "тринадцатое": 13,
  "четырнадцать": 14, "четырнадцатого": 14, "четырнадцатый": 14, "четырнадцатое": 14,
  "пятнадцать": 15, "пятнадцатого": 15, "пятнадцатый": 15, "пятнадцатое": 15,
  "шестнадцать": 16, "шестнадцатого": 16, "шестнадцатый": 16, "шестнадцатое": 16,
  "семнадцать": 17, "семнадцатого": 17, "семнадцатый": 17, "семнадцатое": 17,
  "восемнадцать": 18, "восемнадцатого": 18, "восемнадцатый": 18, "восемнадцатое": 18,
  "девятнадцать": 19, "девятнадцатого": 19, "девятнадцатый": 19, "девятнадцатое": 19,
  "двадцать": 20, "двадцатого": 20, "двадцатый": 20, "двадцатое": 20,
  "тридцать": 30, "тридцатого": 30, "тридцатый": 30, "тридцатое": 30,
...
function extractTime(text: string): string | null {
  ...
  m = text.match(/(?:^|\s)(?:в|на)\s+(\d{1,2}|[а-яё]+)\s*(?:час(?:а|ов)?|утра|дня|вечера)(?!\s*\d)/i);
  if (!m) m = text.match(/(?:^|\s)(?:в|на)\s+(\d{1,2}|[а-яё]+)(?!\s*\d)/i);
  if (m) {
    let h = parseInt(m[1] as string, 10);
...
function extractDate(text: string): { dayString?: string, relativeDays?: number, targetWeekday?: number } | null {
  ...
  let m = text.match(/(\d{1,2}|[а-яё]+)\s*([а-яё]+)/i);
  if (m) {
...
function extractPatientName(text: string, actionRegexStr: string): string | null {
...
function extractCost(text: string): number | null {
  ...
  const mWord = text.match(/(?:стоимость|цена|оплата|взяли|с пациента|оплатила|оплатил)\s+([а-яё\s]+)\s*(тысяч|тыс|руб|р|₽)/i);
  if (mWord) {
    let sum = 0;
    let hasHalf = false;
    ...
    if (sum > 0) {
      if (hasHalf) sum += 0.5;
      if ((mWord[2] as string).startsWith("тыс") || (mWord[1] as string).includes("тысяч") || (mWord[1] as string).includes("тыс")) {
        sum *= 1000;
      }
      return sum;
    }
...
function extractEmkSections(text: string, updates: EmkUpdates) {
  ...
  const tokens = text.split(/(^|[^а-яё])(жалоб[а-я]*|анамнез[а-я]*|объективн[а-я]*|осмотр[а-я]*|диагноз[а-я]*|лечени[а-я]*|план[а-я]*|рекомендаци[а-я]*|проведен[а-я]*)([^а-яё]|$)/i);
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    const isKeyword = /^(жалоб[а-я]*|анамнез[а-я]*|объективн[а-я]*|осмотр[а-я]*|диагноз[а-я]*|лечени[а-я]*|план[а-я]*|рекомендаци[а-я]*|проведен[а-я]*)$/i.test(lowerToken);

    if (isKeyword) {
      if (lowerToken.startsWith("жалоб")) matchedSection = "complaint";
      else if (lowerToken.startsWith("анамнез")) matchedSection = "anamnesis";
      else if (lowerToken.startsWith("объектив") || lowerToken.startsWith("осмотр")) matchedSection = "objectiveStatus";
      else if (lowerToken.startsWith("диагноз")) matchedSection = "diagnosis";
      else if (lowerToken.startsWith("лечен") || lowerToken.startsWith("план") || lowerToken.startsWith("рекомендац") || lowerToken.startsWith("проведен")) matchedSection = "treatmentPlan";
    }
    ...
    if (matchedSection) {
       ...
       if (matchedSection !== currentSection) {
         ...
         if (currentSection && currentContent.trim()) {
            updates[currentSection] = (updates[currentSection] ? updates[currentSection] + " " : "") + currentContent.trim().replace(/^[:\-]+/, '').trim();
         }
...
function expandToothRanges(text: string): string[] {
  ...
  if (text.includes("все зубы") || text.includes("обе челюсти") || text.includes("санаци")) {
...
export function parseDictationLocally(transcript: string, context: ParserContext): SmartAction | null {
  ...
  if (context === "schedule" || text.includes("пациент")) {
    if (text.match(/(отмени|удали).*(запись|прием)|отмени/)) {
      const pName = extractPatientName(text, "отмени|удали(?:ть)?\\s*(?:запись|прием)?");
    ...
    if (text.match(/(перенеси|перенести)/)) {
      ...
      if (pName && (dateInfo || timeStr)) {
        return {
          action: "reschedule",
          payload: { patientName: pName, dayOffset: dateInfo?.relativeDays, targetWeekday: dateInfo?.targetWeekday, exactDate: dateInfo?.dayString, time: timeStr }
        };
      }
      return null; // Fallback to LLM
    }
    if (text.match(/(запиши|записать|создай запись|запись для|новый пациент)/)) {
      const dateInfo = extractDate(text);
      const timeStr = extractTime(text);
      ...
      const pName = extractPatientName(text, "запиши|записать|создай запись(?: для)?|запись для|новый пациент");
  ...
  if (openCardMatch && ((openCardMatch[1] as string)?.length ?? 0) > 3) {
    return { action: "open_card", payload: { patientName: (openCardMatch[1] as string)?.trim() } };
  }

  const implantMatch = text.match(/(?:поставь|добавь)?\s*имплант(?:ат)?\s*(\d+[\.,]?\d*)\s*(?:на|x|х)\s*(\d+[\.,]?\d*)/i);
  if (implantMatch && !text.includes("зуб")) {
    return { action: "add_implant", payload: { diameter: parseFloat((implantMatch[1] as string)?.replace(',', '.') || "0"), length: parseFloat((implantMatch[2] as string)?.replace(',', '.') || "0") } };
  }
  ...
  if (context === "visit") {
    ...
    const hasStructuredEmk = !!(emkUpdates.complaint || emkUpdates.objectiveStatus || emkUpdates.diagnosis || emkUpdates.treatmentPlan);
      ...
      for (const clause of clauses) {
        ...
        for (const [keyword, state] of Object.entries(STATE_MAPPING)) {
          ...
          if (regex.test(clause)) {
            ...
            if (!hasStructuredEmk) {
              if (state === "treatment") {
                if (!emkUpdates.complaint) emkUpdates.complaint = (clause as string).trim();
                if (!emkUpdates.diagnosis) emkUpdates.diagnosis = keyword;
              } else if (state === "implant" || state === "prosthetics") {
                if (!emkUpdates.treatmentPlan) emkUpdates.treatmentPlan = clause.trim();
              }
            }
            break;
          }
        ...
        if (foundState) {
          ...
          targetTeeth.forEach(code => {
             if (!toothUpdates.some(tu => tu.code === code && tu.state === foundState)) {
    ...
    if (hasValidMatch || cost || hasStructuredEmk) {
      if (emkUpdates.complaint && !hasStructuredEmk) emkUpdates.complaint = emkUpdates.complaint.charAt(0).toUpperCase() + emkUpdates.complaint.slice(1);
