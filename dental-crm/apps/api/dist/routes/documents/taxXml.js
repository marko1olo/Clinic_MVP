...
import { apiError, configuredTaxOfficeCode, documentIssueBlockReason, documentIssueChainBlockReason, findIssuedDuplicateTaxCertificate, frozenTaxXmlClinicProfile, frozenTaxXmlPatient, frozenTaxXmlPayments, resolveDocumentRenderContext, taxSnapshotDocument, taxXmlSourceSnapshotSha256, } from "../documents.js";
...
async function handleGetTaxXml(request, reply) {
    ...
    if (taxDocumentUsesPaymentSnapshot(document.kind) && !taxPaymentSnapshot) {
        ...
        if (duplicateTaxCertificate) {
            return reply
                .code(409)
                .send(apiError("За этот налоговый год и этого налогоплательщика уже выдана налоговая справка. XML ФНС нельзя собирать до аннулирования или корректировки предыдущей справки."));
        }
        return reply
            .code(409)
