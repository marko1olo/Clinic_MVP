...
def inject_dummy_data():
    ...
    if names:
        ...
        c.execute(
            f"SELECT name FROM patients WHERE name IN ({placeholders})", names)
    ...
    if new_patients_data:
        c.executemany(
            'INSERT INTO patients (name, phone, created_at) VALUES (?, ?, ?)', new_patients_data)
        ...
        c.execute(
            f"SELECT id FROM patients WHERE name IN ({placeholders_new})", inserted_names)
        ...
        appointments_data = [
            (pid, 'Др. Хаус', old_date, 'completed', now.isoformat()) for pid in inserted_ids]
