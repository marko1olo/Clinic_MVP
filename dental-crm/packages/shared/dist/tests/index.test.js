...
describe("documentAmountSource", () => {
    test("returns expected amount source for different document kinds", () => {
        ...
        assert.strictEqual(documentAmountSource("prescription_medication_order"), "none");
    });
    test("handles all valid document kinds without throwing and returns a valid source", () => {
        ...
        for (const kind of documentKindSchema.options) {
            ...
            assert.ok(validSources.includes(result), `Invalid source '${result}' for document kind '${kind}'`);
        }
    });
});
// ════════════════════════════════════════════════════════════════════
...
describe("documentRequiresPaidRecord", () => {
    ...
    test("edge cases: handles invalid or missing document kinds gracefully", () => {
        assert.strictEqual(documentRequiresPaidRecord(""), false);
        assert.strictEqual(documentRequiresPaidRecord("invalid_kind"), false);
        assert.strictEqual(documentRequiresPaidRecord(undefined), false);
        assert.strictEqual(documentRequiresPaidRecord(null), false);
    });
});
// ════════════════════════════════════════════════════════════════════
...
describe("documentPayloadDisallowedKeys", () => {
    ...
    test("returns array of disallowed keys when payload contains keys not allowed for the given kind", () => {
        ...
        assert.deepStrictEqual(documentPayloadDisallowedKeys("patient_intake_questionnaire", payload), ["paidMedicalServicesContract"]);
    });
...
describe("buildRuleBasedVisitDraftFromTranscript", () => {
    test("happy path: extracts all sections perfectly from a structured transcript", () => {
        ...
        assert.strictEqual(draft.anamnesis, "зуб болит уже третий день, аллергии на медикаменты нет");
        assert.strictEqual(draft.objectiveStatus, "глубокая кариозная полость, перкуссия резко болезненна");
        assert.strictEqual(draft.diagnosis, "острый верхушечный периодонтит 36 зуба");
        assert.strictEqual(draft.treatmentPlan, "анестезия, коффердам, экстирпация пульпы, механическая и медикаментозная обработка корневых каналов, пломбирование каналов гуттаперчей, временная пломба");
