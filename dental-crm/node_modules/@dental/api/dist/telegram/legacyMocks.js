...
import { createAiRecognitionJobSchema, createClinicalRuleSchema, denteTelegramChatLinkListResponseSchema, denteTelegramChatLinkPublicSchema, denteTelegramLinkCodeCreatedSchema, denteTelegramLinkCodeListResponseSchema, denteTelegramMessagePreviewSchema, denteTelegramOutboxResponseSchema, documentKindMetadata, uiPreferencesSchema, } from "@dental/shared";
...
export const denteTelegramBotSettings = {
    version: 1,
    organizationId,
...
function syncDenteTelegramOutboxDeliveryReceiptsMap() {
...
export const treatmentPlanScenarios = [
    {
...
export const aiRecognitionJobs = [];
export const imagingViewerSessions = [];
export const dicomWorkbenchBundles = [];
export const speechTranscriptionChunks = [];
...
export const visitSaveReceipts = [];
export const visitDraftAutosaves = [];
export function findVisitById(visitId) {
...
function buildVisitNoteChecklistItem() {
    const visitNoteReady = Boolean(activeVisit.complaint &&
...
function buildClinicalRulesChecklistItem(clinical) {
    return {
        id: "clinical-rules",
        visitId: activeVisit.id,
...
function buildImagingReviewChecklistItem() {
    const activeImages = imagingStudies.filter((study) => study.patientId === activeVisit.patientId &&
...
function buildLegalDocumentsChecklistItem() {
    const activeDocuments = documents.filter((document) => document.patientId === activeVisit.patientId &&
    ...
    const missingDocumentKinds = requiredDocumentKinds.filter((kind) => !activeDocuments.some((document) => document.kind === kind));
    return {
        id: "legal-documents",
        visitId: activeVisit.id,
...
function buildAiDraftReviewChecklistItem() {
    const hasReviewedAiDraft = aiRecognitionJobs.some((job) => job.patientId === activeVisit.patientId &&
...
function buildPaymentLinkChecklistItem(billing) {
    const formatRub = (amountRub) => `${amountRub.toLocaleString("ru-RU")} ₽`;
    return {
        id: "payment-link",
        visitId: activeVisit.id,
...
function buildPostVisitInstructionsChecklistItem() {
    const postVisitInstruction = communicationTasks.find((task) => task.visitId === activeVisit.id &&
...
function buildVisitCloseChecklist() {
...
function summarizeClinicalEvaluations(evaluations) {
...
function normalizedClinicalRuleServiceIds(values) {
    return Array.from(new Set(values.map((value) => value.trim()).filter(Boolean))).slice(0, 80);
}
export function createClinicalRule(input) {
...
export function updateClinicalRule(input) {
...
function isOpenCommunicationTask(task) {
...
export function buildCommunicationSummary() {
    ...
    return {
        ...
        completedToday: communicationTasks.filter((task) => task.status === "completed" &&
            task.lastEventAt?.startsWith(todayPrefix)).length,
...
function buildPatientInsights() {
    ...
    for (const doc of documents) {
        if (doc.patientId === null)
            continue;
        if (!documentsByPatient.has(doc.patientId))
            documentsByPatient.set(doc.patientId, []);
        documentsByPatient.get(doc.patientId).push(doc);
    }
    const tasksByPatient = new Map();
    for (const task of communicationTasks) {
        if (isOpenCommunicationTask(task) && task.patientId !== null) {
            if (!tasksByPatient.has(task.patientId))
                tasksByPatient.set(task.patientId, []);
            tasksByPatient.get(task.patientId).push(task);
        }
    }
    const imagesByPatient = new Map();
    for (const image of imagingStudies) {
        if (image.patientId === null)
            continue;
        if (!imagesByPatient.has(image.patientId))
            imagesByPatient.set(image.patientId, []);
    ...
    for (const payment of payments) {
        if (payment.status === "paid" && payment.patientId !== null) {
            if (!paymentsByPatient.has(payment.patientId))
                paymentsByPatient.set(payment.patientId, []);
            paymentsByPatient.get(payment.patientId).push(payment);
        }
    }
    const planItemsByPatient = new Map();
    for (const item of treatmentPlanItems) {
        if (item.patientId === null)
            continue;
        if (!planItemsByPatient.has(item.patientId))
            planItemsByPatient.set(item.patientId, []);
    ...
    return patients.map((patient) => {
        ...
        const plannedRub = patientPlanItems.reduce((total, item) => total +
            Math.max(0, item.quantity * item.unitPriceRub - item.discountRub), 0);
...
function normalizeOptionalWeekdays(value) {
    const rawDays = Array.isArray(value) ? value : [];
    return Array.from(new Set(rawDays
        .filter((day) => Number.isInteger(day) && day >= 0 && day <= 6)
        .sort((left, right) => left - right)));
}
function normalizeClinicScheduleDefaults(input) {
    ...
    const workdayEnd = clockToMinutes(requestedEnd) > clockToMinutes(workdayStart)
        ? requestedEnd
        : defaultClinicScheduleDefaults.workdayEnd;
    const requestedBuffer = input?.appointmentBufferMinutes;
    const buffer = typeof requestedBuffer === "number" &&
...
function defaultStaffWorkingHours() {
    ...
    if (Array.isArray(input)) {
        input.forEach((day) => {
            ...
            const requestedEnd = isClockTime(day.end)
                ? day.end
                : defaultClinicScheduleDefaults.workdayEnd;
            const end = clockToMinutes(requestedEnd) > clockToMinutes(start)
                ? requestedEnd
                : defaultClinicScheduleDefaults.workdayEnd;
...
function normalizeMutableScheduleState() {
...
export function validScheduleTimeZone(value) {
...
function appointmentClinicTimeParts(value, sourceTimeZone = clinicProfile.timezone) {
...
function appointmentClinicDateKey(value, sourceTimeZone = clinicProfile.timezone) {
    const date = new Date(value);
...
function appointmentsShareClinicDate(left, right) {
    return (appointmentClinicDateKey(left.startsAt) ===
        appointmentClinicDateKey(right.startsAt));
}
function appointmentWeekday(appointment, timeZone = clinicProfile.timezone) {
    return appointmentClinicTimeParts(appointment.startsAt, timeZone).weekday;
}
function appointmentStartMinute(appointment, timeZone = clinicProfile.timezone) {
    return appointmentClinicTimeParts(appointment.startsAt, timeZone).minute;
}
function appointmentEndMinute(appointment, timeZone = clinicProfile.timezone) {
    return appointmentClinicTimeParts(appointment.endsAt, timeZone).minute;
}
function appointmentWithinClinicScheduleDefaults(appointment, scheduleDefaults, timezone) {
...
function appointmentWithinClinicSchedule(appointment) {
    return appointmentWithinClinicScheduleDefaults(appointment, clinicProfile.scheduleDefaults, clinicProfile.timezone);
}
function appointmentWithinStaffSchedule(appointment, staff, label = "врач") {
    if (!staff)
...
function appointmentWithinChairSchedule(appointment, chair) {
    if (!chair)
...
function appointmentWithinPatientPreference(appointment, patient) {
    ...
    if (preference.preferredAppointmentStart &&
...
function staffDailyCapacityMinutes(staff) {
    return workingHoursDailyCapacityMinutes(staff.workingHours ?? null);
}
function buildAppointmentReadiness(patientInsights = buildPatientInsights()) {
    ...
    return appointments.map((appointment) => {
        ...
        const hasScheduleBlocker = !clinicScheduleCheck.ready ||
        ...
        const checks = [
            ...
            },
            {
                ...
                detail: `${doctor ? "врач есть" : "нет врача"} · ${chair ? chair.name : "нет кресла"} · ${assistant
            ...
            },
            {
                key: "schedule",
                title: "Расписание",
                ready: clinicScheduleCheck.ready &&
...
function buildRecommendedActions(patientInsights = buildPatientInsights()) {
    const actions = [];
    ...
    const taxDraft = documents.find((document) => document.kind === "tax_deduction_certificate" &&
...
function buildScheduleSuggestions(readiness = buildAppointmentReadiness()) {
    const suggestions = [];
    const priorityRank = {
        urgent: 0,
        important: 1,
    ...
    for (let index = 0; index < sorted.length - 1; index += 1) {
        ...
        if (!current || !next)
            continue;
        if (!appointmentsShareClinicDate(current, next))
            continue;
        const gapMinutes = Math.round((new Date(next.startsAt).getTime() - new Date(current.endsAt).getTime()) /
            60000);
        const sameAssistant = Boolean(current.assistantUserId &&
            current.assistantUserId === next.assistantUserId);
        const sameResource = current.doctorUserId === next.doctorUserId ||
            sameAssistant ||
    ...
    if (overbooked) {
        add({
            ...
            priority: "urgent",
            ownerRole: overbooked.kind === "doctor"
                ? "doctor"
...
function appointmentBelongsToShiftDate(appointment, shiftDate) {
    return appointmentClinicDateKey(appointment.startsAt) === shiftDate;
}
function workloadState(utilizationPercent, appointmentCount) {
    if (appointmentCount === 0)
        return "idle";
    if (utilizationPercent >= 96)
        return "overbooked";
    if (utilizationPercent >= 72)
        return "tight";
    return "healthy";
}
function buildResourceLoad(input) {
    ...
    const rawUtilizationPercent = input.capacityMinutes > 0
    ...
    if (state === "idle")
        flags.push("Нет записей на смену");
    if (state === "tight")
    ...
    if (rawUtilizationPercent > utilizationPercent)
        flags.push(`Фактическая загрузка ${rawUtilizationPercent}%, шкала ограничена 200%`);
    return {
        id: input.id,
        kind: input.kind,
        title: input.title,
...
function buildDoctorLoads() {
    ...
    return activeDoctors.map((doctor) => {
        const doctorAppointments = appointments.filter((appointment) => appointment.doctorUserId === doctor.id &&
...
function buildAssistantLoads() {
    ...
    return activeAssistants.map((assistant) => {
        const assistantAppointments = appointments.filter((appointment) => appointment.assistantUserId === assistant.id &&
            appointmentBelongsToShiftDate(appointment, shiftDate));
        return buildResourceLoad({
            id: assistant.id,
            kind: "assistant",
...
function buildChairLoads() {
    ...
    return chairs
        ...
        .map((chair) => {
        const chairAppointments = appointments.filter((appointment) => appointment.chairId === chair.id &&
            appointmentBelongsToShiftDate(appointment, shiftDate));
        return buildResourceLoad({
            id: chair.id,
            kind: "chair",
            title: chair.name,
            subtitle: [chair.room, chair.specialization].filter(Boolean).join(" · ") ||
...
function buildRoleQueues() {
    ...
    return [
        ...
        },
        {
            ...
            openItems: plannedAppointments +
                draftDocuments +
                billing.unpaidDocuments +
                communication.paymentReminders,
            nextAction: billing.totalDueRub > 0
        ...
        },
        {
            ...
            openItems: appointments.filter((appointment) => appointment.status === "confirmed")
        ...
        },
        {
            ...
            nextAction: clinicProfile.mode === "network_clinic"
...
function buildScheduleWarnings() {
    ...
    if (clinicProfile.mode === "network_clinic" &&
        !staffMembers.some((member) => member.role === "manager" || member.role === "owner")) {
        warnings.push({
...
function buildModeFit() {
    ...
    if (clinicProfile.mode === "one_chair") {
        if (activeChairs !== 1)
            blockers.push("Для режима 1 кабинета должно быть ровно одно активное кресло");
        if (doctors > 1)
            upgrades.push("Если врачи работают параллельно, включить режим малой клиники");
        upgrades.push("Держать расписание как одну очередь смены без филиальной аналитики");
    }
    if (clinicProfile.mode === "small_clinic") {
        if (doctors < 2)
            blockers.push("Малой клинике нужен минимум второй врач или внешний специалист");
        if (activeChairs < 2)
            blockers.push("Нужно минимум два кресла/кабинета для реального распределения");
        if (admins < 1)
    ...
    if (clinicProfile.mode === "network_clinic") {
        if (!clinicProfile.networkEnabled)
            blockers.push("Сетевой флаг не включен");
        if (managers < 1)
            blockers.push("Нужен управляющий или владелец");
        if (doctors < 2)
            blockers.push("Сеть без нескольких врачей не дает операционного смысла");
        if (activeChairs < 2)
            blockers.push("Нужны кабинеты/кресла по филиалам");
        upgrades.push("Добавить филиалы, централизованные шаблоны, аудит и импорт по источникам");
    }
    const fitScore = Math.max(35, 100 -
        blockers.length * 18 -
    ...
    return {
        ...
        blockers,
        upgrades,
        lowFrictionNextStep: blockers[0] ??
...
export const protocolTemplates = protocolTemplateSeeds.map((template) => ({ ...template, updatedAt: nowIso }));
...
function isLocalDicomPath(value) {
...
function redactLocalDicomPath(value) {
    if (!value)
        return null;
    if (!isLocalDicomPath(value))
        return value;
    return `redacted-local-dicom-path:${shortHash(value)}`;
}
function redactDicomReferenceId(value) {
    if (!value)
        return null;
...
function redactLocalDicomPathsInText(value) {
    return value
        .replace(/[A-Za-z]:[\\/][^\r\n]*(?=:\s|$)/g, (match) => redactLocalDicomPath(match) ?? match)
        .replace(/\\\\[^\r\n]*(?=:\s|$)/g, (match) => redactLocalDicomPath(match) ?? match)
        .replace(/dicomfile:([A-Za-z]:[\\/][^\s\r\n]+)/gi, (_match, filePath) => `dicomfile:${redactLocalDicomPath(filePath) ?? filePath}`);
}
function redactDicomWarningList(warnings) {
    return uniqueStrings(warnings
        .map((warning) => redactLocalDicomPathsInText(warning))
        .filter((warning) => warning.trim()));
}
function cloneDicomWorkbenchManifestForServerStorage(manifest) {
    ...
    if (clone.launchManifest.viewerUrl &&
...
function sanitizeDicomWorkbenchBundleForServerStorage(bundle) {
...
function normalizePostVisitCheckupDelayHoursByTopic(input) {
    const source = input && typeof input === "object" && !Array.isArray(input)
        ? input
        : {};
    const normalized = { ...defaultPostVisitCheckupDelayHoursByTopic };
    for (const key of Object.keys(defaultPostVisitCheckupDelayHoursByTopic)) {
        const value = typeof source[key] === "number"
            ? source[key]
...
export function resetToZeroMode(role) {
    patients.length = 0;
...
export function saveUiPreferences(input) {
    const incomingSavedAt = input.savedAt && Number.isFinite(Date.parse(input.savedAt))
        ? input.savedAt
        : new Date().toISOString();
    if (uiPreferences?.savedAt &&
        input.savedAt &&
...
export function getDenteTelegramBotSettings() {
    return denteTelegramBotSettings;
}
function normalizeAppointmentReminderLeadTimes(values) {
    const normalized = [
        ...new Set((values ?? [])
...
function assertTelegramPublicUrlPathIsSafe(fieldName, parsed) {
...
function safeTelegramBotUsername(value) {
    try {
...
function normalizeTelegramPublicHttpsUrl(fieldName, value) {
    ...
    const sensitiveKeys = Array.from(parsed.searchParams.keys()).filter((key) => telegramPublicUrlSensitiveQueryKeys.has(key.trim().toLowerCase()));
...
export function safeDenteTelegramPublicHttpsUrl(fieldName, value) {
    try {
...
const defaultDenteTelegramVisualCardUrls = {
    mainMenu: null,
    appointment: null,
    documents: null,
    tax: null,
...
function normalizeDenteTelegramVisualCardUrls(input) {
    const source = input && typeof input === "object" && !Array.isArray(input)
        ? input
        : {};
    return {
...
function normalizeExistingDenteTelegramVisualCardUrls(input) {
    try {
...
export function updateDenteTelegramBotSettings(input) {
    ...
    const nextSettings = {
        ...
        patientLinkTokenTtlMinutes: input.patientLinkTokenTtlMinutes ??
            denteTelegramBotSettings.patientLinkTokenTtlMinutes,
        appointmentReminderLeadTimesHours: input.appointmentReminderLeadTimesHours !== undefined
        ...
        reviewRequestDelayHours: input.reviewRequestDelayHours !== undefined
            ? normalizeReviewRequestDelayHours(input.reviewRequestDelayHours)
            : normalizeReviewRequestDelayHours(denteTelegramBotSettings.reviewRequestDelayHours),
        postVisitCheckupDelayHoursByTopic: input.postVisitCheckupDelayHoursByTopic !== undefined
        ...
        staffEscalationChannel: input.staffEscalationChannel !== undefined
...
function telegramEnvRecord(value) {
    return Boolean(value) && typeof value === "object" && !Array.isArray(value);
}
function telegramEnvString(value) {
    return typeof value === "string" && value.trim() ? value.trim() : null;
}
function denteTelegramBotConfigIdForSettings(settings = denteTelegramBotSettings, botUsername = null) {
...
function configuredClinicTelegramBotFromJson() {
    ...
    const match = records.filter(telegramEnvRecord).find((record) => {
        const organizationId = telegramEnvString(record.organizationId) ??
            telegramEnvString(record.orgId);
        const clinicId = telegramEnvString(record.clinicId);
        return (organizationId === denteTelegramBotSettings.organizationId ||
...
function configuredTelegramBotConfigId() {
    const clinicJson = configuredClinicTelegramBotFromJson();
...
function normalizeDenteTelegramBotConfigId(value) {
    return typeof value === "string" && value.trim() ? value.trim() : null;
}
function normalizeDenteTelegramBotScopedLedgers() {
    const fallbackBotConfigId = configuredTelegramBotConfigId();
    for (const linkCode of denteTelegramLinkCodes) {
        linkCode.botConfigId =
            normalizeDenteTelegramBotConfigId(linkCode.botConfigId) ??
                fallbackBotConfigId;
    }
    for (const chatLink of denteTelegramChatLinks) {
        chatLink.botConfigId =
            normalizeDenteTelegramBotConfigId(chatLink.botConfigId) ??
                fallbackBotConfigId;
    }
}
function configuredTelegramBotToken() {
    if (denteTelegramBotSettings.mode === "clinic_owned_bot") {
        return (configuredClinicTelegramBotFromJson()?.botToken ||
            process.env.DENTE_TELEGRAM_OWN_BOT_TOKEN?.trim() ||
            process.env.DENTE_TELEGRAM_CLINIC_BOT_TOKEN?.trim() ||
            null);
    }
    return (process.env.DENTE_TELEGRAM_BOT_TOKEN?.trim() ||
...
export function denteTelegramVisualCardUrlFor(settings = denteTelegramBotSettings, cardKey = "mainMenu") {
    return (safeHttpsUrl(settings.visualCardUrls?.[cardKey]) ??
        safeHttpsUrl(settings.welcomeImageUrl));
}
function denteTelegramVisualCardKeyForTemplate(templateKind) {
    if (templateKind === "appointment_reminder" ||
    ...
    if (templateKind === "payment_reminder_notice")
        return "billing";
    if (templateKind === "post_visit_instruction_link" ||
...
function denteTelegramVisualCardUrlForTemplate(templateKind, settings = denteTelegramBotSettings) {
    return denteTelegramVisualCardUrlFor(settings, denteTelegramVisualCardKeyForTemplate(templateKind));
}
function resolveDenteTelegramOutboxRuntimeScope(runtime) {
    return {
...
function denteTelegramPortalSectionForTemplate(templateKind) {
    if (templateKind === "document_ready_notice")
        return "documents";
    if (templateKind === "tax_document_request_status")
        return "tax";
...
function denteTelegramPortalUrlForSection(section, settings = denteTelegramBotSettings) {
...
function denteTelegramPortalUrlForTemplate(templateKind, settings = denteTelegramBotSettings) {
    return denteTelegramPortalUrlForSection(denteTelegramPortalSectionForTemplate(templateKind), settings);
}
function denteTelegramPortalRowForTemplate(templateKind, settings = denteTelegramBotSettings) {
    const portal = denteTelegramPortalUrlForTemplate(templateKind, settings);
    return portal ? [{ text: "Открыть DENTE", url: portal }] : [];
}
function sanitizeDenteTelegramBotSettingsInPlace() {
    ...
    const sanitized = {
        ...
        welcomeImageUrl: safeHttpsUrl(denteTelegramBotSettings.welcomeImageUrl) ??
    ...
    const changed = denteTelegramBotSettings.webhookBaseUrl !== sanitized.webhookBaseUrl ||
        ...
        JSON.stringify(denteTelegramBotSettings.postVisitCheckupDelayHoursByTopic) !== JSON.stringify(sanitized.postVisitCheckupDelayHoursByTopic);
    Object.assign(denteTelegramBotSettings, sanitized);
    if (changed)
        persistMutableState();
}
sanitizeDenteTelegramBotSettingsInPlace();
function telegramChatEncryptionKey() {
...
function encryptTelegramChatId(chatId) {
    if (!chatId)
        return null;
...
function decryptTelegramChatTransportRef(chatTransportRef) {
    if (!chatTransportRef)
        return null;
    const key = telegramChatEncryptionKey();
    if (!key)
...
function telegramChatEncryptionReady() {
    return Boolean(telegramChatEncryptionKey());
}
function chatIdLast4(chatId) {
    const normalized = chatId?.trim();
    return normalized ? normalized.slice(-4) : null;
}
function normalizeDenteTelegramLinkCode(code) {
    return code.trim().toUpperCase().replace(/\s+/g, "");
}
function fingerprintDenteTelegramLinkCode(code) {
    const salt = process.env.DENTE_TELEGRAM_LINK_CODE_SALT?.trim() ||
        denteTelegramBotSettings.organizationId;
    return createHash("sha256")
        .update(`${salt}:${normalizeDenteTelegramLinkCode(code)}`)
        .digest("hex");
}
function expireStaleDenteTelegramLinkCodes(now = new Date()) {
    let changed = false;
...
function validateDenteTelegramSubject(subjectType, subjectId, organizationScope) {
    const subject = subjectType === "patient"
        ? patients.find((patient) => patient.organizationId === organizationScope &&
            patient.id === subjectId)
        : staffMembers.find((staff) => staff.organizationId === organizationScope &&
            staff.id === subjectId);
    if (!subject) {
...
function resolveDenteTelegramClinicId(inputClinicId, organizationScope) {
    return (inputClinicId?.trim() ||
        (organizationScope === clinicProfile.organizationId
            ? clinicProfile.organizationId
            : organizationScope));
}
function publicDenteTelegramLinkCode(code) {
    const { codeFingerprint: _codeFingerprint, ...publicCode } = code;
    return publicCode;
}
export function extractDenteTelegramLinkCode(text) {
    if (!text)
        return null;
    const match = text
        .toUpperCase()
...
function normalizeDenteTelegramLedgerOptions(input, fallbackStatus) {
...
export function createDenteTelegramLinkCode(input) {
    ...
    persistMutableState();
    const botUsername = safeTelegramBotUsername(input.botUsername) ??
...
export function consumeDenteTelegramLinkCode(code, chatFingerprintValue, chatId = null, scope = {}) {
    expireStaleDenteTelegramLinkCodes();
    ...
    const linkCode = denteTelegramLinkCodes.find((candidate) => candidate.organizationId === organizationId &&
    ...
    linkCode.usedAt = now;
    let chatLink = denteTelegramChatLinks.find((candidate) => candidate.organizationId === organizationId &&
        candidate.botConfigId === botConfigId &&
...
export function listDenteTelegramLinkCodes(limit = 50) {
    expireStaleDenteTelegramLinkCodes();
    ...
    return denteTelegramLinkCodes
        .filter((linkCode) => linkCode.organizationId === denteTelegramBotSettings.organizationId &&
...
export function buildDenteTelegramLinkCodeList(input = 50) {
    expireStaleDenteTelegramLinkCodes();
    const options = normalizeDenteTelegramLedgerOptions(input, "all");
    const currentClinicId = options.clinicId;
    const visibleCodes = denteTelegramLinkCodes.filter((linkCode) => linkCode.organizationId === options.organizationId &&
...
export function listDenteTelegramChatLinks(limit = 50) {
    const currentClinicId = clinicProfile.organizationId;
    const botConfigId = configuredTelegramBotConfigId();
    return denteTelegramChatLinks
        .filter((link) => link.organizationId === denteTelegramBotSettings.organizationId &&
        link.botConfigId === botConfigId &&
        (link.clinicId === currentClinicId || link.clinicId === null))
        .slice(0, Math.max(0, Math.min(100, limit)));
}
export function buildDenteTelegramChatLinkList(input = 50) {
    const options = normalizeDenteTelegramLedgerOptions(input, "all");
    const currentClinicId = options.clinicId;
    const visibleLinks = denteTelegramChatLinks.filter((link) => link.organizationId === options.organizationId &&
    ...
    const filteredLinks = visibleLinks.filter((link) => {
        ...
        if (options.subjectType !== "all" &&
            ...
            return false;
        if (options.subjectId && link.subjectId !== options.subjectId)
            return false;
        return true;
    });
    const offset = Number.parseInt(options.cursor, 10);
...
export function revokeDenteTelegramChatLink(linkId, scope = {}) {
    const currentClinicId = scope.clinicId?.trim() || clinicProfile.organizationId;
    const organizationId = scope.organizationId?.trim() || denteTelegramBotSettings.organizationId;
    const botConfigId = scope.botConfigId?.trim() || configuredTelegramBotConfigId();
    const chatLink = denteTelegramChatLinks.find((link) => link.id === linkId &&
        link.status === "active" &&
        link.organizationId === organizationId &&
        link.botConfigId === botConfigId &&
        (link.clinicId === currentClinicId || link.clinicId === null)) ?? null;
    if (!chatLink)
        return null;
...
function telegramAppointmentTimeLabel(appointment) {
    const date = new Date(appointment.startsAt);
    if (Number.isNaN(date.getTime()))
        return "в согласованное время";
    const timeZone = validScheduleTimeZone(clinicProfile.timezone);
    return getAppointmentTimeFormatter(timeZone).format(date).replace(",", "");
}
const staffDigestVisibleAppointmentStatuses = new Set([
    "planned",
    "confirmed",
    "arrived",
    "in_treatment",
]);
const staffDigestClinicWideRoles = new Set([
    "owner",
    "manager",
    "administrator",
]);
function staffRoleLabelForTelegramDigest(role) {
    const labels = {
        owner: "владелец",
        doctor: "врач",
...
function staffCanSeeTelegramDigestAppointment(staff, appointment) {
    if (staffDigestClinicWideRoles.has(staff.role))
        return true;
    if (staff.role === "doctor")
        return appointment.doctorUserId === staff.id;
    if (staff.role === "assistant")
        return appointment.assistantUserId === staff.id;
    return false;
}
function staffCanSeeTelegramDigestTask(staff, task) {
    if (staffDigestClinicWideRoles.has(staff.role))
        return true;
    return task.assignedRole === staff.role;
}
export function buildDenteTelegramMessagePreviewData(templateKind, context, baseWarning) {
    switch (templateKind) {
...
export function renderDenteTelegramMessagePreview(input, settings = denteTelegramBotSettings) {
    ...
    const patientVisualTemplateKinds = [
    ...
    const staffVisualTemplateKinds = [
        "staff_daily_digest",
    ];
    const photoUrl = visualCardUrl &&
    ...
    const portalRequired = input.templateKind === "document_ready_notice" ||
    ...
    const baseWarning = "В Telegram не включаются диагнозы, номера зубов, план лечения, снимки, налоговые PDF, детализация оплаты и копии меддокументов.";
    const context = {
        clinicName,
        appointmentTime,
    ...
    if (input.templateKind === "staff_daily_digest") {
        const staff = input.staffId
            ? (staffMembers.find((member) => member.id === input.staffId &&
        ...
        const scopedAppointments = appointments.filter((appointment) => appointment.organizationId === settings.organizationId &&
        ...
        const scopedTasks = communicationTasks.filter((task) => task.organizationId === settings.organizationId &&
    ...
    const appointmentCallbackUnavailable = (input.templateKind === "appointment_reminder" ||
    ...
    return denteTelegramMessagePreviewSchema.parse({
        ...
        replyMarkup: preview.allowedByDefault
            ? telegramReplyMarkupFor(input.templateKind, input.appointmentId ?? null, settings)
            : null,
        photoUrl: preview.allowedByDefault ? photoUrl : null,
    });
}
function telegramTemplateKindForTask(task) {
    if (task.intent === "appointment_confirmation")
        return "appointment_confirmation";
    if (task.intent === "payment_reminder")
        return "payment_reminder_notice";
    if (task.intent === "document_ready")
        return "document_ready_notice";
    if (task.intent === "recall")
        return "recall_notice";
    if (task.intent === "post_visit_instruction")
        return "post_visit_instruction_link";
    return "callback_request_received";
}
const denteTelegramAppointmentCallbackCodes = {
    confirm: "c",
    reschedule: "r",
    call_request: "p",
};
const denteTelegramAppointmentCallbackActions = {
    c: "confirm",
    r: "reschedule",
    p: "call_request",
};
function denteTelegramCallbackSecret() {
    return (process.env.DENTE_TELEGRAM_CALLBACK_SECRET?.trim() ||
        process.env.DENTE_TELEGRAM_WEBHOOK_SECRET?.trim() ||
        null);
}
function denteTelegramAppointmentCallbacksReady() {
    return Boolean(denteTelegramCallbackSecret());
}
function normalizeDenteTelegramAppointmentCallbackScope(scope, settings = denteTelegramBotSettings) {
    const organizationId = scope?.organizationId?.trim() ||
        settings.organizationId ||
        denteTelegramBotSettings.organizationId;
    const clinicId = scope?.clinicId?.trim() || organizationId;
    const settingsBotUsername = safeTelegramBotUsername(settings.mode === "clinic_owned_bot"
...
function denteTelegramAppointmentCallbackSignature(action, appointmentId, expiresAtSecondsBase36, scope) {
...
function denteTelegramSignatureEqual(left, right) {
    const leftBuffer = Buffer.from(left);
    const rightBuffer = Buffer.from(right);
    return (leftBuffer.length === rightBuffer.length &&
        timingSafeEqual(leftBuffer, rightBuffer));
}
function appointmentCallbackExpiryBase36(appointmentId) {
...
export function buildDenteTelegramAppointmentCallbackData(action, appointmentId, scope) {
    ...
    return `d1.${actionCode}.${compactAppointmentId}.${expiresAtSecondsBase36}.${denteTelegramAppointmentCallbackSignature(action, appointmentId, expiresAtSecondsBase36, scope)}`;
}
function dashedUuidFromCompact(value) {
    const normalized = value.toLowerCase();
    if (!/^[0-9a-f]{32}$/.test(normalized))
        return null;
    return `${normalized.slice(0, 8)}-${normalized.slice(8, 12)}-${normalized.slice(12, 16)}-${normalized.slice(16, 20)}-${normalized.slice(20)}`;
}
function parseDenteTelegramAppointmentCallbackData(callbackData, scope) {
    ...
    if (!denteTelegramSignatureEqual(signature, expectedSignature))
        return null;
    return { action, appointmentId, expiresAtSeconds };
}
function appointmentCallbackActionLabel(action) {
    if (action === "confirm")
        return "подтверждение приема";
    if (action === "reschedule")
        return "запрос переноса приема";
    return "просьба перезвонить";
}
function appointmentStatusLabelForTelegram(status) {
    const labels = {
...
function appointmentCallbackStatusAllowed(action, status) {
    if (action === "confirm")
        return status === "planned";
    return status === "planned" || status === "confirmed";
}
function findExistingTelegramCallbackTask(appointment, action) {
    const title = action === "reschedule"
        ? "Пациент просит перенести прием"
        : "Пациент просит перезвонить";
    const workflowCode = action === "reschedule"
        ? "telegram_appointment_reschedule_request"
        : "telegram_appointment_call_request";
    return (communicationTasks.find((task) => task.organizationId === appointment.organizationId &&
...
function ensureTelegramCallbackCommunicationTask(input) {
    ...
    const task = {
        ...
        dueAt: input.now,
        title: input.action === "reschedule"
            ? "Пациент просит перенести прием"
            : "Пациент просит перезвонить",
        body: input.action === "reschedule"
        ...
        workflowCode: input.action === "reschedule"
            ? "telegram_appointment_reschedule_request"
            : "telegram_appointment_call_request",
...
export function handleDenteTelegramAppointmentCallback(input) {
    ...
    const chatLink = denteTelegramChatLinks.find((link) => link.organizationId === organizationId &&
        link.botConfigId === botConfigId &&
    ...
    if (!appointmentCallbackStatusAllowed(parsed.action, appointment.status)) {
        return {
            ...
            taskId: null,
            eventId: null,
            suggestedReply: `Запись сейчас в статусе '${appointmentStatusLabelForTelegram(appointment.status)}'. Кнопка не применена. Свяжитесь с клиникой для уточнения.`,
...
function telegramFeatureForTemplate(templateKind) {
    const map = {
...
function activeTelegramChatLinkFor(subjectType, subjectId, organizationScope = denteTelegramBotSettings.organizationId, botConfigId = configuredTelegramBotConfigId()) {
    return (denteTelegramChatLinks.find((link) => link.organizationId === organizationScope &&
        link.botConfigId === botConfigId &&
        link.subjectType === subjectType &&
        link.subjectId === subjectId &&
        link.status === "active") ?? null);
}
const telegramScheduleVisibleStatuses = new Set([
    "planned",
    "confirmed",
    "arrived",
    "in_treatment",
]);
function activeTelegramChatLinkByFingerprint(chatFingerprintValue, scope = {}) {
    if (!chatFingerprintValue)
        return null;
    const organizationId = scope.organizationId?.trim() || denteTelegramBotSettings.organizationId;
    const currentClinicId = scope.clinicId?.trim() || clinicProfile.organizationId;
    const botConfigId = scope.botConfigId?.trim() || configuredTelegramBotConfigId();
    return (denteTelegramChatLinks.find((link) => link.organizationId === organizationId &&
        link.botConfigId === botConfigId &&
        (link.clinicId === currentClinicId || link.clinicId === null) &&
        link.chatFingerprint === chatFingerprintValue &&
        link.status === "active") ?? null);
}
const denteTelegramDocumentRequestTopics = {
    tax: {
        workflowCode: "telegram_tax_document_request",
        taskTitle: "Пациент запросил налоговые документы",
        taskBody: "Пациент запросил налоговые документы в Telegram. В DENTE проверьте плательщика, фискальные чеки, периоды 2021-2023 и данные для КНД 1151156. Готовые PDF выдавайте только через защищенный портал.",
        ...
        responseCreatedText: "Запрос передан администратору DENTE. Клиника проверит платежи, плательщика и подготовит налоговые документы в защищенном портале.",
        responseRepeatedText: "Запрос на налоговые документы уже есть в очереди DENTE. Мы обновили время обращения и подняли задачу для администратора.",
        priority: "high",
    ...
    billing: {
        ...
        taskBody: "Пациент запросил финансовые документы в Telegram. В DENTE проверьте счет, чек, акт, возврат, рассрочку или историю оплат. Документы и суммы выдавайте только через защищенный портал.",
        ...
        responseCreatedText: "Запрос передан администратору DENTE. Клиника проверит счета, чеки, акты или возвраты и откроет документы в защищенном портале.",
        responseRepeatedText: "Запрос на финансовые документы уже есть в очереди DENTE. Мы обновили время обращения для администратора.",
        priority: "normal",
    ...
    medical: {
        ...
        taskBody: "Пациент запросил медицинские документы в Telegram. В DENTE проверьте личность, полномочия получателя и подготовьте выписку, копии, расписку выдачи или КТ/снимки без передачи медданных в Telegram.",
        ...
        responseCreatedText: "Запрос передан администратору DENTE. Медицинские документы подготовят после проверки личности и выдадут через защищенный портал.",
        responseRepeatedText: "Запрос на медицинские документы уже есть в очереди DENTE. Мы обновили время обращения для администратора.",
        priority: "normal",
    ...
    patientForms: {
        ...
        taskBody: "Пациент запросил формы пациента в Telegram. В DENTE подготовьте анкету, согласия, ПДн, представителя, отказ или фото/видео-согласие по ситуации следующего визита.",
        ...
        responseCreatedText: "Запрос передан администратору DENTE. Формы, согласия и анкеты подготовят в приложении и откроют в защищенном портале.",
        responseRepeatedText: "Запрос на формы уже есть в очереди DENTE. Мы обновили время обращения для администратора.",
        priority: "normal",
        auditCreatedAction: "telegram_patient_forms_request_created",
        auditRepeatedAction: "telegram_patient_forms_request_repeated",
    },
};
function findExistingTelegramDocumentRequestTask(organizationScope, patientId, topic) {
    const requestTopic = denteTelegramDocumentRequestTopics[topic];
    return (communicationTasks.find((task) => task.organizationId === organizationScope &&
        ...
        isOpenCommunicationTask(task)) ?? null);
}
export function createDenteTelegramDocumentRequest(chatFingerprintValue, topic, scope = {}) {
    ...
    const patient = patients.find((candidate) => candidate.organizationId === organizationScope &&
...
const denteTelegramCareRequestTopics = {
    extraction: {
        workflowCode: "telegram_care_extraction_request",
        careTopic: "extraction",
        taskTitle: "Пациент запросил памятку после удаления",
        taskBody: "Пациент нажал кнопку памятки после удаления в Telegram. Врач должен проверить карту, назначение, осложнения и выдать персональные рекомендации DENTE перед отправкой ссылки пациенту.",
        ...
        responseCreatedText: "Запрос персональной памятки передан врачу DENTE. Врач проверит карту и подготовит рекомендации в приложении.",
        responseRepeatedText: "Запрос памятки после удаления уже есть в очереди DENTE. Мы обновили время обращения и подняли задачу врачу.",
        responseIssuedText: "Персональная памятка после удаления уже выпущена в DENTE. Откройте защищенный портал или нажмите администратора, если состояние ухудшается.",
        priority: "high",
    ...
    implant: {
        ...
        taskBody: "Пациент нажал кнопку памятки после имплантации в Telegram. Врач должен проверить операцию, назначения, ограничения, контрольный визит и выдать персональные рекомендации DENTE перед отправкой ссылки пациенту.",
        ...
        responseCreatedText: "Запрос персональной памятки передан врачу DENTE. Врач проверит операцию, назначения и подготовит рекомендации в приложении.",
        responseRepeatedText: "Запрос памятки после имплантации уже есть в очереди DENTE. Мы обновили время обращения и подняли задачу врачу.",
        responseIssuedText: "Персональная памятка после имплантации уже выпущена в DENTE. Откройте защищенный портал или нажмите администратора, если нужна связь с клиникой.",
        priority: "high",
    ...
    filling: {
        ...
        taskBody: "Пациент нажал кнопку памятки после пломбы в Telegram. Врач должен проверить карту, окклюзию, анестезию, ограничения и выдать персональные рекомендации DENTE перед отправкой ссылки пациенту.",
        ...
        responseCreatedText: "Запрос персональной памятки передан врачу DENTE. Врач проверит карту и подготовит рекомендации в приложении.",
        responseRepeatedText: "Запрос памятки после пломбы уже есть в очереди DENTE. Мы обновили время обращения для врача.",
        responseIssuedText: "Персональная памятка после пломбы уже выпущена в DENTE. Откройте защищенный портал или нажмите администратора, если нужна связь с клиникой.",
        priority: "normal",
    ...
    endo: {
        ...
        taskBody: "Пациент нажал кнопку памятки после лечения каналов в Telegram. Врач должен проверить зуб, этап эндодонтии, временную или постоянную реставрацию, назначения и контрольный визит перед отправкой персональных рекомендаций DENTE.",
        ...
        responseCreatedText: "Запрос персональной памятки передан врачу DENTE. Врач проверит лечение каналов, реставрацию и подготовит рекомендации в приложении.",
        responseRepeatedText: "Запрос памятки после эндодонтии уже есть в очереди DENTE. Мы обновили время обращения и подняли задачу врачу.",
        responseIssuedText: "Персональная памятка после эндодонтии уже выпущена в DENTE. Откройте защищенный портал или нажмите администратора, если боль усиливается.",
        priority: "high",
    ...
    surgery: {
        ...
        taskBody: "Пациент нажал кнопку памятки после хирургического вмешательства в Telegram. Врач должен проверить операцию, швы, гемостаз, назначения, ограничения и план контрольного осмотра перед отправкой персональных рекомендаций DENTE.",
        ...
        responseCreatedText: "Запрос персональной памятки передан врачу DENTE. Врач проверит операцию, назначения и подготовит рекомендации в приложении.",
        responseRepeatedText: "Запрос памятки после хирургии уже есть в очереди DENTE. Мы обновили время обращения и подняли задачу врачу.",
        responseIssuedText: "Персональная памятка после хирургии уже выпущена в DENTE. Откройте защищенный портал или нажмите администратора при кровотечении, отеке или температуре.",
        priority: "high",
    ...
    anesthesia: {
        ...
        taskBody: "Пациент нажал кнопку памятки после местной анестезии в Telegram. Врач должен проверить проведенный прием, препарат, ожидаемое онемение, ограничения по еде и признаки, при которых нужна связь с клиникой.",
        ...
        responseCreatedText: "Запрос персональной памятки передан врачу DENTE. Врач проверит прием, анестезию и подготовит рекомендации в приложении.",
        responseRepeatedText: "Запрос памятки после анестезии уже есть в очереди DENTE. Мы обновили время обращения для врача.",
        responseIssuedText: "Персональная памятка после анестезии уже выпущена в DENTE. Откройте защищенный портал или нажмите администратора, если онемение или боль беспокоят.",
        priority: "normal",
    ...
    hygiene: {
        ...
        taskBody: "Пациент нажал кнопку памятки после профгигиены в Telegram. Врач или гигиенист должен проверить карту, рекомендации по уходу, ограничения и выдать персональные рекомендации DENTE перед отправкой ссылки пациенту.",
        ...
        responseCreatedText: "Запрос персональной памятки передан врачу DENTE. Клиника проверит карту и подготовит рекомендации в приложении.",
        responseRepeatedText: "Запрос памятки после гигиены уже есть в очереди DENTE. Мы обновили время обращения для врача.",
        responseIssuedText: "Персональная памятка после гигиены уже выпущена в DENTE. Откройте защищенный портал или нажмите администратора, если нужна связь с клиникой.",
        priority: "normal",
    ...
    prosthetics: {
        ...
        taskBody: "Пациент нажал кнопку памятки после протезирования в Telegram. Врач должен проверить конструкцию, адаптацию, временный цемент или постоянную фиксацию, ограничения и гарантийные условия перед отправкой персональных рекомендаций DENTE.",
        ...
        responseCreatedText: "Запрос персональной памятки передан врачу DENTE. Врач проверит конструкцию, фиксацию и подготовит рекомендации в приложении.",
        responseRepeatedText: "Запрос памятки после протезирования уже есть в очереди DENTE. Мы обновили время обращения для врача.",
        responseIssuedText: "Персональная памятка после протезирования уже выпущена в DENTE. Откройте защищенный портал или нажмите администратора, если конструкция мешает.",
        priority: "normal",
    ...
    orthodontics: {
        ...
        taskBody: "Пациент нажал кнопку памятки после ортодонтического приема в Telegram. Врач должен проверить аппарат, элайнеры или брекеты, режим ношения, уход, ограничения и дату контроля перед отправкой персональных рекомендаций DENTE.",
        ...
        responseCreatedText: "Запрос персональной памятки передан врачу DENTE. Врач проверит аппарат, режим ношения и подготовит рекомендации в приложении.",
        responseRepeatedText: "Запрос памятки после ортодонтии уже есть в очереди DENTE. Мы обновили время обращения для врача.",
        responseIssuedText: "Персональная памятка после ортодонтии уже выпущена в DENTE. Откройте защищенный портал или нажмите администратора, если аппарат натирает или отклеился.",
        priority: "normal",
    ...
    periodontology: {
        ...
        taskBody: "Пациент нажал кнопку памятки после пародонтологического приема в Telegram. Врач должен проверить десны, кровоточивость, назначенный уход, ограничения и сроки контроля перед отправкой персональных рекомендаций DENTE.",
        ...
        responseCreatedText: "Запрос персональной памятки передан врачу DENTE. Врач проверит десны, уход и подготовит рекомендации в приложении.",
        responseRepeatedText: "Запрос памятки после пародонтологии уже есть в очереди DENTE. Мы обновили время обращения для врача.",
        responseIssuedText: "Персональная памятка после пародонтологии уже выпущена в DENTE. Откройте защищенный портал или нажмите администратора, если кровоточивость или боль усиливаются.",
        priority: "normal",
...
function findIssuedTelegramCareDocument(organizationScope, patientId, topic) {
    const requestTopic = denteTelegramCareRequestTopics[topic];
    return (documents.find((document) => document.organizationId === organizationScope &&
        ...
        document.payload.postVisitRecommendations.careTopic ===
            requestTopic.careTopic) ?? null);
}
function findExistingTelegramCareRequestTask(organizationScope, patientId, topic) {
    const requestTopic = denteTelegramCareRequestTopics[topic];
    return (communicationTasks.find((task) => task.organizationId === organizationScope &&
        task.patientId === patientId &&
        task.appointmentId === null &&
...
export function createDenteTelegramCareRequest(chatFingerprintValue, topic, scope = {}) {
    const chatLink = activeTelegramChatLinkByFingerprint(chatFingerprintValue, scope);
    if (!chatLink) {
        return {
...
function telegramScheduleLine(index, appointment, roleLabel = null) {
...
function telegramScheduleRoleForStaff(appointment, staffId) {
...
function visibleTelegramScheduleAppointments(organizationScope = denteTelegramBotSettings.organizationId) {
    const nowMs = Date.now();
    const graceMs = 15 * 60 * 1000;
    return appointments
        .filter((appointment) => {
        const endsAtMs = Date.parse(appointment.endsAt);
        return (appointment.organizationId === organizationScope &&
...
export function buildDenteTelegramLinkedScheduleReply(chatFingerprintValue, scope = {}, settings = denteTelegramBotSettings) {
    const chatLink = activeTelegramChatLinkByFingerprint(chatFingerprintValue, scope);
    if (!chatLink) {
        return {
    ...
    const linkedAppointments = chatLink.subjectType === "patient"
        ? visibleAppointments
    ...
    if (!linkedAppointments.length) {
        return {
            ...
            appointmentCount: 0,
            blockedReason: null,
            replyMarkup: chatLink.subjectType === "staff"
    ...
    if (chatLink.subjectType === "patient") {
        ...
        return {
            ...
            replyMarkup: nearestAppointment
                ? telegramScheduleReplyMarkupForPatientAppointment(nearestAppointment.id, scope)
                : null,
        };
    }
    return {
        text: [
            ...
            ...linkedAppointments.map((appointment, index) => telegramScheduleLine(index, appointment, telegramScheduleRoleForStaff(appointment, chatLink.subjectId))),
            "",
...
function findExistingTelegramContactRequestTask(organizationScope, patientId) {
    return (communicationTasks.find((task) => task.organizationId === organizationScope &&
        task.patientId === patientId &&
        task.appointmentId === null &&
        task.documentId === null &&
...
export function createDenteTelegramContactRequest(chatFingerprintValue, scope = {}) {
    const chatLink = activeTelegramChatLinkByFingerprint(chatFingerprintValue, scope);
    if (!chatLink) {
        return {
...
function telegramOutboxSafeTitle(templateKind, subjectType) {
    if (subjectType === "staff")
        return "Ежедневная сводка DENTE";
    const titles = {
...
function issuedPostVisitRecommendationExists(input, organizationScope = denteTelegramBotSettings.organizationId) {
    if (!input.visitId && !input.documentId)
        return false;
    return documents.some((document) => {
        ...
        if (input.documentId && document.id !== input.documentId)
            return false;
        if (input.visitId && document.visitId !== input.visitId)
            return false;
        return true;
    });
}
function buildDenteTelegramOutboxItem(input, runtimeScope) {
    ...
    const preview = input.task && input.subjectType === "patient"
    ...
    const chatTransportReady = Boolean(chatLink?.chatTransportRef &&
    ...
    else if (input.templateKind === "post_visit_instruction_link" &&
...
function paymentReminderOutboxId(patientId, balanceDueRub) {
    return `payment-reminder:${patientId}:${Math.max(0, Math.round(balanceDueRub))}`;
}
function paymentReminderAlreadyCovered(outboxItemId) {
    return telegramOutboxItemAlreadySent(outboxItemId);
}
function patientPaymentBalanceRub(patientId, organizationScope = denteTelegramBotSettings.organizationId) {
    const plannedRub = treatmentPlanItems.reduce((total, item) => {
        if (item.organizationId === organizationScope &&
    ...
    const paidRub = payments.reduce((total, payment) => {
        if (payment.organizationId === organizationScope &&
...
function patientPaymentReminderScheduledAt(patientId, organizationScope = denteTelegramBotSettings.organizationId) {
    const latestPaidAtMs = payments
        .filter((payment) => payment.organizationId === organizationScope &&
        payment.patientId === patientId &&
        payment.status === "paid")
...
function buildDenteTelegramPaymentReminderItems(runtimeScope) {
    const runtime = resolveDenteTelegramOutboxRuntimeScope(runtimeScope);
    const organizationScope = runtime.settings.organizationId;
    return patients.flatMap((patient) => {
        if (patient.organizationId !== organizationScope)
            return [];
        ...
        if (paymentReminderAlreadyCovered(itemId))
            return [];
        return [
...
function recallOutboxId(item) {
    return `recall:${item.visitId ?? item.id}:${item.patientId}`;
}
function recallAlreadyCovered(outboxItemId) {
    return telegramOutboxItemAlreadySent(outboxItemId);
}
function treatmentPlanItemAppointment(item) {
    if (!item.visitId)
        return null;
    const visit = findVisitById(item.visitId);
    return visit
        ? (appointments.find((appointment) => appointment.id === visit.appointmentId) ?? null)
        : null;
}
function recallScheduledAt(item) {
...
function buildDenteTelegramRecallItems(runtimeScope) {
    const runtime = resolveDenteTelegramOutboxRuntimeScope(runtimeScope);
    const organizationScope = runtime.settings.organizationId;
    ...
    return treatmentPlanItems.flatMap((item) => {
        ...
        if (recallAlreadyCovered(itemId))
            return [];
        return [
            buildDenteTelegramOutboxItem({
                id: itemId,
...
export function findDenteTelegramOutboxDeliveryReceipt(outboxItemId, clientMutationId) {
    if (!clientMutationId)
        return null;
    return (denteTelegramOutboxDeliveryReceiptsMap.get(`${outboxItemId}:${clientMutationId}`) ?? null);
}
export function claimDenteTelegramOutboxDeliveryReceipt(item, clientMutationId, warnings) {
...
function reviewRequestOutboxIdForVisit(visitId, patientId) {
    return `review:${visitId}:${patientId}`;
}
function reviewRequestScheduledAtFromBase(baseAt, fallbackAt = nowIso, settings = denteTelegramBotSettings) {
...
function reviewRequestScheduledAt(payment, settings = denteTelegramBotSettings) {
    return reviewRequestScheduledAtFromBase(payment.paidAt ?? payment.createdAt, payment.createdAt, settings);
}
function reviewRequestScheduledAtForVisit(visit, settings = denteTelegramBotSettings) {
...
function reviewRequestAlreadySent(outboxItemId) {
    return telegramOutboxItemAlreadySent(outboxItemId);
}
function telegramOutboxItemAlreadySent(outboxItemId) {
    return auditEvents.some((event) => event.entityType === "telegram_outbox" &&
        event.entityId === outboxItemId &&
        event.action === "telegram_outbound_sent");
}
function postVisitInstructionOutboxId(visitId, patientId) {
    return `post-visit:${visitId}:${patientId}`;
}
function postVisitInstructionScheduledAt(visitId) {
    const appointment = activeVisit.id === visitId
...
function postVisitInstructionTaskKeepsOutboxClaim(task) {
    if (["sent", "delivered", "completed"].includes(task.status))
        return true;
    return task.channel === "telegram" && isOpenCommunicationTask(task);
}
function postVisitInstructionAlreadyCovered(visitId, patientId, outboxItemId) {
    const hasTask = communicationTasks.some((task) => task.patientId === patientId &&
...
function appointmentReminderOutboxId(appointment, leadTimeHours) {
    return `appointment-reminder:${appointment.id}:${leadTimeHours}h:${appointment.patientId ?? "unknown"}`;
}
function appointmentReminderScheduledAt(appointment, leadTimeHours) {
    const startsAtMs = Date.parse(appointment.startsAt);
    if (!Number.isFinite(startsAtMs))
        return nowIso;
    return new Date(startsAtMs - leadTimeHours * 60 * 60 * 1000).toISOString();
}
function appointmentReminderInsideDispatchWindow(appointment, leadTimeHours, nowMs) {
    const startsAtMs = Date.parse(appointment.startsAt);
...
function appointmentReminderAlreadySent(outboxItemId) {
    return auditEvents.some((event) => event.entityType === "telegram_outbox" &&
        event.entityId === outboxItemId &&
        event.action === "telegram_outbound_sent");
}
function taxDocumentRequestOutboxId(document) {
    return `tax-request:${document.id}:${document.patientId}`;
}
function taxDocumentRequestAlreadySent(outboxItemId) {
    return telegramOutboxItemAlreadySent(outboxItemId);
}
function taxApplicationScheduledAt(document) {
    const requestedAt = document.payload?.taxDeductionApplication?.requestedAt ??
        document.issuedAt ??
        nowIso;
...
function taxApplicationSlaWarning(document) {
...
function buildDenteTelegramTaxDocumentRequestItems(runtimeScope) {
    const runtime = resolveDenteTelegramOutboxRuntimeScope(runtimeScope);
    const organizationScope = runtime.settings.organizationId;
    const activePatientIds = new Set(patients.filter((p) => p.status === "active").map((p) => p.id));
...
const documentReadyNoticeExcludedKinds = new Set([
    "tax_deduction_application",
    "post_visit_recommendations",
]);
function documentReadyOutboxId(document) {
    return `document-ready:${document.id}:${document.patientId}`;
}
function documentReadyScheduledAt(document) {
    const issuedAtMs = Date.parse(document.issuedAt ?? "");
    if (!Number.isFinite(issuedAtMs))
        return nowIso;
    return new Date(issuedAtMs + 5 * 60 * 1000).toISOString();
}
function documentReadyTaskKeepsOutboxClaim(task) {
    if (["sent", "delivered", "completed"].includes(task.status))
        return true;
    return task.channel === "telegram" && isOpenCommunicationTask(task);
}
function documentReadyAlreadyCovered(document, outboxItemId) {
    const hasTask = communicationTasks.some((task) => task.patientId === document.patientId &&
...
function buildDenteTelegramDocumentReadyItems(runtimeScope) {
    const runtime = resolveDenteTelegramOutboxRuntimeScope(runtimeScope);
    const organizationScope = runtime.settings.organizationId;
    ...
    return documents.flatMap((document) => {
        ...
        if (document.status !== "issued")
            return [];
        if (documentReadyNoticeExcludedKinds.has(document.kind))
            return [];
        const patient = activePatients.get(document.patientId);
        if (!patient)
            return [];
        const itemId = documentReadyOutboxId(document);
        if (documentReadyAlreadyCovered(document, itemId))
            return [];
        return [
            buildDenteTelegramOutboxItem({
                id: itemId,
...
function staffDailyDigestOutboxId(staffId, now = new Date()) {
    return `staff-digest:${appointmentClinicDateKey(now.toISOString())}:${staffId}`;
}
function staffDailyDigestAlreadySent(outboxItemId) {
    return auditEvents.some((event) => event.entityType === "telegram_outbox" &&
        event.entityId === outboxItemId &&
        event.action === "telegram_outbound_sent");
}
function buildDenteTelegramAppointmentReminderItems(runtimeScope) {
    const runtime = resolveDenteTelegramOutboxRuntimeScope(runtimeScope);
    const { settings } = runtime;
    const nowMs = Date.now();
    const activePatientsMap = new Map(patients.filter((p) => p.status === "active").map((p) => [p.id, p]));
    return appointments.flatMap((appointment) => {
        if (appointment.organizationId !== settings.organizationId)
            return [];
        if (!appointment.patientId)
            return [];
        const patientId = appointment.patientId;
        if (!["planned", "confirmed"].includes(appointment.status))
            return [];
        const startsAtMs = Date.parse(appointment.startsAt);
        if (!Number.isFinite(startsAtMs) || startsAtMs <= nowMs)
            return [];
        ...
        return normalizeAppointmentReminderLeadTimes(settings.appointmentReminderLeadTimesHours).flatMap((leadTimeHours) => {
            ...
            if (appointmentReminderAlreadySent(itemId))
                return [];
            if (!appointmentReminderInsideDispatchWindow(appointment, leadTimeHours, nowMs))
                return [];
            return [
                buildDenteTelegramOutboxItem({
                    id: itemId,
...
function buildDenteTelegramPostVisitInstructionItems(runtimeScope) {
    const runtime = resolveDenteTelegramOutboxRuntimeScope(runtimeScope);
    const organizationScope = runtime.settings.organizationId;
    const visitPatientPairs = new Map();
    const activePatientsMap = new Map(patients.filter((p) => p.status === "active").map((p) => [p.id, p]));
    for (const item of treatmentPlanItems) {
        if (!item.visitId || item.organizationId !== organizationScope)
            continue;
...
function postVisitCheckupOutboxId(document) {
    return `post-visit-checkup:${document.visitId ?? document.id}:${document.patientId}`;
}
function postVisitCheckupDelayHours(careTopic, settings = denteTelegramBotSettings) {
...
function postVisitCheckupScheduledAt(document, settings = denteTelegramBotSettings) {
    const issuedAtMs = Date.parse(document.issuedAt ?? "");
...
function postVisitCheckupAlreadyCovered(outboxItemId) {
    return telegramOutboxItemAlreadySent(outboxItemId);
}
function buildDenteTelegramPostVisitCheckupItems(runtimeScope) {
    const runtime = resolveDenteTelegramOutboxRuntimeScope(runtimeScope);
    const organizationScope = runtime.settings.organizationId;
    return documents.flatMap((document) => {
        if (document.organizationId !== organizationScope)
        ...
        if (postVisitCheckupAlreadyCovered(itemId))
            return [];
        return [
            buildDenteTelegramOutboxItem({
                id: itemId,
...
function reviewRequestVisitIsClosedByVisit(visit) {
    if (visit.status === "signed")
        return true;
    const appointment = appointments.find((item) => item.id === visit.appointmentId) ?? null;
    return appointment?.status === "completed";
}
function reviewRequestClosedVisitCandidates(organizationScope = denteTelegramBotSettings.organizationId) {
    return [activeVisit].filter((visit) => {
        if (visit.organizationId !== organizationScope)
            return false;
...
function buildDenteTelegramReviewRequestItems(runtimeScope) {
    const runtime = resolveDenteTelegramOutboxRuntimeScope(runtimeScope);
    const organizationScope = runtime.settings.organizationId;
    ...
    const pushReviewRequest = (input) => {
        ...
        if (reviewRequestAlreadySent(itemId))
            return;
        seenSubjects.add(subjectKey);
        items.push(buildDenteTelegramOutboxItem({
            id: itemId,
            task: null,
            subjectType: "patient",
    ...
    const closedVisits = reviewRequestClosedVisitCandidates(organizationScope).sort((left, right) => (right.updatedAt ?? right.createdAt).localeCompare(left.updatedAt ?? left.createdAt));
    ...
    const activePatientsById = new Map(patients.filter((p) => p.status === "active").map((p) => [p.id, p]));
    for (const payment of paidMilestones) {
        const patient = activePatientsById.get(payment.patientId) ?? null;
        if (!patient || !reviewRequestVisitIsClosed(payment))
            continue;
...
function reviewRequestVisitIsClosed(payment) {
    if (!payment.visitId)
        return false;
...
function denteTelegramMainMenuRow() {
    return [{ text: "Главное меню", callback_data: "dente:start" }];
}
function telegramReplyMarkupForReviewRequest(settings) {
    const reviewUrl = safeHttpsUrl(settings.clinicReviewUrl);
    const mapsUrl = safeHttpsUrl(settings.clinicMapsUrl);
    const buttons = [];
    if (reviewUrl)
        buttons.push({ text: "Оценить клинику", url: reviewUrl });
    if (mapsUrl)
        buttons.push({ text: "Открыть карту", url: mapsUrl });
    return buttons.length
        ? { inline_keyboard: [buttons, denteTelegramMainMenuRow()] }
        : null;
}
function telegramReplyMarkupForAppointment(appointmentId, signedAppointmentCallbackScope) {
    if (appointmentId) {
        if (!denteTelegramAppointmentCallbacksReady()) {
            return {
                inline_keyboard: [
        ...
        return {
            inline_keyboard: [
                [
                    {
                        text: "Подтвердить",
                        callback_data: buildDenteTelegramAppointmentCallbackData("confirm", appointmentId, signedAppointmentCallbackScope),
                    },
                    {
                        text: "Перенести",
                        callback_data: buildDenteTelegramAppointmentCallbackData("reschedule", appointmentId, signedAppointmentCallbackScope),
                    },
                ],
                [
                    {
                        text: "Позвоните мне",
                        callback_data: buildDenteTelegramAppointmentCallbackData("call_request", appointmentId, signedAppointmentCallbackScope),
                    },
                    { text: "Конфиденциальность", callback_data: "dente:privacy" },
                ],
                denteTelegramMainMenuRow(),
...
function telegramReplyMarkupForDocumentReadyNotice(portalRow) {
    const rows = [
        portalRow,
        [
...
function telegramReplyMarkupForTaxDocumentRequestStatus(portalRow) {
    const rows = [
        portalRow,
        [
...
function telegramReplyMarkupForPaymentReminderNotice(portalRow) {
    const rows = [
        portalRow,
        [
...
function telegramReplyMarkupForPostVisit(portalRow) {
    const rows = [
        portalRow,
        [
...
function telegramReplyMarkupForRecallNotice(portalRow) {
    const rows = [
        portalRow,
        [
...
function telegramReplyMarkupForStaffDailyDigest(portalRow) {
    const rows = [
        portalRow,
        [
            { text: "Расписание", callback_data: "dente:schedule" },
...
function telegramReplyMarkupFor(templateKind, appointmentId = null, settings = denteTelegramBotSettings, appointmentCallbackScope = {}) {
    const portalRow = denteTelegramPortalRowForTemplate(templateKind, settings);
    const signedAppointmentCallbackScope = normalizeDenteTelegramAppointmentCallbackScope(appointmentCallbackScope, settings);
    switch (templateKind) {
        case "review_request":
...
function telegramScheduleReplyMarkupForPatientAppointment(appointmentId, scope = {}) {
...
export function prepareDenteTelegramOutboxDelivery(outboxItemId, runtimeScope) {
...
export function recordDenteTelegramOutboxDelivery(input) {
    ...
    const task = input.item.taskId
        ? (communicationTasks.find((candidate) => candidate.id === input.item.taskId) ?? null)
        : null;
    ...
    recordAuditEvent({
        ...
        action: input.status === "sent"
            ? "telegram_outbound_sent"
            : "telegram_outbound_failed",
        reason: input.status === "sent"
            ? `Telegram safe template ${input.item.templateKind} sent; message id ${input.telegramMessageId ?? "unknown"}.${clientMutationId ? ` clientMutationId=${clientMutationId}.` : ""}`
...
function normalizeDenteTelegramOutboxOptions(input = 100) {
    const source = typeof input === "number" ? { limit: input } : input;
...
function denteTelegramOutboxItemMatchesStatus(item, status, nowMs) {
    if (status === "all")
        return true;
    if (status === "due") {
        if (item.deliveryStatus !== "ready")
...
function buildAllDenteTelegramOutboxItems(now, runtimeScope) {
    const runtime = resolveDenteTelegramOutboxRuntimeScope(runtimeScope);
    const organizationScope = runtime.settings.organizationId;
    const taskItems = communicationTasks
        ...
        .map((task) => buildDenteTelegramOutboxItem({
        id: `task:${task.id}`,
        task,
        subjectType: "patient",
...
function findDenteTelegramOutboxItem(outboxItemId, runtimeScope) {
    return (buildAllDenteTelegramOutboxItems(new Date().toISOString(), runtimeScope).find((item) => item.id === outboxItemId) ?? null);
}
export function buildDenteTelegramOutbox(input = 100, runtimeScope) {
    ...
    if (!runtime.botTokenConfigured)
        warnings.push("Подключите бота Telegram в серверных настройках для реальной отправки сообщений.");
...
export function listDenteTelegramWebhookEvents(limit = 20, organizationScope = denteTelegramBotSettings.organizationId, botConfigId) {
    return denteTelegramWebhookEvents
        .filter((event) => event.organizationId === organizationScope &&
        (!botConfigId || event.botConfigId === botConfigId))
        .slice(0, Math.max(0, Math.min(200, limit)));
}
function findDenteTelegramWebhookUpdate(updateId, organizationScope = denteTelegramBotSettings.organizationId, botConfigId) {
    return (denteTelegramWebhookEvents.find((event) => event.organizationId === organizationScope &&
        event.updateId === updateId &&
        (!botConfigId || event.botConfigId === botConfigId)) ?? null);
}
export function hasDenteTelegramWebhookUpdate(updateId, organizationScope = denteTelegramBotSettings.organizationId, botConfigId) {
...
export function claimDenteTelegramWebhookUpdate(input) {
    ...
    if (existing) {
        ...
        const retryEvent = {
            ...
            warnings: [
                ...existing.warnings.filter((warning) => warning !== "stale_processing_retry"),
...
export function recordDenteTelegramWebhookEvent(input) {
    const organizationScope = input.organizationId ?? denteTelegramBotSettings.organizationId;
    const existingIndex = denteTelegramWebhookEvents.findIndex((event) => event.organizationId === organizationScope &&
...
function buildDocumentChainSummary(document) {
...
function normalizePatientAdministrativeProfile(input) {
    const preferredAppointmentStart = isClockTime(input?.preferredAppointmentStart)
        ? input.preferredAppointmentStart
        : null;
    const requestedPreferredAppointmentEnd = isClockTime(input?.preferredAppointmentEnd)
        ? input.preferredAppointmentEnd
        : null;
    const preferredAppointmentEnd = preferredAppointmentStart &&
...
function normalizePatientAdministrativeProfiles() {
...
export function createPatient(input) {
    ...
    if (!activeVisitPatientExists) {
        ...
        if (activeAppointment)
            activeAppointment.patientId = patient.id;
    }
    recordAuditEvent({
        entityType: "patient",
        entityId: patient.id,
...
export function updatePatient(patientId, input) {
    ...
    if (input.birthDate !== undefined)
        patient.birthDate = normalizeDateOnlyInput(input.birthDate, "Дата рождения пациента");
    if (input.phone !== undefined)
        patient.phone = nullableTrimmed(input.phone);
    if (input.email !== undefined)
        patient.email = nullableTrimmed(input.email);
    if (input.notes !== undefined)
        patient.notes = nullableTrimmed(input.notes);
...
export function updatePatientAdministrativeProfile(patientId, input) {
    const patient = patients.find((candidate) => candidate.id === patientId);
    if (!patient) {
        throw new Error("Пациент не найден");
    }
...
function assertAppointmentReferenceExists(input) {
    ...
    if (input.doctorUserId !== undefined && input.doctorUserId !== null) {
        const doctor = staffMembers.find((member) => member.id === input.doctorUserId &&
            member.active &&
            (member.role === "doctor" || member.role === "owner"));
        if (!doctor)
            throw new Error("Врач для записи не найден или не активен");
    }
    if (input.assistantUserId !== undefined && input.assistantUserId !== null) {
        const assistant = staffMembers.find((member) => member.id === input.assistantUserId &&
            member.active &&
    ...
    if (input.chairId !== undefined && input.chairId !== null) {
        ...
        if (!chair)
            throw new Error("Кресло для записи не найдено или не активно");
    }
}
function mergedAppointmentTimes(appointment, input) {
...
const scheduleBlockingAppointmentStatuses = new Set([
    "planned",
    "confirmed",
    "arrived",
    "in_treatment",
...
function assertActiveVisitAppointmentStatusChangeIsSafe(appointment, input) {
    if (activeVisit.appointmentId !== appointment.id ||
        activeVisit.status !== "draft")
        return;
    if (input.status === undefined || input.status === appointment.status)
        return;
    if (!terminalAppointmentStatuses.has(input.status))
        return;
    throw new Error("Нельзя закрыть, отменить или отметить неявку записи, пока связанный прием открыт как черновик");
}
function appointmentRequiresHardScheduleValidation(appointment) {
    const endsAtMs = Date.parse(appointment.endsAt);
    return (scheduleBlockingAppointmentStatuses.has(appointment.status) &&
        Number.isFinite(endsAtMs) &&
        endsAtMs >= Date.now());
}
function appointmentIntervalsOverlap(left, right) {
...
function assertNoAppointmentResourceOverlap(candidate) {
    if (!appointmentRequiresHardScheduleValidation(candidate))
        return;
    const overlapping = appointments.find((appointment) => appointment.id !== candidate.id &&
...
function assertAppointmentWithinOperationalHours(candidate) {
    if (!appointmentRequiresHardScheduleValidation(candidate))
        return;
    ...
    const patient = candidate.patientId
        ? patients.find((item) => item.id === candidate.patientId && item.status === "active")
        : null;
    if (!patient) {
    ...
    const doctor = candidate.doctorUserId
        ? staffMembers.find((member) => member.id === candidate.doctorUserId && member.active)
        : null;
    if (doctor) {
    ...
    const assistant = candidate.assistantUserId
        ? staffMembers.find((member) => member.id === candidate.assistantUserId &&
            member.active &&
            member.role === "assistant")
        : null;
    if (assistant) {
...
function assertAppointmentCanBeScheduled(candidate) {
    assertAppointmentWithinOperationalHours(candidate);
    assertNoAppointmentResourceOverlap(candidate);
}
function assertStaffWorkingHoursCoverExistingAppointments(member, workingHours) {
    const candidateStaff = { ...member, workingHours };
    const blockingAppointment = appointments.find((appointment) => scheduleBlockingAppointmentStatuses.has(appointment.status) &&
        appointmentRequiresHardScheduleValidation(appointment) &&
...
function assertChairWorkingHoursCoverExistingAppointments(chair, workingHours) {
    const candidateChair = { ...chair, workingHours };
    const blockingAppointment = appointments.find((appointment) => scheduleBlockingAppointmentStatuses.has(appointment.status) &&
        appointmentRequiresHardScheduleValidation(appointment) &&
...
function assertClinicScheduleDefaultsCoverExistingAppointments(scheduleDefaults, timezone) {
    const blockingAppointment = appointments.find((appointment) => appointment.organizationId === organizationId &&
        appointmentRequiresHardScheduleValidation(appointment) &&
...
export function updateAppointment(appointmentId, input) {
    ...
    if (input.chairId !== undefined)
        appointment.chairId = input.chairId;
...
function todayLocalIsoDateOnly() {
    const now = new Date();
    const local = new Date(now.getTime() - now.getTimezoneOffset() * 60_000);
    return local.toISOString().slice(0, 10);
}
function normalizeDateOnlyInput(value, fieldLabel) {
    const trimmed = value?.trim();
    if (!trimmed)
        return null;
...
export function updateClinicMode(mode) {
    const currentModePreset = clinicProfile.mode === "solo_doctor"
        ? 60
    ...
    if (shouldApplyModePreset)
        clinicProfile.defaultVisitMinutes = nextModePreset;
...
export function updateClinicProfile(input) {
    const nextTimezone = input.timezone !== undefined
        ? input.timezone.trim()
        : clinicProfile.timezone;
    if (input.timezone !== undefined)
        assertValidScheduleTimeZone(nextTimezone);
    if (input.scheduleDefaults !== undefined || input.timezone !== undefined) {
        assertClinicScheduleDefaultsCoverExistingAppointments(normalizeClinicScheduleDefaults(input.scheduleDefaults ?? clinicProfile.scheduleDefaults), nextTimezone);
    }
    ...
    if (input.inn !== undefined)
        clinicProfile.inn = nullableTrimmed(input.inn);
    if (input.kpp !== undefined)
        clinicProfile.kpp = nullableTrimmed(input.kpp);
    ...
    if (input.timezone !== undefined)
        clinicProfile.timezone = nextTimezone;
    ...
    if (input.scheduleDefaults !== undefined)
        clinicProfile.scheduleDefaults = normalizeClinicScheduleDefaults(input.scheduleDefaults);
...
export function updateStaffWorkingHours(staffId, input) {
...
export function createChair(input) {
    const chair = {
        id: randomUUID(),
        organizationId,
...
export function updateChairWorkingHours(chairId, input) {
...
export function listAiRecognitionJobs() {
    return aiRecognitionJobs.slice(0, 20);
}
export function createAiRecognitionJob(input) {
...
function speechRecordingRecoveryFromChunks(recordingId, chunks) {
    ...
    for (const chunk of sortedChunks) {
        if (chunk.status === "transcribed")
            statusCounts.transcribed++;
        else if (chunk.status === "fallback_text")
            statusCounts.fallback_text++;
        else if (chunk.status === "needs_provider_key")
            statusCounts.needs_provider_key++;
...
export function recordSpeechTranscriptionChunk(input) {
...
function assertActiveVisitDraftMutationAllowed() {
    if (activeVisit.status !== "draft") {
        throw new Error("Прием уже закрыт или аннулирован");
    }
}
export function getVisitDraftAutosave(visitId) {
    if (visitId !== activeVisit.id)
        return null;
    if (activeVisit.status !== "draft")
        return null;
    return visitDraftAutosaves.find((draft) => draft.visitId === visitId) ?? null;
}
export function upsertVisitDraftAutosave(input) {
...
export function acceptVisitDraft(input) {
    ...
    const duplicateReceipt = clientMutationId
        ? visitSaveReceipts.find((receipt) => receipt.visitId === input.visitId &&
...
const documentTitles = Object.fromEntries(Object.entries(documentKindMetadata).map(([kind, metadata]) => [
    kind,
    metadata.title,
]));
function documentSnapshotDirectoryPath() {
    return (process.env.DENTAL_DOCUMENT_SNAPSHOT_DIR ??
...
function moveSnapshotTempFile(tempPath, snapshotPath) {
    let lastError = null;
...
export function storeIssuedDocumentSnapshot(documentId, html) {
...
export function issueGeneratedDocument(documentId, options = {}) {
    const document = documents.find((candidate) => candidate.id === documentId);
    ...
    document.releaseJournalEntry = options.releaseJournalEntry
        ? {
            ...
            sourceSnapshotSha256: options.releaseJournalEntry.sourceSnapshotSha256 ??
                snapshot?.sha256 ??
...
export function storeTaxXmlSnapshot(documentId, input) {
    const document = documents.find((candidate) => candidate.id === documentId);
    if (!document || document.status !== "issued")
        return null;
...
export function voidGeneratedDocument(documentId, options = {}) {
    const document = documents.find((candidate) => candidate.id === documentId);
    if (!document) {
        return null;
    }
...
function normalizeFiscalReceiptDetails(input) {
    if (!input)
        return null;
...
function assertPaidPaymentFiscalReceiptOperation(input) {
...
export function findPaymentByClientMutationId(clientMutationId) {
...
export function createPayment(input) {
    ...
    const payment = {
        ...
        fiscalReceiptUrl: input.fiscalReceiptUrl?.trim() ||
...
const communicationTaskOutcomeLabels = {
...
export function completeCommunicationTask(input) {
...
function viewerModeForImagingKind(kind) {
    if (kind === "cbct")
        return "mpr";
    if (kind === "photo")
        return "photo";
...
function normalizeViewerAnnotations(annotations) {
    const now = new Date().toISOString();
...
export function getOrCreateImagingViewerSession(study) {
...
export function saveImagingViewerSession(studyId, input) {
...
function dicomWorkbenchSeriesKeyFromManifest(manifest) {
...
function dicomWorkbenchWarnings(manifest) {
...
const dicomRenderTextureStrategyAuditLabels = {
...
export function saveDicomWorkbenchBundle(input) {
    const now = new Date().toISOString();
    ...
    if (existingIndex >= 0)
        ...
        dicomWorkbenchBundles.unshift(bundle);
...
export function listDicomWorkbenchBundles(limit = 8) {
...
export function createImagingStudy(input) {
    ...
    const study = {
        ...
        status: "needs_review",
        aiSummary: nullableTrimmed(input.aiSummary) ??
