...
window.getBaseUrl = (url) => {
	if (!url) return "";
	return url.replace("_enhanced", "");
};

window.getEnhancedUrl = (url) => {
	if (!url) return "";
	if (url.includes("_enhanced")) return url;
...
document.addEventListener("DOMContentLoaded", () => {
			...
			try {
				...
				if (data.url) {
					...
					if (showSlider) {
						...
						const originalUrl = window.getBaseUrl(cleanUrl);
						const enhancedUrl = window.getEnhancedUrl(cleanUrl);
	...
	window.loadData = (imageUrl, reportText, summaryContent) => {
		...
		const baseImageUrl = window.getBaseUrl(cleanUrl);
	...
	const renderIncomingTray = (scans) => {
		...
		scans.forEach((scan) => {
			...
			const thumbUrl = window.getBaseUrl(scan.image);
		...
		try {
			...
			if (
				...
				(cleanCurrentImage === cleanLatestImage ||
					window.getBaseUrl(cleanCurrentImage) ===
						window.getBaseUrl(cleanLatestImage))
			) {
				if (
			...
			if (data.recent_scans && incomingTray) {
				...
				const activeScan = data.recent_scans.find((s) => {
					...
					return (
						...
						window.getBaseUrl(sClean) === window.getBaseUrl(cleanCurrentImage)
					);
				});
				if (activeScan) {
					if (
			...
			if (data.is_processing) {
				...
				const isCurrentProcessing =
					...
					(cleanCurrentImage === cleanProcessingImage ||
						window.getBaseUrl(cleanCurrentImage) ===
							window.getBaseUrl(cleanProcessingImage));
	...
	if (btnSaveHistory) {
		btnSaveHistory.addEventListener("click", async () => {
			...
			const payload = {
				...
				enhanced_image: window.getEnhancedUrl(currentImage),
				brightness: bVal,
				contrast: cVal,
				inverted: invVal,
				scale: scale,
