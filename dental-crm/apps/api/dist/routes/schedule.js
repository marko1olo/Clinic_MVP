...
export async function registerScheduleRoutes(app) {
    ...
    app.get("/api/schedule/clipboard-items", async (request, reply) => {
        ...
        return items.map((item) => ({
            ...item,
            copiedAt: item.copiedAt instanceof Date ? item.copiedAt.toISOString() : String(item.copiedAt)
        }));
    });
    app.post("/api/schedule/clipboard-items", async (request, reply) => {
        ...
        const endsMs = source.endsAt instanceof Date ? source.endsAt.getTime() : Date.parse(String(source.endsAt));
    ...
    app.post("/api/schedule/clipboard-items/:id/paste", async (request, reply) => {
        ...
        const durationMinutes = typeof clipItem.durationMinutes === "number" && clipItem.durationMinutes > 0
