...
const paymentValidationMessage = "Ни одно поле оплаты не прошло проверку. Проверьте сумму, дату, способ оплаты, фискальный чек и данные плательщика и повторите запись оплаты.";
 ...
 */
function paymentValidationDetail(issues) {
...
function isDuplicateClientMutationError(error) {
    ...
    if (candidate.cause)
        return isDuplicateClientMutationError(candidate.cause);
    return false;
}
...
function paymentRetrySignatureFromInput(input) {
    ...
    return {
        ...
        method: input.method,
        fiscalReceiptNumber: cleanPaymentText(input.fiscalReceiptNumber) ??
            fiscalReceiptLabel(fiscalReceipt),
        fiscalReceiptIssuedAt: cleanPaymentText(input.fiscalReceiptIssuedAt),
        fiscalReceiptUrl: cleanPaymentText(input.fiscalReceiptUrl) ??
...
function paymentRetryMatchesExisting(existingPayment, input) {
    return (JSON.stringify(paymentRetrySignatureFromPayment(existingPayment)) ===
 ...
 */
async function requirePayoutAccess(request, reply) {
    // Секрет периметра остаётся первым барьером, как на всех защищённых чтениях.
    if (!(await requireClinicalReadAccess(request, reply, "billing payouts read")))
        return null;
    ...
    if (!identity.verified) {
        reply.code(401).send({
            ...
            message: "Клиника определена не подписанным токеном, а заголовком разработки. " +
    ...
    if (!identity.userId || !identity.role) {
        reply.code(401).send({
            ...
            message: "Нужен вход сотрудника: расчёт выплат показывает зарплату конкретных врачей, " +
...
export async function registerBillingRoutes(app) {
    ...
    app.get("/api/billing/payouts", async (request, reply) => {
        ...
        for (const edge of ["from", "to"]) {
            ...
            if (resolved === null) {
                return reply.code(400).send({
                    error: "PayoutValidationError",
                    message: "Границы периода не разобраны. Передайте календарную дату вида ГГГГ-ММ-ДД либо дату со временем и смещением.",
                });
            }
        ...
        try {
            ...
            return reply.code(500).send({
                ...
                message: "Расчёт выплат не выполнен: сервер не смог посчитать суммы по базе. " +
    ...
    app.post("/api/billing/payments", async (request, reply) => {
        ...
        if (existingPayment) {
            if (existingPayment.patientId !== paymentInput.patientId ||
        ...
        try {
         ...
         записанную. */
            if (isDuplicateClientMutationError(error)) {
                ...
                if (alreadyStored) {
                    if (alreadyStored.patientId !== paymentInput.patientId ||
