...
async function run() {
  ...
  ws.on('open', () => {
    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
            send('Runtime.evaluate', {
              expression: '(() => { return document.querySelector("details.workspace-role-switcher summary strong")?.innerText + " | " + window.location.hash + " | " + document.querySelector(".nav-item.active")?.href; })()'
            });
        }, 4000);
    }, 4000);
  });
