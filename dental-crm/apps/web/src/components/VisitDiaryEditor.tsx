...
function getIcdColor(code: string): string {
  ...
  return ICD_GROUP_COLORS[entry?.group ?? "Другое"] ?? "bg-zinc-500/10 text-zinc-400 border-zinc-500/25";
}
...
export const VisitDiaryEditor: React.FC<VisitDiaryEditorProps> = ({ visitId, patientId }) => {
  ...
  const handleAutoResize = (e: React.ChangeEvent<HTMLTextAreaElement> | React.FocusEvent<HTMLTextAreaElement>) => autoResize(e.target);
  ...
  useEffect(() => {
    document.querySelectorAll<HTMLTextAreaElement>(".auto-resize-ta").forEach(autoResize);
  }, [diary, isLocked]);
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm print-layer">
      <div className="bg-zinc-50/40 text-black w-full max-w-3xl rounded-xl shadow-2xl flex flex-col max-h-[92vh] print-content">
        <div className="flex items-center justify-between p-4 border-b border-gray-200 bg-gray-50 rounded-t-xl no-print">
          ...
          <button onClick={() => setShowPreview(false)} className="text-gray-500 hover:text-black flex items-center gap-1 text-sm">
        ...
        <div className="p-8 overflow-y-auto" id="print-043">
          ...
          <div className="space-y-5">
            <div className="page-break-avoid">
              <h4 className="font-bold border-b border-gray-300 mb-2">S — Жалобы и анамнез (Subjective)</h4>
              <p className="text-sm whitespace-pre-wrap">{diary.anamnesis || "—"}</p>
            </div>
            <div className="page-break-avoid">
              <h4 className="font-bold border-b border-gray-300 mb-2">O — Объективный статус (Status Localis)</h4>
              <p className="text-sm whitespace-pre-wrap">{diary.statusLocalis || "—"}</p>
            </div>
            <div className="page-break-avoid">
              <h4 className="font-bold border-b border-gray-300 mb-2">A — Диагноз (Assessment)</h4>
              <p className="text-sm">
                <strong>МКБ-10:</strong> {diary.diagnosisIcd10 || "—"}{" "}
                {icdEntry ? `(${icdEntry.label})` : ""}
                {diary.diagnosisTooth ? ` | Зуб по FDI: ${diary.diagnosisTooth}` : ""}
              </p>
            </div>
            <div className="page-break-avoid">
              <h4 className="font-bold border-b border-gray-300 mb-2">P — Лечение и план (Plan)</h4>
        ...
        <div className="p-4 border-t border-gray-200 bg-gray-50 flex justify-end rounded-b-xl no-print gap-3">
          <button onClick={() => setShowPreview(false)} className="px-4 py-2 border border-gray-300 rounded-lg text-sm text-gray-700 hover:bg-gray-100">Закрыть</button>
          <button onClick={() => window.print()} className="px-6 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-medium shadow flex items-center gap-2 text-sm">
    ...
    <div className="bg-zinc-950/90 backdrop-blur-xl border border-zinc-800/80 rounded-2xl p-5 shadow-[0_0_60px_-15px_rgba(16,185,129,0.15)] flex flex-col gap-5 relative overflow-hidden group no-print">
      ...
      <div className="relative flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-bold text-zinc-100">Клинический дневник SOAP</h2>
      ...
      <div className="relative grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="lg:col-span-2 bg-zinc-900/40 p-4 rounded-xl border border-zinc-800/60 space-y-3">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <div className="sm:col-span-2 space-y-1.5 relative" ref={icdRef}>
              <label className="text-xs tracking-widest uppercase text-zinc-400 font-semibold flex items-center gap-1.5">
                <span className="text-amber-400 font-mono font-bold">A</span> — Диагноз МКБ-10
              </label>
              {diary.diagnosisIcd10 ? (
                <div className={`w-full rounded-xl px-4 py-3 text-sm font-medium border flex items-center gap-2 ${getIcdColor(diary.diagnosisIcd10)} transition-all`}>
                  <span className="font-mono bg-black/20 px-2 py-0.5 rounded text-xs">{diary.diagnosisIcd10}</span>
                  <span className="flex-1 truncate">
                    {ICD10_DICTIONARY.find(i => i.code === diary.diagnosisIcd10)?.label ?? "Диагноз выбран"}
                  </span>
                  {!isLocked && (
                    <button
                      onClick={() => { setDiary(p => ({ ...p, diagnosisIcd10: "" })); setIcdSearch(""); }}
                    ...
                    <div className="absolute z-30 top-full left-0 right-0 mt-1 bg-zinc-800 border border-zinc-700 rounded-xl shadow-2xl overflow-hidden max-h-52 overflow-y-auto">
                      {filteredIcd.map(icd => (
                        <div
                          ...
                          onMouseDown={e => { e.preventDefault(); handleIcdSelect(icd.code); }}
                        >
                          <span className={`px-2 py-0.5 rounded text-xs font-mono border shrink-0 ${ICD_GROUP_COLORS[icd.group] ?? ""}`}>
                            {icd.code}
                          </span>
                          <div className="min-w-0">
                            <div className="text-sm text-zinc-200 truncate">{icd.label}</div>
                            <div className="text-xs text-zinc-500">{icd.group}</div>
                          </div>
                        </div>
                      ))}
                    </div>
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                onChange={e => setDiary(p => ({ ...p, diagnosisTooth: e.target.value }))}
                placeholder="16, 36..."
                maxLength={8}
              />
            </div>
        ...
        <div className="space-y-1.5 lg:col-span-2">
          <label className="text-xs tracking-widest uppercase text-zinc-400 font-semibold flex items-center gap-1.5">
            ...
            <span className="text-emerald-400 font-mono font-bold">P</span> — Лечение и рекомендации
          </label>
          <textarea
            id="diary-treatment"
            disabled={isLocked}
            ...
            onChange={e => { handleAutoResize(e); setDiary(p => ({ ...p, treatmentDescription: e.target.value })); }}
            onFocus={handleAutoResize}
      ...
      {!isLocked ? (
        ...
        <div className="border-t border-zinc-800/60 pt-4 flex items-center gap-3 text-xs text-zinc-500">
          ...
          <span>
            Дневник подписан{lockedAt ? ` • ${new Date(lockedAt).toLocaleString("ru-RU")}` : ""}.
            {diaryHash && <span className="ml-2 font-mono text-zinc-600">{diaryHash.slice(0, 16)}…</span>}
          </span>
          <button onClick={() => setShowPreview(true)} className="ml-auto flex items-center gap-1 text-zinc-400 hover:text-zinc-200 transition-colors">
        ...
        <div className="modal-overlay">
          <div className="modal-content">
            <div className="modal-header">
              ...
              <p className="modal-subtitle">Введите ПИН-код врача для ЭЦП подписания дневника.</p>
            </div>
            ...
            <div className="modal-footer">
              <button onClick={() => { setShowPinDialog(false); setPinCode(""); }} className="modal-btn secondary">Отмена</button>
              <button id="diary-pin-confirm" onClick={confirmLock} className="modal-btn primary">Подтвердить</button>
            </div>
          </div>
        </div>,
        document.body
