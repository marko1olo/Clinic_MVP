...
import { patients, serviceCatalogItems, toothStateHistory, toothStates, treatmentItems, treatmentPlanItemsNew, treatmentPlans, } from "../db/schema.js";
import { chargeLineKopecks, debtNumericText, rublesFromKopecks, } from "../money/patientDebt.js";
...
async function ensureToothStateHistoryTable() {
    if (toothStateHistoryTableReady)
        return;
    try {
...
function ledgerRowId(planId, slot) {
 ...
 */
const LEDGER_STATUSES_OWNED_BY_PLAN = new Set(["proposed", "approved"]);
...
export async function registerOdontogramRoutes(app) {
    ...
    app.post("/api/patients/:patientId/tooth-states/batch", async (request, reply) => {
        ...
        const inserted = await db.transaction(async (tx) => {
            ...
            const previousByTooth = new Map(previousStates.map((row) => [row.toothNumber, row]));
            await tx
                .delete(toothStates)
                .where(and(eq(toothStates.patientId, patientId), inArray(toothStates.toothNumber, toothNumbers)));
            ...
            if (changedTeeth.length > 0) {
                await tx.insert(toothStateHistory).values(changedTeeth.map((toothNumber) => ({
                    organizationId,
                    patientId,
                    toothNumber,
        ...
        try {
            ...
            planId = await db.transaction(async (tx) => {
                ...
                const ownedRows = await tx
                    ...
                    .from(treatmentItems)
                    .where(and(eq(treatmentItems.organizationId, organizationId), eq(treatmentItems.patientId, patientId), sql `left(${treatmentItems.id}::text, ${sql.raw(String(LEDGER_ID_PREFIX_LENGTH))}) = ${ledgerPrefix}`));
                /*
                ...
                if (input.items.length > 0) {
                    ...
                    const knownServiceIds = new Set(priceIdCandidates.length === 0
                        ? []
                        : (await tx
                            .select({ id: serviceCatalogItems.id })
                            .from(serviceCatalogItems)
                            .where(and(eq(serviceCatalogItems.organizationId, organizationId), inArray(serviceCatalogItems.id, priceIdCandidates)))).map((row) => row.id));
                    /*
                    ...
                    const completedItems = await tx
                        ...
                        .from(treatmentItems)
                        .where(and(eq(treatmentItems.organizationId, organizationId), eq(treatmentItems.patientId, patientId), eq(treatmentItems.status, "completed")));
                    ...
                    const evaluation = await evaluateClinicalRulesInDb(organizationId, {
                        ...
                        completedServiceIds: completedItems.map((r) => r.serviceId).filter(Boolean),
                    ...
                    const ledgerValues = input.items.map((item, index) => {
                        while (slot <= LEDGER_MAX_SLOT &&
                            keptSlots.has(ledgerRowId(savedPlanId, slot).toLowerCase())) {
                            slot += 1;
                        }
                        if (slot > LEDGER_MAX_SLOT) {
                            throw new Error(`Позиции плана лечения некуда записать: свободных слотов книги лечения не осталось (позиция ${index + 1}).`);
                        }
                         ...
                         */
                        const unitPriceRub = rublesFromKopecks(chargeLineKopecks({
                            patientId,
                            status: ledgerStatus,
                            unitPriceRub: item.price,
                            quantity: 1,
                        ...
                        return {
                            ...
                            serviceId: knownServiceIds.has(item.priceId)
                                ? item.priceId
                                : null,
                            toothCode: item.toothNumber === null || item.toothNumber === undefined
                                ? null
                            ...
                            unitPriceRub,
                            discountRub: rublesFromKopecks(chargeLineKopecks({
                                patientId,
                                status: ledgerStatus,
