...
function numberToWordsRu(num) {
    ...
    function getGroup(n, female) {
        ...
        if (n > 0) {
            res.push((female ? unitsFemale[n] : units[n]) || "");
        }
        return res.join(" ");
    }
    const result = [];
    let part = Math.floor(num / 1000000000);
    if (part > 0) {
        const gr = getGroup(part, false);
        const rem = part % 100;
        if (rem >= 11 && rem <= 19)
            result.push(gr + " миллиардов");
        else if (rem % 10 === 1)
            result.push(gr + " миллиард");
        else if (rem % 10 >= 2 && rem % 10 <= 4)
            result.push(gr + " миллиарда");
        else
            result.push(gr + " миллиардов");
        num %= 1000000000;
    }
    part = Math.floor(num / 1000000);
    if (part > 0) {
        const gr = getGroup(part, false);
        const rem = part % 100;
        if (rem >= 11 && rem <= 19)
            result.push(gr + " миллионов");
        else if (rem % 10 === 1)
            result.push(gr + " миллион");
        else if (rem % 10 >= 2 && rem % 10 <= 4)
            result.push(gr + " миллиона");
        else
            result.push(gr + " миллионов");
        num %= 1000000;
    }
    part = Math.floor(num / 1000);
    if (part > 0) {
        const gr = getGroup(part, true);
        const rem = part % 100;
        if (rem >= 11 && rem <= 19)
            result.push(gr + " тысяч");
        else if (rem % 10 === 1)
            result.push(gr + " тысяча");
        else if (rem % 10 >= 2 && rem % 10 <= 4)
            result.push(gr + " тысячи");
        else
            result.push(gr + " тысяч");
        num %= 1000;
    }
    if (num > 0 || result.length === 0) {
        result.push(getGroup(num, false));
    }
...
function getKopecksStr(rem) {
    if (rem >= 11 && rem <= 19)
        return "копеек";
    if (rem % 10 === 1)
        return "копейка";
    if (rem % 10 >= 2 && rem % 10 <= 4)
        return "копейки";
    return "копеек";
}
export function rubWords(value) {
    if (value === null || value === undefined)
        return "не указана";
    const totalKopecks = Math.round(value * 100);
    const roubles = Math.floor(totalKopecks / 100);
    const kopecks = totalKopecks % 100;
    const words = numberToWordsRu(roubles);
    let rubStr = "рублей";
    const rem = roubles % 100;
    if (rem >= 11 && rem <= 19) {
        rubStr = "рублей";
    }
    else if (rem % 10 === 1) {
    ...
    const kopStr = getKopecksStr(kopecks);
    const kopecksPadded = kopecks.toString().padStart(2, "0");
    const mainFormatted = value.toLocaleString("ru-RU", {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2,
    });
    const capitalizedWords = words.charAt(0).toUpperCase() + words.slice(1);
    return `${mainFormatted} руб. (${capitalizedWords} ${rubStr} ${kopecksPadded} ${kopStr})`;
}
...
function hasPersonNameParts(value) {
...
function clinicalToothStatusLabel(value) {
    const labels = {
        sound: "без патологии",
        watch: "наблюдение",
        caries: "кариес",
...
function clinicalToothRowsTable(rows) {
    const toothRows = rows
        .map((item) => `<tr>${cell(item.toothOrArea)}${cell(item.surfaces.map(clinicalToothSurfaceLabel).join(", "))}${cell(clinicalToothStatusLabel(item.status))}${cell(item.diagnosisOrFinding)}${cell(item.indication)}${cell(item.plannedAction)}${cell(item.prognosis)}${cell(item.periodontalStatus)}${cell(item.implantOrProstheticNotes)}${cell(item.orthodonticNotes)}</tr>`)
        .join("");
    return `<table>
    <tr>
      <th>Зуб/сегмент</th>
...
function clinicLegalProfileMissingFields(profile) {
    if (!profile)
...
function signatureParty(role, fullName) {
    const trimmed = fullName?.trim();
    return trimmed ? `${role} (${trimmed})` : role;
}
function issueSignatureModeLabel(mode) {
...
function issueSignatureAttestationBlock(document) {
    const attestation = document.signatureAttestation;
    if (!attestation || document.status === "draft")
        return "";
...
function releaseMaterialKindLabel(kind) {
    if (kind === "original")
        return "оригинал";
    if (kind === "copy")
        return "копия";
    if (kind === "extract")
        return "выписка";
    if (kind === "dicom_archive")
        return "архив исходных снимков";
    if (kind === "mixed")
        return "смешанный комплект";
    return "иное";
}
function releaseDeliveryMethodLabel(method) {
    if (method === "paper")
        return "бумага";
    if (method === "pdf")
        return "PDF";
    if (method === "dicom_archive")
        return "архив исходных снимков";
    if (method === "secure_link")
        return "защищенная ссылка";
    if (method === "physical_media")
        return "физический носитель";
    return "иной канал";
}
function releaseJournalEntryKindLabel(kind) {
    if (kind === "request_registered")
        return "зарегистрирован запрос";
    if (kind === "extract_issued")
        return "выдана выписка";
    return "закрыта выдача";
}
function releaseJournalBlock(document) {
    const journal = document.releaseJournalEntry;
    if (!journal || document.status === "draft")
        return "";
...
function baseDocument(title, patient, document, body, context) {
...
<head>
  ...
  <style>
    ...
    body { color: #211f1c; font: 14px/1.5 'Roboto', Arial, sans-serif; margin: 40px; }
    ...
    @media print {
      ...
      .notice { background: none; border-color: #000; color: #000; }
...
const treatmentPlanBackedFinancialKinds = new Set([
...
function serviceCatalogMap(context) {
...
function treatmentPlanTotalRub(document, context) {
    const total = financialDocumentTreatmentItems(document, context).reduce((sum, item) => sum + treatmentPlanItemTotalRub(item), 0);
    return total > 0 ? total : document.totalAmountRub;
}
function financialServiceRows(document, context, includeStatus = false) {
...
function financialServiceTable(document, context, includeStatus = false) {
    return `<table>
...
function paidMedicalServicesContract(document, context) {
    ...
    if (payload) {
        const customerRows = [
            ...
            row("Дата договора", formatDateRu(payload.contractDate)),
    ...
    <table>
      ...
      ${row("Ориентировочная стоимость", rubWords(payload.estimatedTotalRub))}
    ...
    <table>
      ...
      ${row("Ориентировочная сумма", rubWords(treatmentPlanTotalRub(document, context)))}
...
function completedWorksAct(document, context) {
    ...
    <table>
      ...
      ${row("Дата акта", formatDateRu(payload.actDate))}
      ${row("Договор", payload.contractNumber)}
      ${row("ID выданного договора", payload.linkedContractDocumentId)}
      ${row("Период оказания", `с  по ${payload.servicePeriodEnd}`)}
      ${row("Врач-исполнитель", payload.doctorFullName)}
      ${row("Состав работ", payload.acceptedServicesSummary)}
      ${row("Сумма по акту", rubWords(payload.totalByActRub))}
      ${row("Фактически оплачено", rubWords(payload.paidRub))}
    ...
    ${financialServiceTable(document, context, true)}
    <table>
      ${row("Фактически оплачено", rubWords(paidTotalForDocument(document, context) || document.totalAmountRub))}
      ${row("Сумма по акту", rubWords(treatmentPlanTotalRub(document, context)))}
    </table>
...
function firstTaxPayment(payments) {
    return payments[0] ?? null;
}
function namePartsForTax(fullName) {
...
function identityDocumentKindCode(value) {
    ...
    if (normalized.includes("военн") || normalized.includes("military"))
        return "07";
    if (normalized.includes("вид на жительство"))
        return "12";
    return "91";
}
function identityDocumentNumberForTax(value) {
    const normalized = present(value);
    if (!normalized)
        return null;
    const passport = normalized.match(/(\d{2})\s*(\d{2})\s*(\d{6})/);
    if (passport)
        return `${passport[1]}${passport[2]} ${passport[3]}`;
...
function taxReceiptList(payments) {
    return payments.map((payment) => paymentReceiptLabel(payment)).join("; ");
}
function kndPersonRows(prefix, parts, inn, birthDate, identity) {
...
function officialKnd1151156PrintBlock(document, patient, context, taxPayments, taxpayerPayment, regularTreatmentRub, expensiveTreatmentRub) {
    ...
    <table>
      ...
      ${kndPersonRows("Налогоплательщик", taxpayerNameParts, payerInnForTax(taxpayerPayment, patient), payerBirthDateForTax(taxpayerPayment, patient), taxpayerIdentityDocument)}
...
function payerNameForTax(payment, patient) {
    return payment?.payerFullName?.trim() || patient.fullName;
}
function payerInnForTax(payment, patient) {
...
const taxApplicationRelationshipLabels = {
...
function normalizedTaxpayerRelationship(value) {
    ...
    if (["ward", "подопечный", "подопечная", "опекаемый", "опекаемая"].includes(normalized))
        return "ward";
    return null;
}
function taxpayerPatientSameFlag(payment) {
...
function hasFiscalReceiptNumber(payment) {
    return Boolean(present(payment.fiscalReceiptNumber));
}
function hasAllFiscalReceipts(payments) {
    return payments.every(hasFiscalReceiptNumber);
}
function hasFiscalReceiptDate(payment) {
    return isValidDateLike(payment.fiscalReceiptIssuedAt);
}
function hasAllFiscalReceiptDates(payments) {
...
function hasPaymentPayerIdentity(payment) {
    const payerInnLength = digitsOnly(payment.payerInn).length;
    return Boolean(hasPersonNameParts(payment.payerFullName) &&
...
function hasAllPaymentPayerIdentities(payments) {
    return payments.every(hasPaymentPayerIdentity);
}
function normalizedFiscalReceiptNumber(value) {
    return (present(value) ?? "").replace(/\s+/g, " ").toLocaleUpperCase("ru-RU");
}
function normalizedDocumentValue(value) {
    return (present(value) ?? "").replace(/\s+/g, " ").toLocaleLowerCase("ru-RU");
}
function paymentReceiptStoredFieldMatchesPayload(storedValue, payloadValue) {
...
function paymentReceiptSelectionBlockReason(document, context) {
    if (document.kind !== "payment_receipt")
        return null;
...
function completedWorksActFiscalReceiptBlockReason(document, context) {
    if (document.kind !== "completed_works_act")
        return null;
    const payload = document.payload?.completedWorksAct;
    if (!payload)
...
function paymentRefundCorrectionFiscalReceiptBlockReason(document, context) {
    if (document.kind !== "payment_refund_correction_request")
        return null;
...
function hasExplicitTaxDeductionCode(payment) {
    return payment.taxDeductionCode === "1" || payment.taxDeductionCode === "2";
}
function paymentMethodForDocument(payment) {
    const labels = {
        cash: "наличные",
        card: "карта",
...
function paymentReceiptRows(payments) {
    if (!payments.length) {
...
function taxRegistryRows(payments, document) {
    if (!payments.length) {
    ...
    return payments
        ...
        return `<tr><td>${escapeHtml(dateLabel)}</td><td>${escapeHtml(paymentReceiptLabel(payment))}</td><td>${escapeHtml(taxPaymentCodeLabel(taxPaymentCode(payment)))}</td><td>${escapeHtml(rub(payment.amountRub))}</td></tr>`;
    })
        .join("");
}
function taxDeductionCertificate(document, patient, context) {
    ...
    ${officialKnd1151156PrintBlock(document, patient, context, taxPayments, taxpayerPayment, regularTreatmentRub, expensiveTreatmentRub)}
...
function taxDeductionApplication(document, patient, context) {
...
function informedConsent(document) {
    const payload = document.payload?.informedConsent;
    if (payload) {
        return `<div class="notice">
...
function procedureSpecificConsentPacket(document) {
    const payload = document.payload?.procedureSpecificConsent;
    if (payload) {
...
function treatmentPlan(document) {
    const payload = document.payload?.treatmentPlan;
    if (payload) {
        const stageRows = payload.plannedStages
            .map((stage) => `<tr>${cell(stage.stageName)}${cell(stage.plannedServices)}${cell(stage.plannedTiming)}${cell(stage.clinicalNotes?.trim() || "по клинической ситуации")}${cell(rub(stage.estimatedAmountRub ?? null))}</tr>`)
            .join("");
...
function treatmentPlanAcceptanceVariantLabel(value) {
    const labels = {
        urgent: "срочный",
...
function treatmentPlanAcceptance(document) {
    const payload = document.payload?.treatmentPlanAcceptance;
    if (!payload) {
        return fallbackTreatmentPlanAcceptance(document);
    }
    const stageRows = payload.acceptedStages
        .map((stage) => `<tr>${cell(stage.stageName)}${cell(stage.plannedServices)}${cell(stage.plannedTiming)}${cell(rub(stage.estimatedAmountRub ?? null))}</tr>`)
        .join("");
...
function fallbackTreatmentPlanAcceptance(document) {
    return `<h2>Согласование плана лечения</h2>
    <table>
...
function anesthesiaConsentLog(document) {
    const payload = document.payload?.anesthesiaConsentLog;
    if (payload) {
        const doseRows = payload.doseRows
            .map((dose) => `<tr>${cell(dose.time)}${cell(dose.medication)}${cell(dose.doseMl)}${cell(dose.zone)}${cell(dose.reaction ?? "без особенностей")}</tr>`)
            .join("");
...
function prescriptionMedicationOrder(document) {
    const payload = document.payload?.prescriptionMedicationOrder;
    if (payload) {
        const medicationRows = payload.medications
            .map((medication) => `<tr>${cell(medication.medication)}${cell(medication.dosage)}${cell(medication.instructions)}${cell(medication.duration)}</tr>`)
            .join("");
...
function personalDataConsent(document, patient) {
    const payload = document.payload?.personalDataProcessingConsent;
    if (payload) {
...
function minorLegalRepresentativeConsent(document, patient) {
    const payload = document.payload?.minorLegalRepresentativeConsent;
    if (payload) {
...
function photoVideoConsent(document) {
...
function medicalInterventionRefusal(document) {
    const payload = document.payload?.medicalInterventionRefusal;
    if (!payload) {
...
function treatmentCostEstimate(document, context) {
    const payload = document.payload?.treatmentCostEstimate;
    if (payload) {
      ...
      <table>
        ${row("Итого по смете", rubWords(payload.totalAmountRub))}
    ...
    ${financialServiceTable(document, context, true)}
    <table>
      ${row("Итого по смете", rubWords(treatmentPlanTotalRub(document, context)))}
...
function paymentInvoice(document, context) {
    ...
    if (payload) {
      ...
      <table>
        ${row("Итого к оплате", rubWords(payload.totalAmountRub))}
      </table>
      ${checkList([
    ...
    <table>
      ...
      ${row("Сумма к оплате", rubWords(treatmentPlanTotalRub(document, context)))}
...
function paymentReceipt(document, context) {
    const payload = document.payload?.paymentReceipt;
...
function installmentPaymentSchedule(document, context) {
    const payload = document.payload?.installmentPaymentSchedule;
    if (payload) {
        const statusLabels = {
      ...
      <table>
        ...
        ${row("Плательщик", payload.payerFullName)}
        ${row("Общая сумма", rubWords(payload.totalAmountRub))}
...
function postVisitCareTopicLabel(value) {
    const labels = {
...
function postVisitRecommendations(document) {
    const payload = document.payload?.postVisitRecommendations;
    if (payload) {
        const attestation = document.signatureAttestation;
...
function medicalRecordExtract(document, patient) {
...
function outpatient025uValue(value) {
    return present(value) ?? "нет данных";
}
function outpatient025uCode(value, labels) {
    return value && value !== "unknown" ? labels[value] : "нет данных";
}
function outpatient025uDoctorLine(doctor) {
    if (!doctor)
        return "нет данных";
...
function outpatient025uDiagnosisRowsTable(rows, emptyText) {
    if (!rows.length)
        return `<p>${escapeHtml(emptyText)}</p>`;
    return `<table>
    <tr><th>Дата</th><th>Диагноз</th><th>МКБ-10</th><th>Первичный / повторный</th><th>Врач</th></tr>
    ${rows
        .map((item) => `<tr>${cell(item.date)}${cell(item.diagnosis)}${cell(item.icd10Code)}${cell(outpatient025uFirstOrRepeat(item.firstOrRepeat))}${cell(outpatient025uDiagnosisDoctorLine(item))}</tr>`)
        .join("")}
  </table>`;
}
function outpatient025uTextRecordsTable(rows, emptyText) {
    if (!rows.length)
        return `<p>${escapeHtml(emptyText)}</p>`;
    return `<table>
...
function outpatient025uEventRowsTable(rows, emptyText) {
    if (!rows.length)
        return `<p>${escapeHtml(emptyText)}</p>`;
    return `<table>
    <tr><th>Дата</th><th>Событие</th><th>Организация</th><th>Результат</th><th>Врач</th></tr>
    ${rows
        .map((item) => `<tr>${cell(item.date)}${cell(item.event)}${cell(item.organization)}${cell(item.result)}${cell(outpatient025uDoctorLine(item.doctor))}</tr>`)
        .join("")}
  </table>`;
}
function outpatient025uXrayDoseTable(rows, emptyText) {
    if (!rows.length)
        return `<p>${escapeHtml(emptyText)}</p>`;
    return `<table>
...
function outpatientMedicalCard025u(document, patient) {
    const payload = document.payload?.outpatientMedicalCard025u;
    if (!payload) {
...
function render025uSpecialistVisitRecord(record, index) {
    return `<section>
...
function render025uMedicalOrganization(payload) {
...
function render025uPatientInfo(payload, patient, sexLabel, registrationType, stayType) {
...
function structuredMedicalRecordCopyRequest(document, patient) {
...
function medicalDocumentReleaseReceipt(document, patient) {
...
function xrayCbctReferral(document) {
...
function labWorkOrder(document) {
...
function visitAttendanceCertificate(document, patient) {
    const payload = document.payload?.visitAttendanceCertificate;
    if (!payload) {
...
function warrantyServiceMemo(document) {
    const payload = document.payload?.warrantyServiceMemo;
    if (payload) {
...
function paymentRefundCorrectionRequest(document, context) {
...
function taxDeductionRegistry(document, context) {
    const taxPayments = taxPaymentsForDocument(document, context);
...
function patientIntakeQuestionnaire(document) {
...
function taxpayerIdentityKey(payment, patient) {
    return [
...
function taxDocumentBlockReason(document, patient, context) {
    if (!taxFiscalDocumentKinds.has(document.kind))
        return null;
    if (!document.taxYear)
...
export function documentIssueBlockReason(document, patient, context = {}) {
