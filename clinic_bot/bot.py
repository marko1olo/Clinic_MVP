...
def on_mqtt_message(client, userdata, msg):
    ...
    if topic == TOPIC_XRAY_RESULT:
        ...
        if image_b64:
            ...
            asyncio.run_coroutine_threadsafe(broadcast_photo(
                photo_bytes, caption, report, role='doctor'), loop)
            ...
            asyncio.run_coroutine_threadsafe(
                broadcast(admin_text, role='admin'), loop)
        else:
            text = f"🦷 *Анализ снимка готов*\n👤 _{patient_str}_\n\nНаходки:\n{report}\n"
            asyncio.run_coroutine_threadsafe(
                broadcast(text, role='doctor'), loop)
...
async def main():
    ...
    mqtt_thread = threading.Thread(
        target=start_mqtt, args=(loop,), daemon=True)
    mqtt_thread.start()
