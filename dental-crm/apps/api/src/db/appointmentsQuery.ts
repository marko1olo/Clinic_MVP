...
function appointmentClinicTimeParts(date: Date, sourceTimeZone: string): { weekday: number; minute: number } {
...
async function assertAppointmentCanBeScheduled(
  ...
  const overlapping = activeApps.find((app) => {
    ...
    if (candidate.patientId && app.patientId === candidate.patientId) return true;
    if (candidate.doctorUserId && app.doctorUserId === candidate.doctorUserId) return true;
    if (candidate.assistantUserId && app.assistantUserId === candidate.assistantUserId) return true;
