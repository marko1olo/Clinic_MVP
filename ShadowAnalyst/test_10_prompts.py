...
if os.environ.get("GROQ_API_KEYS"):
    API_KEYS = [k.strip() for k in os.environ["GROQ_API_KEYS"].split(",") if k.strip()]
elif os.environ.get("GROQ_API_KEY"):
    API_KEYS = [os.environ["GROQ_API_KEY"].strip()]
    API_KEYS = []
...
async def call_groq(prompt, b64):
    ...
    if not keys:
        ...
        print(
            "Ошибка: API ключи не настроены. Установите GROQ_API_KEYS или GROQ_API_KEY."
        )
...
# --- 10 СТИЛЕЙ ОТРИСОВКИ ---
def style_1_neon(draw, img, bbox, name):
...
def style_2_sci_fi(draw, img, bbox, name):
...
def style_3_rounded(draw, img, bbox, name):
...
def style_4_ellipse(draw, img, bbox, name):
...
def style_5_dashed(draw, img, bbox, name):
    """Стиль 5: Пунктирная линия (поиск/подозрение)"""
    x1, y1, x2, y2 = bbox.x1, bbox.y1, bbox.x2, bbox.y2
    step = 15
    color = (255, 255, 0)
    for x in range(x1, x2, step * 2):
        draw.line([(x, y1), (min(x + step, x2), y1)], fill=color, width=4)
    for x in range(x1, x2, step * 2):
        draw.line([(x, y2), (min(x + step, x2), y2)], fill=color, width=4)
    for y in range(y1, y2, step * 2):
        draw.line([(x1, y), (x1, min(y + step, y2))], fill=color, width=4)
    for y in range(y1, y2, step * 2):
        draw.line([(x2, y), (x2, min(y + step, y2))], fill=color, width=4)
    draw_text_with_bg(draw, f"5. Пунктир: {name}", x1, max(0, y1 - 45), color)


def style_6_overlay(draw, img, bbox, name):
    """Стиль 6: Заливка полупрозрачным цветом"""
    x1, y1, x2, y2 = bbox.x1, bbox.y1, bbox.x2, bbox.y2
    overlay = Image.new("RGBA", img.size, (0, 0, 0, 0))
    d = ImageDraw.Draw(overlay)
    d.rectangle(
        [x1, y1, x2, y2], fill=(255, 0, 0, 60), outline=(255, 0, 0, 200), width=3
    )
...
def style_7_police(draw, img, bbox, name):
...
def style_8_minimal(draw, img, bbox, name):
...
def style_9_target(draw, img, bbox, name):
...
def style_10_glass(draw, img, bbox, name):
    ...
    with Image.open(IMG_PATH) as img:
        ...
        for obj in objects:
            ...
            if len(box) == 4:
                ...
                if x2 > x1 and y2 > y1:
                    STYLES[i](draw, img, BoundingBox(x1, y1, x2, y2), name)
