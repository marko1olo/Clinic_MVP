...
import { access, open, opendir, readdir, readFile, stat, } from "node:fs/promises";
import net from "node:net";
import os from "node:os";
import path from "node:path";
import { setImmediate as yieldImmediate } from "node:timers/promises";
import { createInflateRaw, deflateSync } from "node:zlib";
import { createImagingStudySchema, dicomFirstFramePreviewRequestSchema, dicomFirstFramePreviewResponseSchema, dicomFolderSeriesPreviewRequestSchema, dicomFolderSeriesPreviewResponseSchema, dicomFolderWorkupPlanRequestSchema, dicomFolderWorkupPlanResponseSchema, dicomLocalFolderDiscoveryRequestSchema, dicomLocalFolderDiscoveryResponseSchema, dicomRenderCachePlanRequestSchema, dicomRenderCachePlanResponseSchema, dicomSeriesPreviewRequestSchema, dicomSeriesPreviewResponseSchema, dicomViewerLaunchManifestRequestSchema, dicomViewerLaunchManifestResponseSchema, dicomViewerToolStateBundleRequestSchema, dicomViewerToolStateBundleResponseSchema, dicomViewerWorkbenchManifestRequestSchema, dicomViewerWorkbenchManifestResponseSchema, dicomWebConnectorCheckRequestSchema, dicomWebConnectorCheckResponseSchema, dicomWorkbenchBundleListResponseSchema, dicomWorkbenchBundleResponseSchema, dicomWorkstationReadinessRequestSchema, dicomWorkstationReadinessResponseSchema, imagingFolderScanRequestSchema, imagingFolderScanResponseSchema, imagingImportCommitResponseSchema, imagingImportPreviewRequestSchema, imagingImportPreviewResponseSchema, imagingStudySchema, imagingViewerSessionResponseSchema, localImagingOrganizerRequestSchema, localImagingOrganizerResponseSchema, normalizeDate, saveDicomWorkbenchBundleRequestSchema, saveImagingViewerSessionRequestSchema, splitLine, } from "@dental/shared";
import { denteAdminSecretHeader, requireClinicalMutationAccess, requireClinicalReadAccess, unguardedBypassAllowed, } from "../accessGuard.js";
import { analyzeVisiographImage } from "../ai/visiograph.js";
import { analyzeImagingStudy } from "../ai/visionAnalyzer.js";
import { createImagingStudyInDb, getAllImagingStudies, getImagingStudiesForPatient, getImagingStudyById, getOrCreateImagingViewerSession, listDicomWorkbenchBundles, saveDicomWorkbenchBundle, saveImagingViewerSession, updateImagingStudyAiSummaryInDb, } from "../db/imagingQuery.js";
...
function parseImagingPayload(schema, value, message) {
...
function configuredDicomWebSettingsSecret() {
 ...
 */
function dicomWebSettingsUnguardedAllowed() {
    return unguardedBypassAllowed("DENTE_SETTINGS_ALLOW_UNGUARDED_MUTATIONS");
}
function timingSafeDicomWebSecretEqual(providedSecret, expectedSecret) {
    if (!providedSecret)
        return false;
...
async function requireDicomWebSettingsAccess(request, reply) {
    const adminSecret = configuredDicomWebSettingsSecret();
    if (!adminSecret) {
        if (dicomWebSettingsUnguardedAllowed())
            return true;
        reply.code(503).send({
    ...
    const normalizedProvidedSecret = Array.isArray(providedSecret)
        ? providedSecret[0]
        : providedSecret;
    if (timingSafeDicomWebSecretEqual(typeof normalizedProvidedSecret === "string"
...
const apiDicomScanAbortMessage = "Сканирование локальных снимков остановлено: клиент закрыл запрос или отменил действие.";
function createApiDicomScanYieldState() {
    return { units: 0, lastYieldAtMs: Date.now() };
}
function createImagingRequestAbortSignal(request) {
...
function throwIfApiDicomScanAborted(signal) {
    if (!signal?.aborted)
        return;
...
function isApiDicomScanAbortError(error) {
...
async function maybeYieldApiDicomScan(state, signal) {
    throwIfApiDicomScanAborted(signal);
    state.units += 1;
    const now = Date.now();
    if (state.units % apiDicomScanYieldEveryUnits !== 0 &&
...
function sendImagingScanCancelled(reply) {
...
async function runAbortableImagingScan(request, reply, operation) {
    ...
    try {
        ...
        if (isApiDicomScanAbortError(error))
            return sendImagingScanCancelled(reply);
        throw error;
    }
}
const imagingStudyNotFoundError = "ImagingStudyNotFound";
const imagingStudyScopeError = "ImagingStudyScopeError";
function sendImagingStudyNotFound(reply) {
    return reply.code(404).send({
        error: imagingStudyNotFoundError,
        message: "Снимок не найден.",
    });
}
function sendImagingStudyScopeError(reply, statusCode, message) {
...
function detectSourceKind(value, fallback) {
    const text = value ?? "";
    if (/dicomweb|qido|wado/i.test(text))
        return "dicomweb";
    if (/pacs|orthanc|dcm4chee/i.test(text))
        return "pacs";
    if (fallback === "dicomweb" || fallback === "pacs")
        return fallback;
    if (/twain|wia/i.test(text))
        return "twain_wia";
    if (/sensor|rvg|ezsensor|carestream|vatech|sopro|xios|schick|kodak|vistascan/i.test(text))
    ...
    if (/watch|folder|папк/i.test(text))
        return "folder_watch";
    return fallback;
}
function extractFilePath(value) {
    const virtualArchivePath = value.match(/[A-Za-zА-Яа-яЁё]:[\\/][^;|\n]+?\.(?:zip)::[^;|\n]+?\.(?:dcm|dicom|ima)\b|\\\\[^;|\n]+?\.(?:zip)::[^;|\n]+?\.(?:dcm|dicom|ima)\b|\/[^;|\n]+?\.(?:zip)::[^;|\n]+?\.(?:dcm|dicom|ima)\b/i)?.[0];
    if (virtualArchivePath)
        return virtualArchivePath.trim();
    const absolutePath = value.match(/[A-Za-zА-Яа-яЁё]:[\\/][^;|\n,]+?(?:\.(?:dcm|dicom|ima|jpg|jpeg|png|tif|tiff|bmp|webp|zip|7z|rar)\b|[\\/]DICOMDIR\b)|\\\\[^;|\n,]+?(?:\.(?:dcm|dicom|ima|jpg|jpeg|png|tif|tiff|bmp|webp|zip|7z|rar)\b|[\\/]DICOMDIR\b)|\/[^;|\n,]+?(?:\.(?:dcm|dicom|ima|jpg|jpeg|png|tif|tiff|bmp|webp|zip|7z|rar)\b|\/DICOMDIR\b)/i)?.[0];
    if (absolutePath)
...
const dicomUidPatternCache = new Map();
function extractDicomUid(value, labels) {
    for (const label of labels) {
        ...
        if (match?.[1])
            return normalizeDicomUid(match[1]);
    }
    return null;
}
function normalizeModality(value) {
    ...
    if (/CBCT|КЛКТ|ККТ/.test(normalized))
        return "CBCT";
    if (/\bCT\b|КТ/.test(normalized))
        return "CT";
    if (/\bDX\b|DIGITAL RADIOGRAPHY/.test(normalized))
        return "DX";
    if (/\bCR\b/.test(normalized))
        return "CR";
    if (/\bPX\b|PAN|OPG|ОПТГ|ОРТОПАН/.test(normalized))
        return "PX";
    if (/CEPH|TRG|ТРГ|ТЕЛЕРЕНТГ/.test(normalized))
        return "CEPH";
    if (/\bIO\b|RVG|ПРИЦЕЛ/.test(normalized))
        return "IO";
    if (/\bMR\b/.test(normalized))
        return "MR";
    if (/\bUS\b/.test(normalized))
        return "US";
    return normalized.slice(0, 24);
}
function modalityToKind(modality, text) {
...
function parseInstanceNumber(value) {
    if (!value)
        return null;
...
const dicomFieldValuePatternCache = new Map();
function extractDicomFieldValue(line, labels) {
    for (const label of labels) {
...
async function parseManifestLine(orgId, line, rowNumber, sourceKind, sourceName) {
    ...
    if (!filePath)
        warnings.push("Нет пути к файлу снимка");
...
export async function parseImagingManifest(orgIdOrInput, maybeInput) {
    ...
    const input = typeof orgIdOrInput === "object"
        ? orgIdOrInput
    ...
    const sourceKind = typeof input === "string"
        ? "folder_watch"
    ...
    const rows = await Promise.all((hasHeader ? lines.slice(1) : lines).map(async (line, index) => {
        ...
        headers.forEach((field, cellIndex) => {
            ...
            if (field === "phone")
                draft.phone = normalizePhone(value);
            else if (field === "kind")
                draft.kind = detectKind(value);
...
async function collectImagingFiles(root, recursive, maxFiles, options = {}, limits = {}) {
    const files = [];
    const warnings = [];
    ...
    await new Promise((resolve, reject) => {
        ...
        async function processFolder(current) {
            try {
...
function quoteManifestCell(value) {
    if (!value)
        return "";
    if (!/[;"\n]/.test(value))
        return value;
...
function buildFolderScanManifest(files) {
...
async function collectDicomHeaderFiles(root, recursive, maxFiles, options = {}, limits = {}) {
    const files = [];
    const warnings = [];
    const queue = [path.resolve(root)];
    const maxFolders = Math.max(1, Math.floor(limits.maxFolders ?? apiDicomDefaultMaxFolders));
    ...
    while (queueIndex < queue.length &&
        ...
        try {
            ...
            for await (const entry of directory) {
                ...
                if (!entry.isFile())
                    continue;
                if (!isDicomHeaderCandidatePath(fullPath))
                    continue;
                files.push(fullPath);
                if (files.length >= maxFiles) {
...
async function buildDicomHeaderManifest(input, options = {}) {
    const rows = [];
    const warnings = [];
    let filesParsed = 0;
    const yieldState = createApiDicomScanYieldState();
...
function isDicomArchivePath(filePath) {
    if (!filePath)
        return false;
...
function isDicomArchiveVirtualEntryPath(filePath) {
    if (!filePath?.includes("::"))
        return false;
...
function isDicomLikeEntry(entryName) {
    const normalized = entryName.replaceAll("\\", "/");
    const extension = path.extname(normalized).toLowerCase();
    return (dicomPixelFileExtensions.has(extension) ||
        /(?:^|\/)DICOMDIR$/i.test(normalized));
}
function isDicomPixelPath(filePath) {
...
function hasDicomMagic(filePath) {
    try {
...
function isDicomHeaderCandidatePath(filePath) {
    if (isDicomPixelPath(filePath) || isZipArchivePath(filePath))
        return true;
...
function emptyDicomHeaderMetadata(warnings = []) {
    return {
        patientName: null,
        modality: null,
...
function parseDicomUnsignedInt(valueBuffer) {
    const text = cleanDicomText(valueBuffer);
    if (text) {
        const parsedText = Number.parseInt(text, 10);
        if (Number.isInteger(parsedText) && parsedText > 0)
            return parsedText;
    }
...
function updateDicomEstimatedPixelBytes(metadata) {
    if (!metadata.imageRows ||
...
function assignDicomHeaderValue(metadata, tagKey, valueBuffer) {
    ...
    if (tagKey === "00280010")
        ...
        else if (tagKey === "00081030")
            metadata.studyDescription = value;
        else if (tagKey === "0008103e")
...
function parseDicomHeader(buffer) {
    ...
    if (!metadata.tagsRead)
        metadata.warnings.push("В доступной части заголовка не найдены известные метаданные снимка.");
    return metadata;
}
...
function redactDicomPreviewText(value) {
    return value
        .replace(/[A-Za-z]:[\\/][^\r\n]*/g, "redacted-local-dicom-path")
        .replace(/\\\\[^\r\n]*/g, "redacted-local-dicom-path");
}
function redactDicomPreviewWarnings(warnings) {
...
function readDicomUs(buffer, bigEndian) {
    if (buffer.length < 2)
        return null;
    return bigEndian ? buffer.readUInt16BE(0) : buffer.readUInt16LE(0);
}
function readDicomDsNumber(buffer) {
...
function dicomTransferSyntaxIsSupported(transferSyntaxUid) {
...
function extractDicomMetadata(buffer, warnings) {
    let cursor = buffer.length >= 132 &&
        buffer.subarray(128, 132).toString("latin1") === "DICM"
        ? 132
        : 0;
    ...
    for (let guard = 0; guard < 100_000 && cursor + 8 <= buffer.length; guard += 1) {
        ...
        else if (tagKey === "00280010")
            rows = readDicomUs(value, bigEndian);
        else if (tagKey === "00280011")
            columns = readDicomUs(value, bigEndian);
        else if (tagKey === "00280100")
            bitsAllocated = readDicomUs(value, bigEndian);
        else if (tagKey === "00280101")
            bitsStored = readDicomUs(value, bigEndian);
        else if (tagKey === "00280103")
            pixelRepresentation = readDicomUs(value, bigEndian);
        else if (tagKey === "00281050")
            windowCenter = readDicomDsNumber(value);
        else if (tagKey === "00281051")
            windowWidth = readDicomDsNumber(value);
...
function buildUnsupportedDicomResponse(metadata, message, nextAction) {
    return {
        status: "unsupported",
...
function buildDicomPreviewRgba(width, height, r, c, invert, sampleValue, renderCenter, renderWindow) {
...
function createDicomPixelSampler(buffer, pixelDataOffset, bytesPerPixel, bitsAllocated, pixelRepresentation, rescaleSlope, rescaleIntercept) {
    return (index) => {
...
function renderDicomPreviewImage(buffer, metadata, maxPreviewEdge) {
    const warnings = [...metadata.warnings];
    const { rows, columns, bitsAllocated, pixelRepresentation, rescaleIntercept, rescaleSlope, pixelDataOffset, windowCenter, windowWidth, photometricInterpretation, } = metadata;
    ...
    const sampleValue = createDicomPixelSampler(buffer, pixelDataOffset, bytesPerPixel, bitsAllocated, pixelRepresentation, rescaleSlope, rescaleIntercept);
    ...
    let window = windowWidth && windowWidth > 1
        ? windowWidth
    ...
    if (!windowCenter || !windowWidth)
        warnings.push("Окно яркости/контраста отсутствовало; предпросмотр использовал min/max окно по выборке.");
    return {
...
function parseDicomFirstFramePixel(buffer, maxPreviewEdge) {
    ...
    if (!dicomTransferSyntaxIsSupported(metadata.transferSyntaxUid) ||
        metadata.bigEndian) {
    ...
    if ((metadata.samplesPerPixel ?? 1) !== 1 ||
    ...
    if (metadata.pixelDataLength < expectedBytes ||
...
function locateLittleEndianPixelData(buffer) {
...
async function readDicomFirstFramePreviewBuffer(filePath, maxFileBytes) {
    const warnings = [];
    ...
    try {
        ...
        const metadata = parseDicomHeader(prefix.buffer);
        const estimatedFrameBytes = metadata.imageRows && metadata.imageColumns && metadata.bitsAllocated
            ? metadata.imageRows *
                metadata.imageColumns *
...
async function buildDicomFirstFramePreview(input, options = {}) {
    ...
    const preferredTargetIndex = typeof input.preferredFileIndex === "number"
        ? Math.min(files.length - 1, input.preferredFileIndex)
        : null;
    const candidateIndexes = preferredTargetIndex === null
        ? files.map((_, index) => index)
        : files
            .map((_, index) => index)
            .sort((left, right) => Math.abs(left - preferredTargetIndex) -
...
function dicomMetadataManifestRow(filePath, metadata, sourceName) {
    const fallback = inferManifestFieldsFromPath(filePath);
    const kind = modalityToKind(metadata.modality, `${metadata.studyDescription ?? ""} ${metadata.seriesDescription ?? ""}`) ??
        fallback.kind ??
        null;
    return [
...
async function readExactFileRange(fileHandle, position, length) {
...
async function readZipCentralDirectoryDetailed(filePath) {
    const warnings = [];
    let stats;
    try {
    ...
    while (cursor + 46 <= directoryBuffer.length &&
        entries.length < Math.min(totalEntries, zipEntryPreviewLimit)) {
        if (directoryBuffer.readUInt32LE(cursor) !== 0x02014b50)
            break;
        ...
        if (fileNameEnd > directoryBuffer.length)
            break;
        ...
        else if (localHeaderOffset + 30 > stats.size ||
    ...
    if (totalEntries > entries.length)
        warnings.push(`ZIP-предпросмотр вернул ${entries.length}/${totalEntries} записей центрального каталога.`);
    return { entries, warnings, fileHandle };
}
async function inflateZipEntryPrefix(fileHandle, entry, dataStart, maxHeaderBytes) {
    return new Promise((resolve) => {
        ...
        inflater.on("data", (chunk) => {
            ...
            if (remainingOutput > 0) {
                const slice = chunk.length > remainingOutput
...
async function zipEntryPrefix(fileHandle, entry, maxHeaderBytes) {
    if (entry.encrypted)
        return {
            buffer: null,
...
async function readZipCentralDirectory(filePath) {
    ...
    if (detailed.fileHandle !== null)
        await detailed.fileHandle.close();
    return {
        entries: detailed.entries.map((entry) => entry.name),
        warnings: detailed.warnings,
    };
}
async function expandDicomArchiveManifestLines(lines) {
    const expandedLines = [];
    const notes = [];
    for (const line of lines) {
        ...
        for (const entry of dicomEntries.slice(0, zipEntryPreviewLimit)) {
            ...
            expandedLines.push(filePath && line.includes(filePath)
...
function dicomFallbackSeriesKey(filePath, row) {
...
function estimateDicomSeriesMemoryMb(input) {
...
function buildMprResourcePolicy(input) {
    ...
    const requiredTier = !input.volumeCandidate
        ? "low_end"
        : input.fileCount <= 80
            ? "standard"
            : input.fileCount <= 220
    ...
    const loadStrategy = archiveVirtualSource
        ? "external_handoff"
        : !input.volumeCandidate
            ...
            : !input.canOpenMpr || hugeStack
                ...
                : input.fileCount > 180
                    ? "mpr_downsampled"
                    : "mpr_full";
    const maxClientSlices = requiredTier === "diagnostic_workstation"
        ? 450
        : requiredTier === "workstation"
            ? 300
            : 160;
    const cacheMode = dicomwebStream
        ? "dicomweb_stream"
    ...
    if (dicomwebStream)
        safetyCaps.push("Передавайте срезы через архив снимков с кешем; не копируйте полное исследование в состояние браузера.");
    if (archiveSource)
        safetyCaps.push("Распакуйте архивы в серверном или локальном обработчике до загрузки просмотра; не разбирайте большие ZIP в оболочке CRM.");
    if (archiveVirtualSource)
        safetyCaps.push("Записи внутри ZIP доступны как метаданные; для КТ-срезов нужен распакованный локальный набор или внешний просмотр.");
    if (hugeStack)
        safetyCaps.push("Для очень больших КЛКТ/КТ-стеков используйте внешний просмотр или отдельный обработчик объема.");
    if (!input.canBuildPanoramic && input.volumeCandidate)
        safetyCaps.push("Панорамная реконструкция отключена, пока не хватает срезов.");
    const nextAction = loadStrategy === "external_handoff"
...
function buildMprReadiness(input) {
    ...
    const volumeCandidate = input.kind === "cbct" ||
        modality === "CT" ||
        modality === "CBCT" ||
        modality === "MR";
    const archiveSource = isDicomArchivePath(input.firstFilePath);
    ...
    if (input.fileCount < minSliceCount)
        blockers.push(`Для просмотра КТ-срезов нужно минимум ${minSliceCount} срезов/файлов в этом предпросмотре.`);
    ...
    if (!input.hasStudySeriesUid)
        warnings.push("Идентификаторы исследования/серии отсутствуют; группировка по папке временная.");
    if (archiveVirtualSource)
        warnings.push("ZIP-серия остается в режиме метаданных и передачи до распаковки или подключения локального обработчика.");
    if (volumeCandidate && input.fileCount < 40)
        warnings.push("Панорамная реконструкция КЛКТ может потребовать более полного стека срезов.");
    ...
    const canOpenMpr = volumeCandidate &&
    ...
    const recommendedLayout = canOpenMpr
        ? input.fileCount >= 40
            ? "mpr_4up"
            : "mpr_3up"
        : archiveVirtualSource ||
            ...
            : input.fileCount > 1
                ? "two_d_stack"
                : "none";
    const panoramicProjections = canBuildPanoramic
        ? ["panoramic_reconstruction", "three_d_volume", "mip"]
        : [];
    const volumePlanningTools = canOpenMpr
        ? [
            "measurement",
            "measure_distance",
            "measure_angle",
    ...
    const panoramicTools = canBuildPanoramic
        ? ["panoramic_curve", "export_snapshot"]
        : [];
    const projections = canOpenMpr
...
function buildDicomSeriesGroups(rows) {
    const buckets = new Map();
    rows.forEach((row) => {
        const key = row.seriesInstanceUid ??
    ...
    return Array.from(buckets.values()).map((seriesRows, index) => {
        ...
        for (const row of seriesRows) {
            if (kind === undefined && row.kind)
                kind = row.kind;
            if (modality === undefined && row.modality)
                modality = row.modality;
            if (patientId === undefined && row.patientId)
            ...
            if (imageRows === undefined && row.imageRows)
                imageRows = row.imageRows;
        ...
        const estimatedPixelBytes = rowPixelBytes > 0
            ? rowPixelBytes
        ...
        if (!studyInstanceUidNull || !seriesInstanceUidNull)
            warnings.add("Нет кодов исследования/серии: серия сгруппирована по папке или описанию");
        if (!patientIdNull)
            warnings.add("Пациент не сопоставлен: перед записью нужен ручной матчинг");
        if (!kindNull)
            warnings.add("Тип исследования не распознан");
        if (kindNull === "cbct" && seriesRows.length < 8)
            warnings.add("Для КЛКТ/КТ-срезов мало срезов: проверьте полный экспорт серии");
        ...
        const status = blocked
            ...
            : patientIdNull && warnings.size === 0
                ? "ready"
                : "warning";
        const recommendedViewer = blocked
            ? "none"
...
async function parseDicomManifestLine(orgId, line, rowNumber, sourceKind, sourceName) {
    ...
    const instanceNumber = parseInstanceNumber(extractDicomFieldValue(line, [
        "InstanceNumber",
        "Instance Number",
        "Instance",
        "Slice",
    ...
    const imageRows = parsePositiveInteger(extractDicomFieldValue(line, [
        "Rows",
        "ImageRows",
        "Image Rows",
        "0028,0010",
    ...
    const imageColumns = parsePositiveInteger(extractDicomFieldValue(line, [
        "Columns",
        "ImageColumns",
        "Image Columns",
        "Cols",
    ...
    const bitsAllocated = parsePositiveInteger(extractDicomFieldValue(line, [
        "Bits Allocated",
        "BitDepth",
        "0028,0100",
        "\\(0028,0100\\)",
    ]));
    const samplesPerPixel = parsePositiveInteger(extractDicomFieldValue(line, [
        "SamplesPerPixel",
        "Samples Per Pixel",
        "Samples",
        "0028,0002",
    ...
    const estimatedPixelBytes = parsePositiveInteger(extractDicomFieldValue(line, [
        "EstimatedPixelBytes",
    ...
    const kind = base.kind ??
        modalityToKind(modality, `${line} ${studyDescription ?? ""} ${seriesDescription ?? ""}`);
    const warnings = [...base.warnings];
    if (!studyInstanceUid || !seriesInstanceUid)
        warnings.push("Коды исследования/серии не найдены, используем папку как временную группу");
...
export async function parseDicomSeriesManifest(orgId, input) {
    ...
    const rows = await Promise.all((hasHeader ? lines.slice(1) : lines).map(async (line, index) => {
        ...
        if (!filePath)
            warnings.push("Нет пути к снимку");
        ...
        return {
            ...
            estimatedPixelBytes: draft.estimatedPixelBytes ??
...
function safeJoinUrl(baseUrl, childPath) {
...
function buildQidoProbeUrl(input) {
 ...
 */
function dicomWebAuthHeaders(authMode) {
    const headers = {
        Accept: "application/dicom+json, application/json;q=0.9, */*;q=0.1",
    };
    const warnings = [];
    if (authMode === "bearer") {
        warnings.push(process.env.DICOMWEB_BEARER_TOKEN?.trim()
    ...
    if (authMode === "basic") {
        warnings.push(process.env.DICOMWEB_BASIC_AUTH?.trim()
...
function connectorStatusFromHttpStatus(httpStatus, fetchError) {
    if (fetchError)
        return "unreachable";
...
function parseIpv4Cidr(cidr, why) {
...
function parseIpv6Cidr(cidr, why) {
    const [prefix, bitsText] = cidr.split("/");
...
function ipv6InRange(bytes, range) {
    const fullBytes = range.bits >> 3;
    for (let index = 0; index < fullBytes; index += 1) {
        if (bytes[index] !== range.base[index])
            return false;
    }
...
function isSafeIpv6Bytes(bytes) {
    ...
    if (ipv6InRange(bytes, ipv4MappedIpv6Range))
    ...
    if (ipv6InRange(bytes, nat64WellKnownRange))
        return isSafeIpv4(embeddedIpv4);
    ...
    if (!ipv6InRange(bytes, globalUnicastIpv6Range))
        return false;
 ...
 */
async function isSafeTarget(urlString) {
    let url;
    try {
    ...
    if (addresses.some((entry) => !isSafeIp(entry.address))) {
        return {
            ok: false,
            reason: "адрес указывает на внутреннюю сеть, loopback или служебный диапазон",
        };
    }
    return { ok: true };
}
async function checkDicomWebConnector(input) {
    ...
    try {
        ...
        if (!safety.ok) {
            ...
            if (response.status >= 300 && response.status < 400) {
                fetchError = true;
                warnings.push("Безопасность: архив снимков ответил перенаправлением, а идти по нему запрещено — цель перенаправления не проходит проверку адреса. Укажите конечный адрес сервиса напрямую.");
            }
            else {
    ...
    if (status === "auth_required")
        warnings.push("Архив снимков ответил, но требует учетные данные или proxy-авторизацию.");
    if (status === "misconfigured")
        warnings.push("Архив снимков не вернул пригодный ответ поиска серий.");
    if (!input.studyInstanceUid || !input.seriesInstanceUid)
        warnings.push("Коды исследования/серии не переданы; готовность получения серии не подтверждена.");
...
function buildOhifViewerUrl(ohifBaseUrl, studyInstanceUid) {
...
function viewerDataSourceKind(input) {
    if (input.launchMode === "dicomweb_url" && input.dicomWebBaseUrl)
        return "dicomweb";
    if (input.launchMode === "local_manifest" &&
    ...
    if (input.launchMode === "local_manifest" && input.firstFilePath)
        return "local_files";
    if (input.launchMode === "external_handoff")
        return "external_viewer";
    return "none";
}
function buildDicomViewerLaunchManifest(input) {
    ...
    let launchMode = "blocked";
    let viewerUrl = null;
    if (!studyInstanceUid || !seriesInstanceUid)
        warnings.add("Идентификаторы исследования/серии отсутствуют; диагностический запуск требует стабильные идентификаторы.");
    ...
    else if (input.allowExternalHandoff &&
    ...
    const cornerstoneVolumeId = studyInstanceUid && seriesInstanceUid
    ...
    const stowRoot = input.dicomWebBaseUrl
        ? safeJoinUrl(input.dicomWebBaseUrl, "/studies")
        : null;
    const nextAction = launchMode === "dicomweb_url"
...
function cornerstoneVolumeIdForSeries(series) {
...
function stableViewerIdPart(value, fallback) {
    return ((value ?? fallback).replace(/[^a-zA-Z0-9._-]+/g, "-").slice(0, 96) ||
        fallback);
}
function targetToolForCrmTool(tool) {
    switch (tool) {
        case "pan":
            return "PanTool";
        case "zoom":
...
function targetToolForAnnotation(annotation) {
...
function buildToolConfigs(input) {
    const tools = [
        {
        ...
        },
        {
            ...
            shortcut: null,
            reason: "Дает контур площади на срезе: дефект, окно синус-лифтинга или ROI.",
        },
        {
            crmTool: "measure_volume",
            shortcut: null,
            reason: "Дает объемный ROI для пазухи, графта, дефекта или дыхательных путей.",
        },
        {
            crmTool: "note",
            shortcut: "N",
...
function buildDicomViewerViewports(input) {
    const series = input.series;
    ...
    return projections.map((projection, index) => ({
        ...
        referencedImageId: canReferenceLocalPixels && index === 0
...
function viewportForAnnotation(annotation, viewports) {
...
function buildToolStateAnnotation(annotation, viewports) {
    const viewport = viewportForAnnotation(annotation, viewports);
    const warnings = new Set();
    if (!viewport)
        warnings.add("Целевая область просмотра недоступна.");
    if (annotation.points.length === 0)
        warnings.add("В аннотации нет точек.");
    if ((annotation.type === "distance" || annotation.type === "angle") &&
    ...
    return {
        ...
        points: annotation.points.map((point, index) => ({
            ...
            canvas: [safeCoordinate(point.x), safeCoordinate(point.y)],
...
function planningTaskKindForQuickActionId(quickActionId) {
    if (quickActionId === "opg_curve")
        return "panoramic_reconstruction";
    if (quickActionId === "ridge_ruler")
        return "distance_measurement";
    if (quickActionId === "implant_axis")
        return "implant_axis";
    if (quickActionId === "area_roi")
        return "area_roi";
    if (quickActionId === "volume_roi")
        return "volume_roi";
    if (quickActionId === "implant_library")
        return "implant_library";
    if (quickActionId === "nerve_canal")
        return "nerve_canal";
    if (quickActionId === "density_probe")
        return "bone_density_probe";
    if (quickActionId === "surgical_guide")
...
function buildDicomViewerPlanningTasks(input) {
    const series = input.series;
    const viewerState = input.viewerState;
    ...
    return taskDefinitions.map((task) => {
        ...
        if ((task.kind === "implant_axis" || task.kind === "surgical_guide") &&
            !implantPlan) {
...
function buildDicomViewerToolStateBundle(input) {
    const series = input.series;
    const warnings = new Set(series.warnings);
    const volumeId = cornerstoneVolumeIdForSeries(series);
    ...
    const target = input.target === "ohif"
        ? "ohif"
        : input.target === "external_viewer" ||
            ...
            : input.target === "generic_json"
                ? "generic_json"
                : "cornerstone3d";
    const nextAction = target === "cornerstone3d"
...
function isRemoteDicomSource(series) {
    return series.sourceKind === "dicomweb" || series.sourceKind === "pacs";
}
function hasExplicitDicomDesktopBridge(client) {
...
function detectDicomClientRuntimeSurface(client) {
    ...
    if (/ipad|tablet/.test(text))
        return "tablet_web";
    if (/android|iphone|ipod|mobile|phone/.test(text))
        return "mobile_web";
    if (/win|mac|linux|x11|desktop|electron|tauri|neutralino|dental-crm-desktop|desktop app|desktop-app/.test(text))
        return "desktop_web";
    return "unknown";
}
function buildDicomClientRuntimeProfile(input) {
    ...
    const label = surface === "desktop_app"
    ...
    const nextAction = executionLane === "desktop_app_mpr"
...
function describeDicomExecutionLaneForOperator(lane) {
    if (lane === "desktop_app_mpr")
        return "настольный КТ-модуль";
    if (lane === "browser_mpr")
        return "КТ-срезы в браузере";
    if (lane === "browser_preview")
        return "легкий просмотр в браузере";
    if (lane === "metadata_only")
        return "только метаданные";
    return "внешний или локальный просмотр";
}
function detectWorkstationTier(input) {
    const memory = input.deviceMemoryGb ?? 0;
    const cores = input.hardwareConcurrency ?? 0;
    const freeStorageMb = input.storageQuotaMb !== null && input.storageUsageMb !== null
        ? Math.max(0, input.storageQuotaMb - input.storageUsageMb)
        : null;
    if (input.webgl2Supported &&
...
function estimateGpuMemoryMb(series) {
    const pixelMb = series.estimatedPixelBytes && series.estimatedPixelBytes > 0
...
function detectGpuClass(client) {
    ...
    if (discreteHint && memory >= 16 && cores >= 8 && max3d >= 2048)
        return "diagnostic";
    if ((discreteHint && max3d >= 1024) ||
        (memory >= 8 && cores >= 8 && max3d >= 1024))
        return "discrete_ok";
    if (memory >= 6 && cores >= 4 && max3d >= 512)
        return "integrated_ok";
    return "integrated_low";
}
function policyRatio(value, min, max) {
...
function roundedPolicyWeight(value) {
    return Math.round(clampNumber(value, 0, 1) * 100) / 100;
}
function freeClientStorageMb(client) {
...
function detectRenderMemoryBudgetClass(input) {
    ...
    if (runtimeProfile.mobileConstrained ||
        ...
        return "constrained";
    if (runtimeProfile.executionLane === "desktop_app_mpr" &&
        gpuClass === "diagnostic" &&
        memory >= 16 &&
        cores >= 8)
        return "diagnostic";
...
function buildDicomRenderHardwarePolicy(input) {
    ...
    const rawWeight = policyRatio(client.deviceMemoryGb, 2, 16) * 0.36 +
...
function diagnosticPixelPolicyFor(input) {
    ...
    if (input.runtimeProfile.executionLane === "browser_mpr" ||
...
function buildGpuRenderPlan(input) {
    ...
    const forceExternal = !forceMetadataOnly &&
        (gpuClass === "none" ||
            !client.indexedDbSupported ||
    ...
    if (gpuClass === "none")
        warnings.add("Графика браузера недоступна: КТ-срезы не могут работать в этом браузере.");
    if (!client.indexedDbSupported)
        warnings.add("Локальное хранилище браузера недоступно: восстановление просмотра не будет надежным.");
    if (sourceNeedsNetwork && !connectorReady)
        warnings.add("Архив снимков не готов, поэтому потоковая передача срезов недоступна.");
    if ((max3dTextureEdge ?? 0) > 0 && (max3dTextureEdge ?? 0) < 512)
        warnings.add("Браузер сообщает слишком маленький лимит для объемного просмотра.");
    ...
    const canSingleTexture = !forceMetadataOnly &&
        !forceExternal &&
        !runtimeProfile.mobileConstrained &&
        ...
        gpuClass !== "integrated_low";
    const shouldBrick = !forceMetadataOnly &&
        !forceExternal &&
        !runtimeProfile.mobileConstrained &&
        !canSingleTexture &&
        (max3dTextureEdge ?? 0) >= 512 &&
        series.fileCount <= series.mprReadiness.resourcePolicy.maxClientSlices;
    const textureStrategy = forceMetadataOnly
        ? "metadata_only"
        : forceExternal
            ? "external_viewer"
            : canSingleTexture
                ...
                : shouldBrick
                    ...
                    : runtimeProfile.mobileConstrained || series.fileCount > 1
                        ? "stack_2d_textures"
                        : "metadata_only";
    const qualityMode = textureStrategy === "external_viewer"
        ? "external"
        : textureStrategy === "metadata_only"
            ...
            : runtimeProfile.executionLane === "desktop_app_mpr" &&
                ...
                : runtimeProfile.mobileConstrained ||
                    ...
                    ? "interactive_low"
                    : "balanced_mpr";
    const downsampleFactor = qualityMode === "diagnostic_full"
        ? 1
        : qualityMode === "balanced_mpr"
            ...
            : qualityMode === "interactive_low"
                ...
                : runtimeProfile.mobileConstrained
                    ? 4
                    : 1;
    const rawTargetSliceBatch = textureStrategy === "external_viewer"
        ? 1
        : textureStrategy === "metadata_only"
            ? 1
    ...
    const useOffscreenCanvas = Boolean(client.offscreenCanvasSupported &&
    ...
    const interactionBudgetMs = qualityMode === "diagnostic_full"
        ? 12
        : qualityMode === "balanced_mpr"
            ? 16
            : 24;
    ...
    const firstPaintStrategy = textureStrategy === "external_viewer"
    ...
    const nextAction = qualityMode === "external"
...
function taskMemoryForRange(start, end, perSliceMb) {
...
function buildDicomRenderInteractionPhases(input) {
    ...
    const movementDownsample = renderPlan.qualityMode === "diagnostic_full"
        ? fileCount > 160
            ? 2
            : 1
    ...
    const idleDownsample = renderPlan.qualityMode === "interactive_low"
    ...
    const interactiveResidentSlices = renderPlan.textureStrategy === "single_3d_texture"
    ...
    return [
        ...
        },
        {
            ...
            targetFrameMs: renderPlan.qualityMode === "diagnostic_full"
                ? 12
                : renderPlan.qualityMode === "balanced_mpr"
                    ? 16
                    : 24,
...
function boundedSliceOrder(values, fileCount, maxItems = 96) {
    const seen = new Set();
    const result = [];
...
function interleavedSliceOrder(fileCount, decimationFactor, offset, maxItems = 128) {
    const values = [];
    for (let index = offset; index < fileCount; index += decimationFactor)
        values.push(index);
    return boundedSliceOrder(values, fileCount, maxItems);
}
function windowSliceOrder(start, end, activeSliceIndex, fileCount, maxItems = 128) {
...
function chooseDicomAdjacentWindow(input) {
    ...
    const preferredSide = rightEdgeDistance < leftEdgeDistance
        ? "after"
    ...
    candidates.sort((left, right) => {
        ...
        if (left.side === preferredSide && right.side !== preferredSide)
            return -1;
        if (right.side === preferredSide && left.side !== preferredSide)
            return 1;
...
function buildDicomProgressiveLoadStages(input) {
    ...
    const baseDecimation = renderPlan.qualityMode === "diagnostic_full"
        ? fileCount > 180
            ? 4
            : 2
        : renderPlan.qualityMode === "balanced_mpr"
    ...
    const stages = [
        ...
        progressiveStage({
            ...
            budgetMs: renderPlan.qualityMode === "diagnostic_full"
                ? 650
                : renderPlan.qualityMode === "balanced_mpr"
                    ? 520
                    : 360,
        ...
        progressiveStage({
            ...
            priority: "interactive",
            target: renderPlan.textureStrategy === "single_3d_texture"
                ? "gpu"
                : canUseWorker
                    ? "web_worker"
                    : "main_thread",
    ...
    stages.push(progressiveStage({
        ...
        kind: "idle_refine",
        label: "уточнение в паузе",
        priority: renderPlan.qualityMode === "interactive_low"
            ? "deferred"
            : "background",
        ...
        sliceEnd: firstWindowEnd,
        sliceOrder: activeOrder,
        decimationFactor: renderPlan.qualityMode === "interactive_low"
...
function buildDicomRenderCachePlan(input) {
    ...
    const firstBatch = clampNumber(Math.min(renderPlan.targetSliceBatch, renderPlan.progressiveSliceWindowCap), 1, Math.max(1, series.mprReadiness.resourcePolicy.maxClientSlices));
    ...
    const workerCount = !canUseWorker
        ...
        : renderPlan.qualityMode === "diagnostic_full"
            ...
            : renderPlan.qualityMode === "balanced_mpr"
                ? 2
                : 1;
    const decodeConcurrency = workerCount > 0
        ? Math.min(workerCount, renderPlan.qualityMode === "diagnostic_full" ? 3 : 2)
        : 1;
    const uploadConcurrency = renderPlan.textureStrategy === "single_3d_texture"
        ? 1
    ...
    const maxResidentSlices = renderPlan.textureStrategy === "single_3d_texture"
    ...
    const gpuMemoryBudgetMb = renderPlan.textureStrategy === "external_viewer"
        ? 0
        : Math.max(16, Math.min(renderPlan.estimatedGpuMemoryMb, Math.ceil(maxResidentSlices * perSliceMb * 1.4)));
    const shouldPersistToIndexedDb = series.mprReadiness.resourcePolicy.cacheMode === "bounded_disk" ||
    ...
    const firstPaintBudgetMs = renderPlan.qualityMode === "diagnostic_full"
        ? 1400
        : renderPlan.qualityMode === "balanced_mpr"
            ? 1000
            : renderPlan.qualityMode === "interactive_low"
    ...
    if (renderPlan.textureStrategy === "external_viewer") {
        ...
        tasks.push(renderTask({
            ...
            kind: renderPlan.textureStrategy === "single_3d_texture"
    ...
    if (renderPlan.qualityMode === "interactive_low")
        warnings.add("Режим слабой станции: держите первый показ в пониженном разрешении и повышайте качество только по явному запросу.");
    if (totalBatches > 8)
        warnings.add("Большой стек: нужны инкрементальные пакеты и видимый прогресс; экран приема блокировать нельзя.");
    const nextAction = renderPlan.textureStrategy === "external_viewer"
...
function buildBaseReadinessChecks(client, runtimeProfile, resourcePolicy, detectedTier, tierOk, freeStorageMb, series, connectorReady, connector) {
    const checks = [];
    checks.push(readinessCheck({
        id: "runtime",
        label: "Режим запуска",
        status: runtimeProfile.networkMode === "offline_remote_blocked"
            ? "fail"
            : runtimeProfile.mobileConstrained
                ? "warn"
                : "pass",
    ...
    checks.push(readinessCheck({
        ...
        detail: series.sourceKind === "dicomweb" || series.sourceKind === "pacs"
...
function buildMemoryPolicyCheck(renderPlan) {
    const memoryPolicyWarn = renderPlan.memoryBudgetClass === "minimum" ||
    ...
    return readinessCheck({
        ...
        nextAction: renderPlan.diagnosticPixelPolicy === "browser_preview_not_diagnostic"
...
function collectReadinessWarnings(client, series, runtimeProfile, tierOk, connectorReady, renderPlan) {
    const warnings = new Set();
    if (!client.online &&
    ...
    if (!tierOk)
        warnings.add("Текущая рабочая станция ниже рекомендованного класса для выбранной политики ресурсов КЛКТ.");
    if (!client.webgl2Supported)
        warnings.add("Для диагностического 3D-просмотра в браузере нужна поддержка современной браузерной графики.");
    if (!client.indexedDbSupported)
        warnings.add("Для восстановления просмотра нужно доступное локальное хранилище браузера.");
    if (!connectorReady)
...
function evaluateReadinessOutcome(client, series, resourcePolicy, runtimeProfile, renderPlan, checks, connectorReady, tierOk) {
    const failCount = checks.filter((check) => check.status === "fail").length;
    const warnCount = checks.filter((check) => check.status === "warn").length;
    const readinessScore = Math.max(0, Math.min(100, 100 - failCount * 30 - warnCount * 14));
    const shouldUseExternalViewer = renderPlan.textureStrategy === "external_viewer" ||
        renderPlan.textureStrategy === "metadata_only" ||
    ...
    const effectiveLoadStrategy = shouldUseExternalViewer
        ? "external_handoff"
        : !tierOk && resourcePolicy.loadStrategy === "mpr_full"
            ? "mpr_downsampled"
            : resourcePolicy.loadStrategy;
    const canOpenInBrowser = !shouldUseExternalViewer &&
        series.mprReadiness.canOpenMpr &&
...
function buildDicomWorkstationReadiness(input) {
    ...
    const detectedTier = runtimeProfile.mobileConstrained
        ? "low_end"
        : hardwareTier;
    const freeStorageMb = client.storageQuotaMb !== null && client.storageUsageMb !== null
    ...
    const connectorReady = series.sourceKind === "dicomweb" || series.sourceKind === "pacs"
        ? connector?.status === "ready"
        : true;
    const checks = buildBaseReadinessChecks(client, runtimeProfile, resourcePolicy, detectedTier, tierOk, freeStorageMb, series, connectorReady, connector);
    ...
    const warnings = collectReadinessWarnings(client, series, runtimeProfile, tierOk, connectorReady, renderPlan);
    const outcome = evaluateReadinessOutcome(client, series, resourcePolicy, runtimeProfile, renderPlan, checks, connectorReady, tierOk);
...
function buildDicomViewerWorkbenchManifest(input) {
...
function defaultDicomDiscoveryRoots() {
    const configured = process.env.DENTAL_DICOM_DISCOVERY_ROOTS?.split(/[;|]/)
        .map((root) => root.trim())
        .filter(Boolean) ?? [];
...
function fingerprintLocalPath(folderPath) {
    return createHash("sha256")
        .update(path.resolve(folderPath))
        .digest("hex")
        .slice(0, 10);
}
function classifyLocalImagingSource(root, folderPath, fromManualRoot) {
...
function safeLocalImagingAlias(prefix, folderPath) {
    return `${prefix} #${fingerprintLocalPath(folderPath).toUpperCase()}`;
}
function folderHintScore(folderPath) {
    const normalized = folderPath.toLowerCase();
    let score = 0;
    if (/dicom|dcm|cbct|ct|кт|ккт|opg|rvg|sidexis|romexis|pacs|study|series/.test(normalized))
        score += 0.16;
    if (/downloads|загруз/.test(normalized))
        score += 0.03;
    return score;
}
function discoveryDepth(root, folderPath) {
    const relative = path.relative(root, folderPath);
    if (!relative || relative === ".")
        return 0;
    return relative.split(path.sep).filter(Boolean).length;
}
function shouldSkipDicomDiscoveryDirectory(directoryName) {
    return dicomDiscoverySkipDirectoryNames.has(directoryName.toLowerCase());
}
async function discoverLocalDicomFolders(input, options = {}) {
    const fromManualRoot = Boolean(input.rootPaths?.length);
    const rawRoots = (input.rootPaths?.length ? input.rootPaths : defaultDicomDiscoveryRoots()).map((root) => path.resolve(root));
    const uniqueRoots = Array.from(new Set(rawRoots));
    const existsChecks = await Promise.all(uniqueRoots.map(async (root) => {
        try {
            await stat(root);
            return true;
        }
    ...
    while (queue.length && scannedFolders < input.maxFolders) {
        ...
        for (const entry of entries) {
            ...
            if (entry.isDirectory()) {
                if (shouldSkipDicomDiscoveryDirectory(entryName))
                    continue;
            ...
            const isImage = imagingFileExtensions.has(extension) &&
                !isArchive &&
                !dicomPixelFileExtensions.has(extension);
            const isDicomDir = /^DICOMDIR$/i.test(entryName);
            const isDicomFile = isDicomPixelPath(fullPath) ||
            ...
            if (isArchive && !firstFilePath)
                firstFilePath = fullPath;
            statPromises.push(stat(fullPath)
        ...
        const reasons = [];
        if (dicomLikeFiles)
            reasons.push(`${dicomLikeFiles} файлов снимков`);
        if (hasDicomDir)
            reasons.push("найден служебный каталог снимков");
        if (archivesFound)
            reasons.push(`${archivesFound} архивов`);
        if (folderHintScore(item.folderPath) > 0)
            reasons.push("имя папки похоже на стоматологический экспорт снимков");
        const confidence = Math.min(1, (dicomLikeFiles >= input.minDicomFiles
            ? 0.56
            : dicomLikeFiles > 0
                ? 0.28
                : 0) +
        ...
        const isCandidate = dicomLikeFiles >= input.minDicomFiles ||
            hasDicomDir ||
    ...
    if (queue.length)
        warnings.add(`Поиск остановлен на maxFolders=${input.maxFolders}. Сузьте корневые папки или увеличьте лимит.`);
    if (!roots.length)
        warnings.add("Нет доступных для чтения корневых папок поиска.");
    if (!candidates.length)
        warnings.add("В выбранных корневых папках не найдены папки, похожие на КТ/снимки.");
...
function detectDentalModelFormat(fileName) {
...
function detectDentalSurfaceModelRole(text) {
    ...
    if (/skull|cranium|cranial|череп/.test(text))
...
function detectDentalModelRole(fileName, folderPath) {
    const fromText = (text) => {
        ...
        if (/lower|mandible|mandibular|niz|low\b|l[ _-]?jaw/.test(text))
            return "lower_arch";
        if (/bite|occlusion|occlusal|prikus/.test(text))
            return "bite";
        if (/bridge|pontic|most/.test(text))
            return "bridge";
        if (/crown|koron|veneer|inlay|onlay/.test(text))
            return "crown";
        if (/aligner|eliner|kap+|cap+|tray/.test(text))
            return "aligner";
        if (/implant.*guide|guide.*implant|surgical.*guide|surg.*guide|pilot.*guide|implant/i.test(text))
...
function scoreDentalModelFile(fileName, folderPath) {
    ...
    if (role !== "unknown")
        score += 0.22;
    if (/intraoral|ios|scan|cad|cam|exocad|3shape|medit|cerec|mesh|model|stl|implant|guide|surface|segmentation/.test(text))
        score += 0.18;
    if (/upper|lower|maxilla|mandible|skull|bone|crown|bridge|aligner|bite|scanbody|scan body/.test(text))
        score += 0.1;
    return Math.min(1, Number(score.toFixed(2)));
}
function organizerFolderHintScore(folderPath) {
    const normalized = normalizeOrganizerText(folderPath);
    let score = folderHintScore(folderPath);
    if (/intraoral|ios|exocad|3shape|medit|cerec|implant|guide|aligner|scanbody|crown|bridge|maxilla|mandible|skull|bone|surface|segmentation|dental|tooth|teeth|orthodont|surgical/.test(normalized))
        score += 0.2;
    if (/patient|case|study|export|clinic|lab|laboratory/.test(normalized))
        score += 0.05;
    return Math.min(0.35, Number(score.toFixed(2)));
}
function isLikelySoftwareResourceFolder(folderPath) {
    const normalized = normalizeOrganizerText(folderPath);
...
function buildOrganizerCaseId(folderPath) {
...
function buildCtSurfaceModelManifest(input) {
    ...
    return {
        ...
        frameOfReferenceUid: null,
        registrationStatus: input.pairingHint === "same_folder_ct_series"
        ...
        warnings,
        nextAction: readiness === "pending_local_bridge"
...
function buildDentalModelWorkbenchManifest(input) {
    ...
    const items = input.modelCandidates.map((model) => {
        ...
        const pairingHint = input.dicomLikeFiles > 0 ? "same_folder_ct_series" : "model_only_folder";
        ...
        const nextAction = loadTarget === "local_bridge"
    ...
    const recommendedTarget = items.reduce((target, item) => targetRank[item.loadTarget] > targetRank[target]
        ? item.loadTarget
...
async function organizeLocalImagingSources(input, options = {}) {
    const fromManualRoot = Boolean(input.rootPaths?.length);
    ...
    for (let i = 0; i < uniqueRoots.length; i += BATCH_SIZE) {
        ...
        const results = await Promise.all(batch.map(async (root) => {
            try {
                await access(root);
                return root;
            }
    ...
    while (queue.length && scannedFolders < input.maxFolders) {
        ...
        for (const entry of entries) {
            ...
            const isModelArchive = extension === ".zip" &&
                hasDentalModelArchiveHint(entryName, item.folderPath);
            const shouldProbeDicomMagic = input.includeDicom &&
                !isArchive &&
                !isImage &&
                !hasModelExtension &&
                (folderHasDicomHint ||
            ...
            const isDicomFile = input.includeDicom &&
        ...
        const dicomConfidence = input.includeDicom && (dicomLikeFiles > 0 || archiveFiles > 0)
            ? Math.min(1, (dicomLikeFiles >= 2 ? 0.58 : dicomLikeFiles > 0 ? 0.32 : 0) +
                (archiveFiles > 0 ? 0.12 : 0) +
                folderScore)
            : 0;
        const modelConfidence = input.includeDentalModels && modelFiles > 0
            ? Math.min(1, (modelFiles >= 2 ? 0.55 : modelFiles > 0 ? 0.36 : 0) +
                Math.min(0.25, modelCandidates.reduce((sum, item) => sum + item.confidence, 0) / 6) +
                folderScore)
            : 0;
        const combinedConfidence = Math.min(1, Math.max(dicomConfidence, modelConfidence) +
            (dicomLikeFiles > 0 && modelFiles > 0 ? 0.12 : 0));
        const candidateLooksUseful = dicomLikeFiles > 0 ||
            modelFiles > 0 ||
            (archiveFiles > 0 && combinedConfidence >= 0.35) ||
            (imageFiles >= 8 && combinedConfidence >= 0.35);
        if (!candidateLooksUseful)
            continue;
        if (dicomLikeFiles === 0 &&
            archiveFiles === 0 &&
            modelFiles > 0 &&
        ...
        if (archiveFiles)
            reasons.push(`${archiveFiles} архивных файлов`);
        if (imageFiles >= 8)
            reasons.push(`${imageFiles} файлов изображений`);
        if (folderScore > 0)
    ...
    if (queue.length)
        warnings.add(`Органайзер остановлен на maxFolders=${input.maxFolders}. Сузьте корни или увеличьте лимит.`);
    if (!roots.length)
        warnings.add("Нет доступных для чтения корневых папок органайзера.");
    const sortedCases = cases
        .sort((left, right) => right.combinedConfidence - left.combinedConfidence ||
        right.dicomLikeFiles - left.dicomLikeFiles ||
        right.modelFiles - left.modelFiles ||
        (right.latestModifiedAt ?? "").localeCompare(left.latestModifiedAt ?? ""))
        .slice(0, input.maxCandidates);
    if (!sortedCases.length)
        warnings.add("В выбранных корнях не найдены кандидаты КТ/снимков или стоматологических 3D-моделей.");
...
function recommendDicomFolderWorkupPath(readiness, series) {
    if (readiness.renderPlan.textureStrategy === "metadata_only" ||
        readiness.runtimeProfile.executionLane === "metadata_only") {
        return "metadata_only";
    }
    if (readiness.canOpenInBrowser &&
    ...
    if (readiness.canOpenInBrowser && series.mprReadiness.canOpenMpr)
        return "open_mpr";
    if (readiness.shouldUseExternalViewer)
        return "external_viewer";
    return "metadata_only";
}
function nextDicomFolderAction(pathKind) {
    switch (pathKind) {
        case "open_mpr":
...
async function buildDicomFolderWorkupPlan(input, options = {}, organizationId = "") {
    ...
    const bestPlan = plans.find((plan) => plan.recommendedPath === "open_mpr") ??
...
export async function registerImagingRoutes(app) {
    ...
    app.post("/api/imaging/imports/preview", async (request, reply) => {
        if (!(await requireClinicalReadAccess(request, reply, "imaging import preview")))
            return;
        const parsed = parseImagingPayload(imagingImportPreviewRequestSchema, request.body, "Предпросмотр снимков не построен: передайте непустой текст или таблицу источника снимков.");
        if (!parsed.ok)
    ...
    app.post("/api/imaging/dicom/series-preview", async (request, reply) => {
        if (!(await requireClinicalReadAccess(request, reply, "dicom series preview")))
            return;
        const parsed = parseImagingPayload(dicomSeriesPreviewRequestSchema, request.body, "Предпросмотр DICOM-серии не построен: передайте непустой список метаданных серии.");
        if (!parsed.ok)
            return reply.code(400).send(parsed.response);
        const input = parsed.data;
        // БЫЛО: getDefaultOrganizationId() — «первая строка таблицы organizations»,
    ...
    app.post("/api/imaging/dicomweb/check", async (request, reply) => {
        if (!(await requireDicomWebSettingsAccess(request, reply)))
            return;
        const parsed = parseImagingPayload(dicomWebConnectorCheckRequestSchema, request.body, "Проверка DICOMweb не выполнена: передайте корректный адрес сервиса и параметры доступа.");
        if (!parsed.ok)
            return reply.code(400).send(parsed.response);
        const input = parsed.data;
    ...
    app.post("/api/imaging/dicom/viewer-launch-manifest", async (request, reply) => {
        if (!(await requireClinicalReadAccess(request, reply, "dicom viewer launch manifest")))
            return;
        const parsed = parseImagingPayload(dicomViewerLaunchManifestRequestSchema, request.body, "Пакет открытия просмотра не построен: передайте выбранную серию и состояние просмотра.");
        if (!parsed.ok)
            return reply.code(400).send(parsed.response);
        const input = parsed.data;
    ...
    app.post("/api/imaging/dicom/viewer-tool-state", async (request, reply) => {
        if (!(await requireClinicalReadAccess(request, reply, "dicom viewer tool state")))
            return;
        const parsed = parseImagingPayload(dicomViewerToolStateBundleRequestSchema, request.body, "Пакет инструментов просмотра не построен: передайте выбранную серию, состояние и разметку.");
        if (!parsed.ok)
            return reply.code(400).send(parsed.response);
        const input = parsed.data;
    ...
    app.post("/api/imaging/dicom/render-cache-plan", async (request, reply) => {
        if (!(await requireClinicalReadAccess(request, reply, "dicom render cache plan")))
            return;
        const parsed = parseImagingPayload(dicomRenderCachePlanRequestSchema, request.body, "План кэша просмотра не построен: передайте серию и план мощности устройства.");
        if (!parsed.ok)
            return reply.code(400).send(parsed.response);
        const input = parsed.data;
    ...
    app.post("/api/imaging/dicom/workstation-readiness", async (request, reply) => {
        if (!(await requireClinicalReadAccess(request, reply, "dicom workstation readiness")))
            return;
        const parsed = parseImagingPayload(dicomWorkstationReadinessRequestSchema, request.body, "Проверка готовности рабочего места не выполнена: передайте серию и сведения об устройстве.");
        if (!parsed.ok)
            return reply.code(400).send(parsed.response);
        const input = parsed.data;
    ...
    app.post("/api/imaging/dicom/viewer-workbench-manifest", async (request, reply) => {
        if (!(await requireClinicalReadAccess(request, reply, "dicom viewer workbench manifest")))
            return;
        const parsed = parseImagingPayload(dicomViewerWorkbenchManifestRequestSchema, request.body, "Рабочий пакет просмотра не построен: передайте серию, устройство и состояние просмотра.");
        if (!parsed.ok)
            return reply.code(400).send(parsed.response);
        const input = parsed.data;
    ...
    app.post("/api/imaging/dicom/workbench-bundles", async (request, reply) => {
        if (!(await requireClinicalMutationAccess(request, reply, "dicom workbench bundle save")))
            return;
        const parsed = parseImagingPayload(saveDicomWorkbenchBundleRequestSchema, request.body, "Набор просмотра не сохранен: передайте сформированный рабочий пакет просмотра.");
        if (!parsed.ok)
            return reply.code(400).send(parsed.response);
        const input = parsed.data;
        // БЫЛО: getDefaultOrganizationId() — «первая строка таблицы organizations»,
        ...
        return reply.code(201).send(dicomWorkbenchBundleResponseSchema.parse({
            bundle,
            warnings: bundle.warnings,
        }));
    });
    app.get("/api/imaging/dicom/workbench-bundles", async (request, reply) => {
        if (!(await requireClinicalReadAccess(request, reply, "dicom workbench bundles")))
            return;
    ...
    app.post("/api/imaging/dicom/local-folder-discovery", async (request, reply) => {
        if (!(await requireClinicalReadAccess(request, reply, "dicom local folder discovery")))
            return;
        const parsed = parseImagingPayload(dicomLocalFolderDiscoveryRequestSchema, request.body, "Поиск папок снимков не запущен: проверьте корни поиска и лимиты обхода.");
        if (!parsed.ok)
            return reply.code(400).send(parsed.response);
        const input = parsed.data;
    ...
    app.post("/api/imaging/local-organizer/scan-preview", async (request, reply) => {
        if (!(await requireClinicalReadAccess(request, reply, "local imaging organizer preview")))
            return;
        const parsed = parseImagingPayload(localImagingOrganizerRequestSchema, request.body, "Разбор локальных снимков не запущен: проверьте корни поиска и лимиты обхода.");
        if (!parsed.ok)
            return reply.code(400).send(parsed.response);
        const input = parsed.data;
    ...
    app.post("/api/imaging/dicom/folder-series-preview", async (request, reply) => {
        if (!(await requireClinicalReadAccess(request, reply, "dicom folder series preview")))
            return;
    ...
    app.post("/api/imaging/dicom/first-frame-preview", async (request, reply) => {
        if (!(await requireClinicalReadAccess(request, reply, "dicom first frame preview")))
            return;
        const parsed = parseImagingPayload(dicomFirstFramePreviewRequestSchema, request.body, "Первый кадр DICOM не построен: выберите папку снимков и безопасные лимиты чтения.");
        if (!parsed.ok)
            return reply.code(400).send(parsed.response);
        const input = parsed.data;
    ...
    app.post("/api/imaging/dicom/folder-workup-plan", async (request, reply) => {
        if (!(await requireClinicalReadAccess(request, reply, "dicom folder workup plan")))
            return;
    ...
    app.post("/api/imaging/imports/commit", async (request, reply) => {
        if (!(await requireClinicalMutationAccess(request, reply, "imaging import commit")))
            return;
        const parsed = parseImagingPayload(imagingImportPreviewRequestSchema, request.body, "Импорт снимков не выполнен: повторно передайте ту же непустую выгрузку перед записью.");
        if (!parsed.ok)
            return reply.code(400).send(parsed.response);
        const input = parsed.data;
        // БЫЛО: getDefaultOrganizationId() — «первая строка таблицы organizations»,
    ...
    app.post("/api/imaging/folders/scan-preview", async (request, reply) => {
        if (!(await requireClinicalReadAccess(request, reply, "imaging folder scan preview")))
            return;
        const parsed = parseImagingPayload(imagingFolderScanRequestSchema, request.body, "Сканирование папки снимков не запущено: выберите папку и безопасные лимиты чтения.");
        if (!parsed.ok)
            return reply.code(400).send(parsed.response);
        const input = parsed.data;
    ...
    app.put("/api/imaging/studies/:id/viewer-session", async (request, reply) => {
        ...
        if (!study)
            return sendImagingStudyNotFound(reply);
        const parsed = parseImagingPayload(saveImagingViewerSessionRequestSchema, request.body, "Сеанс просмотра снимка не сохранен: передайте состояние просмотра и разметку.");
        if (!parsed.ok)
            return reply.code(400).send(parsed.response);
        const input = parsed.data;
        const session = await saveImagingViewerSession(orgId, id, input);
        return reply.code(200).send(imagingViewerSessionResponseSchema.parse({
            session,
            warnings: session.warnings,
        }));
    });
