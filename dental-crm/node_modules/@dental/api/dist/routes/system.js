...
function healthUrl(rawUrl, defaultHealthPath, deriveHealthFromConfiguredPath = false) {
    ...
    else if (deriveHealthFromConfiguredPath &&
...
function buildVisitDictationPlan(readiness) {
    ...
    return {
        ...
        steps: [
            ...
            planStep(2, localBridge
                ? `Использовать ${localBridge.title}`
                : serverSttAvailable
                    ? `Использовать ${speech.providerLabel}`
                    : "Оставить аудио и текст в очереди", "system", primaryPath, true, false, localBridge
        ...
        steps: [
            ...
            planStep(2, ocr
                ...
                : groqReady
                    ...
                    : "Отметить, что нужен OCR", "system", primaryPath, true, false, ocr
...
function buildPricePhotoPlan(readiness) {
    ...
    return {
        ...
        steps: [
            ...
            planStep(2, groqReady
                ? "Классифицировать через серверное распознавание"
                : ocr
                    ? "Считать текст локальным OCR"
                    : "Использовать детерминированный разбор таблицы", "system", primaryPath, true, false, groqReady
    ...
    return {
        ...
        steps: [
            ...
            planStep(2, dicom
                ...
                : "Использовать встроенный разбор метаданных", "system", primaryPath, true, false, dicom
            ...
            planStep(3, ocr
                ...
                : "OCR этикеток остается необязательным", "system", ocr ? "local_bridge" : "manual_review", true, false, ocr
...
export async function registerSystemRoutes(app) {
    ...
    app.post("/api/system/sync/push", async (request, reply) => {
        ...
        if (payload.organizationId && payload.organizationId !== organizationId) {
            return reply.code(403).send({
                error: "TenantViolation",
                message: "Cross-organization modification rejected.",
            });
        }
        ...
        if (!allowedSyncTables.includes(table)) {
            return reply.code(403).send({
                ...
                message: `Таблица ${table} недоступна для синхронизации.`,
            });
        }
    ...
    app.post("/api/system/analyze-legacy-db", async (request, reply) => {
        ...
        if (!organizationId)
            return reply.code(403).send({ error: "OrganizationRequired" });
        const { fileName, fileBase64 } = request.body;
        let fileText = "";
        if (fileBase64) {
            try {
