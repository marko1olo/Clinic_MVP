...
describe('sendVisitDraftMutationError', () => {
  test('handles "Визит не найден" error', () => {
    ...
    assert.strictEqual((codeMock.mock.calls[0] as any).arguments[0], 404);
    assert.deepStrictEqual((sendMock.mock.calls[0] as any).arguments[0], {
      error: 'VisitNotFound',
  ...
  test('handles "Прием уже закрыт или аннулирован" error for autosave', () => {
    ...
    assert.strictEqual((codeMock.mock.calls[0] as any).arguments[0], 409);
    assert.deepStrictEqual((sendMock.mock.calls[0] as any).arguments[0], {
