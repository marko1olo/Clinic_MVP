...
>>>>>>> gitlab/main
    ...
    @patch('ShadowAnalyst.watcher.prepare_image')
    @patch('ShadowAnalyst.watcher.OpenAI')
    def test_analyze_image_api_error_path(self, mock_openai_class, mock_prepare_image):
        mock_prepare_image.return_value = "data:image/jpeg;base64,dummybase64"

        mock_client = MagicMock()
        mock_openai_class.return_value = mock_client

        # API always throws an exception
        mock_client.chat.completions.create.side_effect = Exception("Custom API Error")

        with patch('ShadowAnalyst.watcher.GOOGLE_API_KEYS', ['gkey1']),              patch('ShadowAnalyst.watcher.GROQ_API_KEYS', ['groqkey1']):
            marked_path, report = watcher.analyze_image("dummy.jpg")

        self.assertIsNone(marked_path)
        self.assertIn("Custom API Error", report)
