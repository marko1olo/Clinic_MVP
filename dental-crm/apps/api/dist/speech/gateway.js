...
function isHallucinatedTranscript(text) {
    ...
    for (const entry of HALLUCINATION_BLACKLIST) {
        if (typeof entry === "string") {
            ...
            const isDominant = normalized.startsWith(normalizedEntry) &&
...
function redactedBridgeUrl(value) {
    try {
        const url = new URL(value);
        url.username = "";
        url.password = "";
...
function localWhisperTranscribeUrlFromBase(value) {
    const url = new URL(value);
...
function localSpeechBridgeProbeState(providerId) {
...
async function runLocalSpeechBridgeProbe(providerId) {
    ...
    try {
        ...
        state.checkedAt = Date.now();
        state.latencyMs = null;
        const probeReason = error instanceof Error && error.name === "AbortError"
...
function primeLocalSpeechBridgeProbe(providerId) {
    const state = localSpeechBridgeProbeState(providerId);
    if (!isLocalSpeechProvider(providerId))
        return state;
    if (!providerConfigReady(providerId))
        return state;
    if (state.pending || localSpeechBridgeProbeFresh(state))
        return state;
...
function localSpeechBridgeReady(providerId) {
    const state = primeLocalSpeechBridgeProbe(providerId);
    return state.status === "ready";
}
function localSpeechBridgeProbeWarning(providerId) {
    const state = primeLocalSpeechBridgeProbe(providerId);
    if (!providerConfigReady(providerId))
        return null;
...
function providerConfigMissingEnvVars(providerId) {
...
function anyProviderTranscriptionCurrentlyAvailable(providerIds) {
    return providerIds.some((providerId) => providerTranscriptionCurrentlyAvailable(providerId));
}
function configuredWiredProviders() {
...
export function getSpeechGatewayStatus() {
    ...
    const warnings = [...resolvedProvider.warnings];
    ...
    if (providerId === "none") {
        ...
        if (localProbeWarning)
            warnings.push(localProbeWarning);
    }
...
export function getSpeechProviderRuntimeStatuses() {
    ...
    return speechProviders.map((provider) => {
        ...
        const nextStep = provider.status === "usable_without_key"
...
export function getSpeechGatewayHealthReport() {
    const gateway = getSpeechGatewayStatus();
...
function decodeBase64Audio(value, maxChunkBytes) {
    if (!value?.trim())
        return Buffer.alloc(0);
    if (!/^[A-Za-z0-9+/=]+$/.test(value)) {
        throw new SpeechChunkPayloadError("Аудиофрагмент поврежден или передан не как файл записи. Повторите запись либо оставьте текстовый черновик.");
    }
    const buffer = Buffer.from(value, "base64");
    if (buffer.byteLength > maxChunkBytes) {
        throw new SpeechChunkPayloadError(`Аудиофрагмент слишком большой для текущих настроек (${Math.ceil(buffer.byteLength / 1024 / 1024)} МБ из ${Math.ceil(maxChunkBytes / 1024 / 1024)} МБ). Запишите короче или дождитесь отправки очереди.`);
    }
    return buffer;
}
function publicProviderFailureReason(error) {
...
function confidenceFromWhisperLogprob(values) {
...
async function transcribeOpenAiCompatible(input) {
    ...
    if (compressedSegments) {
        segmentWarnings.push(`${compressedSegments} фрагмент(ов) похожи на сжатый или повторяющийся звук; проверьте текст перед сохранением.`);
    }
    return {
...
function textFromVoskPayload(payload) {
    ...
    if (Array.isArray(alternatives)) {
        const alternative = alternatives.find((item) => item && typeof item === "object");
        if (alternative)
            return stringFromRecord(alternative, ["text", "transcript"]);
    }
    return "";
}
function confidenceFromVoskPayload(payload) {
...
async function transcribeLocalWhisperBridge(input) {
    ...
    if (!endpoint) {
        throw new Error("Локальный модуль Whisper.cpp не настроен: укажите адрес локального модуля в серверных настройках клиники.");
    }
...
async function transcribeLocalVoskBridge(input) {
    ...
    if (input.specialty)
        form.append("specialty", input.specialty);
    const headers = {};
...
async function transcribeCloudflareWhisper(input) {
    ...
    if (!accountId) {
        throw new Error("Cloudflare Workers AI Whisper не настроен полностью: заполните недостающий пункт в серверных настройках распознавания.");
    }
...
async function transcribeWithProvider(input) {
    ...
    if (!providerKeyCount(input.providerId)) {
        throw new Error(`Для ${providerLabels[input.providerId]} не настроен серверный доступ. До подключения врач может использовать локальный текст или браузерную диктовку.`);
    }
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
        ...
        try {
            ...
            if (input.providerId === "groq_whisper") {
                ...
                throw new Error(`${providerLabels[input.providerId]} есть в каталоге, но прямое серверное распознавание пока не включено. Выберите подключенный источник или браузерную диктовку.`);
            }
    ...
    if (lastError instanceof SpeechAsyncJobTimeoutError ||
...
export async function transcribeSpeechChunk(input) {
    const gateway = getSpeechGatewayStatus();
    ...
    let usedProviderId = gateway.providerId;
    ...
    if (!audio.byteLength && localTranscript) {
        ...
        if (!transcript)
            warnings.push("Аудио принято, но серверное распознавание не запущено без доступного источника.");
    }
