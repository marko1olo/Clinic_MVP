...
export function AcceptInvite({ token, onSuccess, onCancel }: AcceptInviteProps) {
      ...
      <div className="auth-modal animate-fade-in-up" style={{ maxWidth: '400px' }}>
        ...
        <form onSubmit={handleSubmit} className="auth-form">
          <div className="auth-form-group">
            <label className="auth-label"><User size={12} className="auth-icon-inline" /> ФИО</label>
            <input type="text" value={fullName} onChange={e => setFullName(e.target.value)} placeholder="Иванов И.И." className="auth-input" disabled={loading} />
          </div>
          <div className="auth-form-group">
            <label className="auth-label"><KeyRound size={12} className="auth-icon-inline" /> Придумайте пароль (для входа из дома)</label>
          ...
          <div className="auth-form-group">
            <label className="auth-label"><KeyRound size={12} className="auth-icon-inline" /> Придумайте PIN-код (4 цифры для работы в клинике)</label>
            <input type="text" maxLength={4} value={pinCode} onChange={e => setPinCode(e.target.value.replace(/\D/g, ''))} placeholder="1234" className="auth-input" disabled={loading} style={{ letterSpacing: '8px', textAlign: 'center', fontSize: '18px' }} />
          </div>

          <button type="submit" disabled={loading} className="auth-submit-btn">
            {loading ? <div className="auth-spinner"></div> : <>Активировать аккаунт <ArrowRight size={16} /></>}
          </button>
        </form>

        <div className="auth-footer-hints auth-footer-hints--border">
          <button type="button" onClick={onCancel} className="auth-link-btn">Вернуться ко входу</button>
        </div>
      </div>
    </div>
  );
