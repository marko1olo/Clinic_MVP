...
export const OdontogramModule = ({ patientId, pediatricMode }: { patientId: string, pediatricMode?: boolean | undefined }) => {
  ...
  const updateToothState = async (toothNumbers: number[], state: ToothState) => {
    setTeethData(prev => {
      let next = [...prev];
  ...
  const handleToothClick = (toothNumber: number, rect: DOMRect) => {
    if (isMultiSelectMode) {
      ...
      const isUpperJaw = toothNumber < 30 || (toothNumber >= 51 && toothNumber <= 65);
      const menuW = 254;
      const menuH = 224;
      const gap = 12;
  ...
  return (
    <div className="flex flex-col lg:flex-row items-start gap-6 w-full h-full p-6 bg-zinc-50/40 dark:bg-zinc-950/40 backdrop-blur-md border border-zinc-200/50 dark:border-zinc-800/50 rounded-2xl shadow-2xl text-slate-900 dark:text-zinc-100">
      <div className="flex-2 min-w-0 flex flex-col gap-6 relative w-full" ref={containerRef}>
        <div className="flex gap-4 p-4 items-center bg-zinc-100/30 dark:bg-zinc-900/30 border-b border-zinc-200/50 dark:border-zinc-800/50 rounded-t-xl">
          <label className="flex items-center gap-2 cursor-pointer select-none">
            <input type="checkbox" checked={isPediatricMode} onChange={e => setIsPediatricMode(e.target.checked)} className="accent-indigo-500" />
            <span className="text-sm font-medium">Детский прикус</span>
          </label>
          <label className={`flex items-center gap-2 cursor-pointer select-none ${isMultiSelectMode ? 'text-indigo-600 dark:text-indigo-400' : ''}`}>
            <input type="checkbox" checked={isMultiSelectMode} onChange={e => { setIsMultiSelectMode(e.target.checked); if (!e.target.checked && selectedTeeth.length === 0) setMenuConfig(null); }} className="accent-indigo-500" />
          ...
          <>
            ...
            >
              ...
              {menuConfig.position === 'bottom' ? (
                <svg className="absolute -top-3 text-zinc-800/50 drop-shadow-md" style={{ left: `${menuConfig.caretOffset}%`, transform: 'translateX(-50%)' }} width="24" height="12" viewBox="0 0 24 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M12 0L24 12H0L12 0Z" fill="currentColor" fillOpacity="0.8" />
                </svg>
              ) : (
                <svg className="absolute -bottom-3 text-zinc-800/50 drop-shadow-md" style={{ left: `${menuConfig.caretOffset}%`, transform: 'translateX(-50%)' }} width="24" height="12" viewBox="0 0 24 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M12 12L24 0H0L12 12Z" fill="currentColor" fillOpacity="0.8" />
                </svg>
              )}
              
              <div className="col-span-2 text-center mb-2 text-sm font-bold text-zinc-100">
                {selectedTeeth.length > 1 ? `Выбрано: ${selectedTeeth.length} зубов` : `Зуб ${menuConfig.toothNumber}`}
              </div>
              <button onClick={() => updateToothState(selectedTeeth, 'Caries')} className="flex items-center justify-center p-3 rounded-xl border transition-all duration-200 font-medium tracking-wide text-xs bg-red-500/10 text-red-400 border-red-500/20 hover:bg-red-500/20">
                Кариес
              </button>
              <button onClick={() => updateToothState(selectedTeeth, 'Pulpitis')} className="flex items-center justify-center p-3 rounded-xl border transition-all duration-200 font-medium tracking-wide text-xs bg-amber-500/10 text-amber-400 border-amber-500/20 hover:bg-amber-500/20">
                Пульпит
              </button>
              <button onClick={() => updateToothState(selectedTeeth, 'Missing')} className="flex items-center justify-center p-3 rounded-xl border transition-all duration-200 font-medium tracking-wide text-xs bg-zinc-800/40 text-zinc-400 border-zinc-700/30 hover:bg-zinc-800/60">
                Отсутствует
              </button>
              <button onClick={() => updateToothState(selectedTeeth, 'Crown')} className="flex items-center justify-center p-3 rounded-xl border transition-all duration-200 font-medium tracking-wide text-xs bg-blue-500/10 text-blue-400 border-blue-500/20 hover:bg-blue-500/20">
                Коронка
              </button>
              <button onClick={() => updateToothState(selectedTeeth, 'Implant')} className="flex items-center justify-center p-3 rounded-xl border transition-all duration-200 font-medium tracking-wide text-xs bg-yellow-500/10 text-yellow-400 border-yellow-500/20 hover:bg-yellow-500/20">
                Имплантат
              </button>
              <button onClick={() => updateToothState(selectedTeeth, 'Healthy')} className="flex items-center justify-center p-3 rounded-xl border transition-all duration-200 font-medium tracking-wide text-xs bg-emerald-500/10 text-emerald-400 border-emerald-500/20 hover:bg-emerald-500/20">
                Здоров
              </button>
              <button 
