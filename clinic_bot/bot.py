...
async def cmd_start(message: Message):
    ...
    if not role:
        db.add_user(
            chat_id, "doctor", message.from_user.full_name
        )  # По умолчанию врач для простоты
        role = "doctor"
...
async def _send_message_safe(chat_id: int, text: str):
    try:
...
async def broadcast(text: str, role: str = "admin"):
    ...
    tasks = [_send_message_safe(chat_id, text) for chat_id in users]
    await asyncio.gather(*tasks)


async def _send_photo_safe(
    chat_id: int, photo_bytes: bytes, caption: str, report_text: str
):
    try:
...
async def broadcast_photo(
    ...
    tasks = [
        _send_photo_safe(chat_id, photo_bytes, caption, report_text)
