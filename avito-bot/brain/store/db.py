...
class Store:
    ...
    def patient_texts_batch(
        self, chat_ids: list[str], limit: int = 20
    ) -> dict[str, tuple[str, ...]]:
        """Массово вытягивает последние реплики пациента для дожима.

        Устраняет N+1 проблему при выборке patient_texts для списка диалогов.
        """
        if not chat_ids:
            return {}
        ...
        query = f"""
            SELECT chat_id, text
            FROM inbox
            WHERE chat_id IN ({placeholders}) AND outgoing = 0
            ORDER BY chat_id, position DESC, rowid DESC
        """

        rows = self._conn.execute(query, chat_ids).fetchall()

        result = {c: [] for c in chat_ids}
        for row in rows:
            chat_id = row["chat_id"]
            if len(result[chat_id]) < limit:
                result[chat_id].append(row["text"])
