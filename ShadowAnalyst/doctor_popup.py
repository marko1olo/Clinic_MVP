    ...
    try:
        with open(CONFIG_FILE, encoding='utf-8') as f:
            ...
            TOPIC_XRAY_RESULT = config.get(
                'mqtt_topic_xray', TOPIC_XRAY_RESULT)
...
class DoctorPopupApp:
    def __init__(self):
        ...
        self.style.configure(
            'Outer.TFrame', background='#38bdf8')  # Accent border
        self.style.configure('Popup.TFrame', background='#0a0e17')  # Main bg
        self.style.configure('Title.TLabel', background='#0a0e17',
                             foreground='#38bdf8', font=('Segoe UI', 11, 'bold'))
        self.style.configure('Body.TLabel', background='#0a0e17',
                             foreground='#e2e8f0', font=('Segoe UI', 9))
        self.style.configure('Alert.TLabel', background='#0a0e17',
                             foreground='#ef4444', font=('Segoe UI', 9, 'bold'))

        # Outer frame to act as 1px border
        self.outer_frame = ttk.Frame(
            self.root, style='Outer.TFrame', padding=1)
        self.outer_frame.pack(fill=tk.BOTH, expand=True)

        self.frame = ttk.Frame(
            self.outer_frame, style='Popup.TFrame', padding=15)
        self.frame.pack(fill=tk.BOTH, expand=True)

        self.lbl_title = ttk.Label(
            self.frame, text='🦷 Анализ снимка', style='Title.TLabel')
        self.lbl_title.pack(anchor=tk.W, pady=(0, 5))

        self.lbl_body = ttk.Label(
            self.frame, text='', style='Body.TLabel', justify=tk.LEFT)
        self.lbl_body.pack(anchor=tk.W)

        self.lbl_alert = ttk.Label(
            self.frame, text='', style='Alert.TLabel', justify=tk.LEFT)
...
def on_message(client, userdata, msg):
    try:
        ...
        userdata['app'].root.after(
            0, userdata['app'].show_popup, filename, findings)
    except Exception as e:
