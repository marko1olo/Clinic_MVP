...
export function startRecallWorker() {
	console.log(
		"[RecallWorker] Started. Polling every 15 min for overdue osseointegration.",
	);
	// Run immediately on start, then on interval
	processOverdueRecalls().catch((err) =>
...
async function processOverdueRecalls() {
	const now = new Date();
	...
	console.log(
		`[RecallWorker] Found ${overdue.length} overdue recall(s). Creating draft tasks.`,
	);
	for (const candidate of overdue) {
		await createRecallTask(candidate, now);
	}
}
async function createRecallTask(candidate, now) {
	try {
		...
		const jaw =
			candidate.jawLocation === "upper" ? "верхней челюсти" : "нижней челюсти";
		...
		console.log(
			`[RecallWorker] Created recall task for patient ${candidate.patientId}`,
		);
	} catch (err) {
		console.error(`[RecallWorker] Failed for ${candidate.reservationId}:`, err);
	}
}
function buildPatientMessage(patientName, jaw) {
	return (
