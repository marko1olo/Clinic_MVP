import { documentKindMetadata, documentPayloadDisallowedKeys, kopecksToNumericString, legacyTaxDeductionCertificateMaxYear, legacyTaxDeductionCertificateMinYear, parseKopecks, sumKopecks, taxDeductionApplicationPayloadSchema, taxDeductionCertificateMinYear, } from "@dental/shared";
/*
...
function taxCertificateRequiresPayerInn(kind) {
    return kind === "legacy_tax_deduction_certificate";
}
function taxPaidDocumentRequiresPaymentSelection(kind) {
    return (kind === "tax_deduction_certificate" ||
...
function selectedPaymentReceiptIds(input) {
    return input.payload?.paymentReceipt?.selectedPaymentIds ?? [];
}
function selectedPaymentRefundCorrectionIds(input) {
    return input.payload?.paymentRefundCorrection?.selectedPaymentIds ?? [];
}
function payloadKindMismatchReason(input) {
...
function paymentPaidInTaxYear(payment, taxYear) {
    return paymentTaxYear(payment) === taxYear;
}
function normalizeInnDigits(value) {
    return (value ?? "").replace(/\D+/g, "");
}
function paymentMatchesTaxPayer(payment, payerInn) {
    const normalizedPayerInn = normalizeInnDigits(payerInn);
    if (!normalizedPayerInn)
        return true;
...
export function taxPaymentSelectionErrorForDocument(input, payments) {
    if (!taxPaidDocumentCanValidatePaymentSelection(input.kind))
        return null;
...
function paymentReceiptStoredInnMatchesPayload(storedValue, payloadValue) {
...
function paymentReceiptPayloadMatchesPayer(payment, payload) {
    ...
    if (!payload.taxSupportRequested)
        return true;
    return (paymentReceiptStoredFieldMatchesPayload(payment.payerBirthDate, payload.payerBirthDate) &&
...
function paymentReceiptMissingPayerFact(payment, payload) {
...
export function paymentReceiptSelectionErrorForDocument(input, payments) {
...
function treatmentCostEstimateMismatchReason(payload, facts) {
    return (financialServiceLinesMismatchReason(payload.serviceLines, "Смета лечения") ??
...
function paymentInvoiceMismatchReason(payload, facts) {
    return (financialServiceLinesMismatchReason(payload.serviceLines, "Счет на оплату") ??
...
function paidContractMismatchReason(payload, facts) {
 ...
 */
function treatmentLineTotalKopecks(item) {
    const outcome = chargeLineOutcome(item);
    return outcome.ok ? outcome.kopecks : 0;
}
export function plannedAmountRubForDocument(kind, input, treatmentPlanItems) {
    const metadata = documentKindMetadata[kind];
    ...
    const totalKopecks = sumKopecks(treatmentPlanItems
