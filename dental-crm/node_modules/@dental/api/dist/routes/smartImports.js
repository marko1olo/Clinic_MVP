...
const imagingKeywordPattern = /rvg|rv[sx]|прицел|прицельн|opg|оптг|ортопан|панорам|trg|трг|ceph|цеф|телерентг|cbct|кт|ккт|dicom|dicomweb|pacs|orthanc|dcm4chee|twain|wia|sensor|ezsensor|carestream|vatech|sidexis|romexis|ondemand|invivo|digora|soredex|trophy|visiodent|durr|dürr|orangedental|myray|newtom|dexis|kavo|gendex|acteon|sopro|sopix|pspix|x[-\s]?mind|3shape|medit|exocad|blue\s*sky|снимок|рентген|томограф/i;
const patientKeywordPattern = /фио|пациент|клиент|телефон|номер|дата рождения|д\.р\.|др|birth|patient|phone|mobile/i;
const clinicKeywordPattern = /клиник|стоматолог|dental|dent|clinic|инн|inn|кпп|kpp|огрн|ogrn|лиценз|license|адрес|address|сайт|website|www\.|https?:\/\/|email|e-mail|почта|банк|бик|р\/с|расчетн|корр/i;
...
const migrationClinicDataContainerPattern = /(?:стомат|клиник|dental|denta|dent|stom|clinic|mis|crm|пациент|patient|1c|1с|миграц|migration|стар|old|legacy).*(?:backup|backups|export|exports|archive|архив|arhiv|выгруз|vygruz|data|db|database|base|баз|baza|dump)|(?:backup|backups|export|exports|archive|архив|arhiv|выгруз|vygruz|data|db|database|base|баз|baza|dump).*(?:стомат|клиник|dental|denta|dent|stom|clinic|mis|crm|пациент|patient|1c|1с|миграц|migration|стар|old|legacy)|(?:база\s*пациент|пациенты|картотек|стоматолог(?:ия|ическая)?|архив\s*клиник|старая\s*(?:база|мис|crm)|выгрузк[аи]?|снимки|рентген|оптг|ккт|(?:^|[\\/\s_-])кт(?:$|[\\/\s_-])|xray|x-ray|cbct|opg|patient\s*db|patients|clinic\s*(?:db|backup|archive)|old\s*(?:db|database|crm))/i;
function migrationClinicDataContainerHint(folderPath) {
    const folderName = path.basename(folderPath);
    const parentName = path.basename(path.dirname(folderPath));
    const localName = parentName && parentName !== folderName
...
const migrationDumpExtensions = new Set([
    ".bak",
    ".backup",
    ".dump",
    ".sql",
...
const migrationTableExtensions = new Set([
    ".csv",
    ".tsv",
    ".xls",
    ".xlsx",
...
const migrationArchiveExtensions = new Set([
    ".zip",
    ".7z",
    ".rar",
    ".tar",
...
const migrationImageExtensions = new Set([
    ".jpg",
    ".jpeg",
    ".png",
    ".tif",
...
const migrationDicomExtensions = new Set([
    ".dcm",
    ".dicom",
    ".ima",
    ".dc3",
...
const migrationWorkstationProfiles = [
    {
        label: "1C/1Cv8",
        kind: "mis_database",
    ...
    },
    {
        ...
        kind: "mis_database",
        pattern: /open\s*dent(?:al)?|opendental|opendent|open\s*dent\s*images|atoz/i,
...
const migrationVendorGuidanceCatalog = [
    {
        label: "Romexis/Planmeca",
        pattern: /romexis|planmeca/i,
        requiredArtifacts: [
    ...
    },
    {
        ...
        recommendedRoute: "Для DEXIS/KaVo/Gendex сначала искать официальную выгрузку снимков; прямое чтение хранения использовать только для чтения и только для предпросмотра метаданных.",
    ...
    },
    {
        ...
        recommendedRoute: "Для Acteon/SOPRO/SOPIX/PSPIX/X-Mind строить список снимков, затем ручную сверку спорных совпадений пациента.",
    ...
    },
    {
        ...
        recommendedRoute: "Для DBF/FoxPro/Clipper использовать отдельную копию всей папки, затем локальный разбор в табличный черновик с контрольными итогами; не писать обратно в старые таблицы.",
...
const classificationRules = [
    ...
    },
    {
        ...
        condition: (text, ctx) => (ctx.hasImagingPathForScoring || ctx.hasImagingKeyword) &&
    ...
    },
    {
        ...
        condition: (text) => /pacs|orthanc|dcm4chee|dicomweb|qido|wado|пакс/i.test(text) &&
    ...
    },
    {
        ...
        condition: (text) => /\.csv|\.tsv|\.xls|\.xlsx|\.xlsm|\.xlsb|выгруз|экспорт/i.test(text) &&
            /(пациент|patient|клиент|visit|визит|payment|оплат|услуг|service)/i.test(text),
    },
];
function classifyLine(line, lineNumber, mode) {
    ...
    const hasImagingPathForScoring = hasImagePath &&
        !(hasClinicLicenseKeyword &&
            !/\.(?:dcm|dicom|ima|dc3|acr|jpg|jpeg|png|tif|tiff|bmp|webp)\b/i.test(text));
    const ctx = {
    ...
    for (const rule of classificationRules) {
        if (rule.condition(text, ctx)) {
            if (rule.category === "imagingScore")
                imagingScore += rule.score;
            else if (rule.category === "patientScore")
                patientScore += rule.score;
            else if (rule.category === "clinicScore")
                clinicScore += rule.score;
...
function stripClinicFieldTail(value) {
    return cleanExtractedValue(value.replace(/(?:^|[\s;|,])(?:инн|inn|кпп|kpp|огрн|ogrn|адрес|address|местонахождение|тел(?:ефон)?|phone|mobile|email|e-mail|почта|сайт|website|www\.|https?:\/\/|лиценз[^\s:=-]*|license|пациент|patient|клиент|client|фио|full\s*name|дата рождения|д\.р\.|dob|birth)(?=$|[\s:;|,=№#.-]).*$/i, ""));
}
...
function firstValidDigits(value, pattern, allowedLengths) {
...
function extractClinicName(value) {
    const legal = value.match(/(?:^|[\s;|,])((?:ООО|ОАО|ПАО|АО)\s+["«]?[A-Za-zА-Яа-яЁё0-9 ._-]+["»]?|ИП\s+[A-Za-zА-Яа-яЁё -]+)/i)?.[1];
    if (legal)
        return stripClinicFieldTail(legal);
    if (!/клиник|стоматолог|dental|dent|clinic/i.test(value))
        return null;
...
function extractAddress(value) {
    ...
    if (!raw)
        return null;
    return (cleanExtractedValue(raw.replace(/(?:^|[\s;|,])(?:инн|inn|кпп|kpp|огрн|ogrn|тел(?:ефон)?|phone|mobile|email|e-mail|почта|сайт|website|пациент|patient|клиент|client|фио|full\s*name|дата рождения|dob|birth)(?=$|[\s:;|,=№#.-]).*$/i, "")) || null);
}
function extractMedicalLicenseNumber(value) {
    return (firstMatch(value, /(?:лиценз[^\s:=-]*|license)\s*(?:№|#|n|no)?\s*[:=-]?\s*([A-Za-zА-Яа-яЁё0-9/.-]{3,80})/i) || null);
}
function extractDateLike(value) {
    return value.match(/\b\d{1,2}[./-]\d{1,2}[./-]\d{4}\b/)?.[0] ?? null;
}
function addClinicField(fields, warnings, key, value, lineNumber) {
...
function buildClinicProfileSuggestion(lines) {
    const fields = {};
    const warnings = [];
    const bankLines = [];
    lines.forEach((line) => {
        ...
        const phone = !lineHasPatientIdentity &&
...
function publicLookupSafeQuery(value) {
    ...
    return value
        .replace(/(?:^|[\s;|,])(?:пациент|patient|клиент|client|фио|full\s*name|дата рождения|д\.р\.|dob|birth)(?=$|[\s:;|,=№#.-]).*$/i, " ")
        .replace(/(?:^|[\s;|,])(?:инн|inn|кпп|kpp|огрн|ogrn)(?=$|[\s:;|,=№#.-])\s*\d[\d\s-]{7,17}\d/gi, " ")
        .replace(/[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}/gi, " ")
        .replace(/(?:\+7|7|8)?[\s(.-]*\d{3}[\s). -]*\d{3}[\s.-]*\d{2}[\s.-]*\d{2}/g, " ")
        .replace(/\b\d{1,2}[./-]\d{1,2}[./-]\d{4}\b/g, " ")
        .replace(/(?:[A-Za-zА-Яа-яЁё]:[\\/][^\s;|,]+|\\\\[^\s;|,]+|\/[^\s;|,]+|\b[^\s;|,]+\.(?:dcm|dicom|ima|jpg|jpeg|png|tif|tiff|bmp|webp|fdb|gdb|mdb|accdb|db|sqlite|sqlite3|dbf|dbt|fpt|cdx|idx|ntx|ndx|mdx|bak|sql|dump|backup|zip|7z|rar)\b)/gi, " ")
        .replace(/(?:^|[^\p{L}\p{N}])(?:studyinstanceuid|seriesinstanceuid|sopinstanceuid|dicomdir|dicomweb|pacs|orthanc|dcm4chee|qido|qido-rs|wado|wado-rs|rvg|opg|cbct|xray|x-ray|кт|ккт|клкт|оптг|трг|рентген[а-яё]*|сним(?:ок|ки|к[а-яё]*)|исследовани[а-яё]*|серия|study|series)(?=$|[^\p{L}\p{N}])(?:[^\s;|,]*)?/giu, " ")
        .replace(/[;|\t]+/g, " ")
        .replace(/\s+/g, " ")
        .trim()
        .slice(0, 180)
...
function publicLookupDigits(value, allowedLengths) {
    const digits = value?.replace(/\D/g, "") ?? "";
    return allowedLengths.includes(digits.length) ? digits : "";
}
function publicLookupLabeledDigits(values, labelPattern, allowedLengths) {
...
function buildClinicSuggestionFromFields(fields) {
    const cleanFields = Object.fromEntries(Object.entries(fields).filter(([, value]) => typeof value !== "undefined" && value !== null && String(value).trim()));
...
function buildManualClinicPublicLookupSuggestion(fields) {
...
function uniqueClinicPublicLookupSuggestions(suggestions) {
    const seen = new Set();
    const result = [];
...
function addPublicLookupTarget(targets, target) {
...
function buildPublicLookupTargets(clinicSuggestion, clinicRawText) {
    const fields = clinicSuggestion?.fields ?? {};
    const clinicQuery = publicLookupSafeQuery([fields.legalName, fields.clinicName, fields.address]
        .filter(Boolean)
        .join(" "));
    ...
    const fallbackQuery = clinicRawText
        ? (rawClinicLines.find((line) => line.length >= 3 &&
...
function dadataToken() {
    return (process.env.DENTAL_DADATA_API_KEY?.trim() ||
        process.env.DADATA_API_KEY?.trim() ||
        "");
}
function dadataObject(value) {
    return value && typeof value === "object" && !Array.isArray(value)
        ? value
        : null;
}
function dadataString(source, key) {
    const value = source?.[key];
    return typeof value === "string" ? value : "";
}
function dadataNestedObject(source, key) {
    return dadataObject(source?.[key]);
}
function mapDadataPartySuggestion(item) {
    ...
    if (ogrn)
        fields.ogrn = ogrn;
    const legalName = dadataString(name, "short_with_opf") ||
        dadataString(name, "full_with_opf") ||
        dadataString(suggestion, "value");
    if (legalName.trim())
        fields.legalName = cleanExtractedValue(legalName);
    const address = dadataString(addressData, "unrestricted_value") ||
        dadataString(addressData, "value");
    if (address.trim())
        fields.address = cleanExtractedValue(address);
...
async function fetchDadataClinicSuggestions(input, safeQuery) {
    ...
    const exactQuery = publicLookupDigits(input.inn, [10, 12]) ||
...
async function buildClinicPublicLookup(input) {
    ...
    const fields = {
        inn: publicLookupDigits(input.inn, [10, 12]) ||
            publicLookupLabeledDigits(rawLookupValues, /(?:инн|inn)\D*(\d[\d\s-]{8,14}\d)/i, [10, 12]) ||
            undefined,
        kpp: publicLookupDigits(input.kpp, [9]) ||
            publicLookupLabeledDigits(rawLookupValues, /(?:кпп|kpp)\D*(\d[\d\s-]{7,11}\d)/i, [9]) ||
            undefined,
        ogrn: publicLookupDigits(input.ogrn, [13, 15]) ||
            publicLookupLabeledDigits(rawLookupValues, /(?:огрн|ogrn)\D*(\d[\d\s-]{11,17}\d)/i, [13, 15]) ||
            undefined,
    ...
    const suggestion = buildClinicSuggestionFromFields(fields);
    const publicLookupTargets = buildPublicLookupTargets(suggestion, [
        fields.legalName,
        fields.clinicName,
        fields.address,
        fields.inn,
        ...
        .filter(Boolean)
        .join("\n"));
    const safeQuery = fields.inn ||
        fields.ogrn ||
        fields.kpp ||
        publicLookupSafeQuery([fields.legalName, fields.clinicName, fields.address]
            .filter(Boolean)
            .join(" "));
    ...
    const suggestions = uniqueClinicPublicLookupSuggestions([...providerResult.suggestions, manualSuggestion].filter(Boolean));
...
function migrationConfiguredRootsEnv(name) {
    return (process.env[name]
        ?.split(/[;|]/)
        .map((root) => root.trim())
        .filter(Boolean) ?? []);
}
function migrationRootExists(root) {
    try {
...
function migrationAvailableWindowsDriveRoots() {
    ...
    for (let code = "D".charCodeAt(0); code <= "Z".charCodeAt(0); code += 1) {
        ...
        if (migrationRootExists(root))
            roots.push(root);
    }
    return roots;
}
function migrationDriveDataRoots(driveRoots) {
    const folderHints = [
        "Dental",
        "Denta",
        "Stomatology",
...
function migrationFingerprint(value) {
    const stableValue = /^https?:\/\//i.test(value) ||
        value.startsWith("\\\\") ||
...
function safeMigrationDiscoveryRoot(root) {
    ...
    if (trimmed.startsWith("\\\\"))
        return `network-root:${fingerprint}`;
    if (/^https?:\/\//i.test(trimmed))
        return `remote-root:${fingerprint}`;
    if (path.isAbsolute(trimmed) || /^[A-Za-z]:[\\/]/.test(trimmed))
        return `local-root:${fingerprint}`;
    return `source-root:${fingerprint}`;
}
function safeMigrationDiscoveryRoots(roots) {
    return uniqueStrings(roots.map((root) => safeMigrationDiscoveryRoot(root)));
}
const migrationSourceRouteStore = new Map();
function isMigrationPublicSourceToken(sourceRef) {
    return /^(?:browser-local|smart-preview|workstation-profile|workstation-signal):[a-f0-9]{8,12}$/i.test(sourceRef);
}
function isMigrationSourceRouteToken(sourceRef) {
    return /^migration-source:[a-f0-9]{8,12}$/i.test(sourceRef);
}
function registerMigrationSourceRoute(sourceRef, sourceKind, safeDisplayName) {
    if (isMigrationPublicSourceToken(sourceRef))
        return sourceRef;
...
function resolveMigrationSourceRoute(sourceRef) {
...
function migrationDiscoveryDepth(root, folderPath) {
    const relative = path.relative(root, folderPath);
    if (!relative || relative === ".")
        return 0;
    return relative.split(path.sep).filter(Boolean).length;
}
function migrationFolderHintScore(folderPath) {
    const normalized = folderPath.toLowerCase();
    let score = 0;
    if (/стомат|стоматология|dental|denta|clinic|клиник|mis|crm|legacy|migration|миграц|перенос|backup|dump|export|выгруз|стар(?:ая|ой)?|база\s*пациент|пациенты|картотек|архив\s*клиник/.test(normalized))
        score += 0.14;
    if (migrationClinicDataContainerHint(folderPath))
        score += 0.16;
    if (legacyMisTextPattern.test(normalized) ||
        /sql\s*server|firebird|interbase|access/.test(normalized))
        score += 0.2;
    if (imagingVendorPattern.test(normalized))
        score += 0.18;
    if (/dicom|dicomdir|cbct|кт|ккт|rvg|opg|оптг|xray|x-ray|рентген|снимк|pacs|orthanc|dcm4chee|radiology|pano|panoramic/.test(normalized))
        score += 0.18;
...
function migrationWorkstationProfileMatches(value) {
    return migrationWorkstationProfiles.filter((profile) => profile.pattern.test(value));
}
function migrationVendorGuidanceMatches(value) {
    return migrationVendorGuidanceCatalog.filter((guidance) => guidance.pattern.test(value));
}
function migrationDirectoryPriority(folderPath) {
    const profileMatches = migrationWorkstationProfileMatches(folderPath);
    const hintScore = migrationFolderHintScore(folderPath);
    if (profileMatches.length >= 2 || hintScore >= 0.34)
        return 2;
    if (profileMatches.length || hintScore >= 0.14)
        return 1;
    return 0;
}
function migrationDiscoveryEntryPriority(entry, folderPath) {
    ...
    if (/^DICOMDIR$/i.test(entryName))
        priority += 100;
    if (migrationDatabaseExtensions.has(extension))
        priority += 92;
    if (migrationDumpExtensions.has(extension))
        priority += 84;
    if (migrationDicomExtensions.has(extension))
        priority += 78;
    if (migrationTableExtensions.has(extension))
        priority += 58;
    if (migrationArchiveExtensions.has(extension))
        priority += 44;
    if (migrationImageExtensions.has(extension))
        priority += 24;
    if (migrationWorkstationProfileMatches(fullPath).length)
        priority += 20;
    return priority;
}
function migrationSourceKindFromCounts(input) {
    ...
    if (profileKind === "vendor_imaging_system")
        return "vendor_imaging_system";
    if (imagingVendorPattern.test(text) ||
        imagingVendorSupplementalPattern.test(text))
        return "vendor_imaging_system";
    if (input.hasDicomDir ||
        input.dicomLikeFiles > 0 ||
        /dicom|cbct|кт|ккт/.test(text))
        return "dicom_folder";
    if (input.imageFiles > 8 ||
        /rvg|opg|оптг|xray|x-ray|рентген|снимк|фото/.test(text))
        return "xray_image_archive";
    if (/\.fdb\b|\.gdb\b|\.fbk\b|\.ib\b|\.ibk\b|\.gbk\b|firebird|interbase/.test(text))
        return "firebird_database";
    if (/\.mdb\b|\.accdb\b|access\b/.test(text))
        return "access_database";
    if (/\.dbf\b|\.dbt\b|\.fpt\b|\.cdx\b|\.idx\b|\.ntx\b|\.ndx\b|\.mdx\b|dbase|foxpro|visual\s*foxpro|clipper|paradox/.test(text))
        return "mis_database";
    if (/\.sqlite\b|\.sqlite3\b|sqlite|\.db\b/.test(text))
        return "sqlite_database";
    if (/mysql|mariadb|postgres|postgresql|pgsql|psql|\.myd\b|\.myi\b|\.frm\b|\.ibd\b/.test(text))
        return "mis_database";
    if (input.dumpFiles > 0 ||
        /\.sql\b|\.dump\b|\.bak\b|\.dt\b|\.mdf\b|\.ldf\b|\.sdf\b|postgres|mysql|mssql|sql\s*server/.test(text))
        return "sql_dump";
    if (input.tableFiles > 0)
        return input.firstMatchPath.toLowerCase().endsWith(".csv") ||
            ...
            ? "csv_export"
            : "spreadsheet_export";
    if (input.archiveFiles > 0)
        return "archive_export";
    if (profileKind)
        return profileKind;
    if (legacySourceSupplementalKeywordPattern.test(text) ||
        legacyMisTextPattern.test(text))
        return "mis_database";
    return "unknown_legacy_source";
}
function migrationDbfFolderSourceRequired(folderPath, firstMatchPath) {
...
function migrationSafeAlias(kind, sourceRef) {
    return `${legacySourceTitles[kind]} #${migrationFingerprint(sourceRef).toUpperCase()}`;
}
function migrationProfileSafeAlias(profileLabel, kind, sourceRef) {
    return `${profileLabel || legacySourceTitles[kind]} #${migrationFingerprint(sourceRef).toUpperCase()}`;
}
function shouldSkipMigrationDiscoveryDirectory(directoryName) {
    return migrationDiscoverySkipDirectoryNames.has(directoryName.toLowerCase());
}
function migrationWorkstationSignalChannelTitle(channel) {
    if (channel === "shortcut")
        return "ярлык ОС";
    if (channel === "installed_app")
        return "установленная программа";
    if (channel === "process")
        return "процесс ОС";
    if (channel === "service")
        return "служба ОС";
...
function normalizeMigrationSignalValues(value) {
    return (value ?? "")
        .split(/\r?\n|[;|]/)
        .map((item) => item.trim())
...
async function readWindowsMigrationWorkstationSignalValues(warnings) {
    if (os.platform() !== "win32")
        return [];
    ...
    try {
        ...
        const stdout = await new Promise((resolve, reject) => {
            const child = execFile(psPath, ["-NoProfile", "-NonInteractive", "-Command", "-"], {
                timeout: 2500,
                maxBuffer: 160 * 1024,
                windowsHide: true,
...
async function collectMigrationWorkstationSignals(input, warnings) {
    ...
    for (const signal of signals) {
        ...
        if (unique.size >= input.maxWorkstationSignals)
            break;
    }
    return Array.from(unique.values());
}
function migrationIsTooBroadDerivedRoot(root) {
...
function migrationExistingDirectory(value) {
    try {
...
function migrationSignalPathFragments(value) {
    ...
    for (const part of parts) {
        ...
        if (executable)
            fragments.add(executable.trim());
        const absolute = part.match(/^([A-Za-zА-Яа-яЁё]:[\\/][^|"\r\n]+|\\\\[^|"\r\n]+)/i)?.[1];
        if (absolute)
            fragments.add(absolute.trim());
    }
    if (!fragments.size) {
...
function migrationNearbyDataRoots(baseDirectory) {
    const nearbyNames = [
        "Data",
        "DB",
        "Database",
...
function migrationProfileRelativeDataRoots(profile) {
    ...
    if (/cliniccards/i.test(profile.label))
        return ["Cliniccards", "ClinicCards"];
    if (/dental4windows/i.test(profile.label))
        return ["Dental4Windows", "D4W"];
    ...
    if (/1c|1cv8/i.test(profile.label))
        return ["1C", "1Cv8", "1cv8"];
    if (/ident|stomx/i.test(profile.label))
        return ["IDENT", "StomX", "Ident"];
...
function migrationRootsFromWorkstationProfiles(profiles) {
    const roots = [];
...
function migrationRootsFromWorkstationSignals(signals) {
    const roots = [];
    for (const signal of signals) {
        ...
        for (const fragment of migrationSignalPathFragments(signal.value)) {
            ...
            if (!directory || migrationIsTooBroadDerivedRoot(directory))
                continue;
...
async function readWindowsMigrationMappedRoots(warnings) {
    if (os.platform() !== "win32")
        return [];
    const script = [
        "$ErrorActionPreference='SilentlyContinue'",
    ...
    try {
        ...
        for (const row of rows) {
            ...
            if (/^[D-Z]:\\$/i.test(root))
                roots.push(root);
            if (displayRoot.startsWith("\\\\"))
                roots.push(displayRoot);
        }
...
function migrationCandidateFromWorkstationSignal(signal) {
...
async function discoverLocalMigrationSources(input) {
    if (process.env.DENTE_ENABLE_LOCAL_MIGRATION_SCAN !== "true") {
        return migrationLocalSourceDiscoveryResponseSchema.parse({
            ...
            candidates: [],
            warnings: [
                "Локальный поиск баз данных отключен в облачном режиме. Загружайте файлы через браузер или используйте локальный модуль миграции.",
            ],
    ...
    if (queue.length)
        warnings.add(`Поиск остановлен после ${input.maxFolders} папок. Выберите папку ближе к старой программе или увеличьте лимит проверки.`);
    ...
    if (!candidates.length)
        warnings.add("Старые базы, выгрузки, архивы или папки снимков не найдены в выбранных корнях.");
...
function migrationWorkupExtractableEntities(kind) {
    if (kind === "dicom_folder" || kind === "pacs_dicom")
        return ["imaging", "dicom_series"];
    if (kind === "vendor_imaging_system")
        return ["imaging", "dicom_series", "patients"];
    if (kind === "xray_image_archive")
        return ["imaging", "patients"];
    ...
    if (kind === "network_share")
        return ["patients", "documents", "imaging", "unknown"];
    if (kind === "firebird_database" ||
        kind === "access_database" ||
        kind === "sqlite_database" ||
        kind === "sql_dump" ||
        kind === "mis_database") {
...
function migrationWorkupHandoffs(kind) {
...
function migrationWorkupSteps(kind, sourceExists) {
    const firstStatus = sourceExists ? "ready" : "manual";
    if (kind === "dicom_folder" ||
        kind === "pacs_dicom" ||
        kind === "vendor_imaging_system") {
        return [
    ...
    return [
        {
            ...
            status: sourceExists ? "ready" : "manual",
...
function migrationSourceKindIsDatabase(kind) {
    return (kind === "firebird_database" ||
        kind === "access_database" ||
        kind === "sqlite_database" ||
        kind === "sql_dump" ||
        kind === "mis_database");
}
function migrationSourceKindIsImaging(kind) {
    return (kind === "dicom_folder" ||
        kind === "pacs_dicom" ||
        kind === "vendor_imaging_system" ||
        kind === "xray_image_archive");
}
function migrationSourceKindIsTableLike(kind) {
    return (kind === "csv_export" ||
        kind === "spreadsheet_export" ||
        kind === "archive_export");
}
function migrationReadinessItem(input) {
    return input;
}
function buildMigrationReadiness(input) {
    ...
    if (!input.sourceExists || hasBlockedAdapter) {
        blockers.push(migrationReadinessItem({
            id: "source_available",
        ...
        warnings.push(migrationReadinessItem({
        ...
        ready.push(migrationReadinessItem({
            id: "source_available",
    ...
    if (migrationSourceKindIsDatabase(input.sourceKind)) {
    ...
    else if (input.sourceKind === "vendor_imaging_system" &&
        !hasBuiltInAdapter) {
        blockers.push(migrationReadinessItem({
        ...
        warnings.push(migrationReadinessItem({
            ...
            nextAction: adapters.find((adapter) => adapter.status === "needs_local_bridge")
        ...
        ready.push(migrationReadinessItem({
            ...
            nextAction: adapters[0]?.nextAction ??
                input.nextAction ??
    ...
    const score = Number(Math.max(0, Math.min(1, 0.92 -
        finalBlockers.length * 0.22 -
        finalWarnings.length * 0.06 +
        Math.min(0.08, finalReady.length * 0.02))).toFixed(2));
    const level = !input.sourceExists || hasBlockedAdapter
        ? "blocked"
        : finalBlockers.some((item) => item.id.includes("export") ||
    ...
    return {
        ...
        ready: finalReady,
        nextAction: finalBlockers[0]?.nextAction ??
...
function migrationBridgeAction(input) {
    return input;
}
function migrationBridgeOutputManifest(kind, endpoint) {
...
function buildMigrationBridgeKit(input) {
    ...
    const commonPrivacy = "Пациенты, телефоны, снимки, файлы базы, пароли и сырые локальные пути остаются в локальном черновике. Публичный поиск получает только реквизиты клиники.";
    ...
    const baseStatus = input.readiness.level === "blocked"
        ? "blocked"
    ...
    if (input.sourceKind === "vendor_imaging_system" ||
...
function buildMigrationLocalSourceWorkup(input) {
    if (process.env.DENTE_ENABLE_LOCAL_MIGRATION_SCAN !== "true") {
    ...
    const normalizedSourceRef = isUrl ||
    ...
    const inferredKind = input.sourceKind ??
    ...
    const safeDisplayName = input.safeDisplayName?.trim() ||
    ...
    let sourceExists = !routeRef.routeExpired &&
        (isUrl || isBrowserLikeManifest || isWorkstationTrace);
    let sourceIsDirectory = false;
    let fileExtension = isUrl || isBrowserLikeManifest || isWorkstationTrace
        ? null
                    ...
                    : isWorkstationSignal
                        ...
                        : sourceIsDirectory
                            ...
                            : sourceExists
                                ? "Локальный файл"
                                : "Путь или источник";
    const smartImportLineSourceRef = routeRef.routeToken ??
        (isMigrationPublicSourceToken(normalizedSourceRef)
            ? normalizedSourceRef
            : registerMigrationSourceRoute(normalizedSourceRef, inferredKind, safeDisplayName));
...
const migrationProbeVendorMatchers = [
...
const migrationProbeArtifactKindTitles = {
...
function migrationProbeArtifactKind(filePath) {
...
async function readMigrationProbeHeader(filePath, maxBytes) {
...
function migrationProbeFormatSignals(filePath, header, kind) {
    const name = path.basename(filePath);
    const extension = path.extname(name).toLowerCase();
    ...
    const signals = [];
    if (extension)
        signals.push(`расширение ${extension}`);
    if (/^DICOMDIR$/i.test(name))
        signals.push("служебный каталог снимков");
    ...
    if (latin.startsWith("PK\u0003\u0004"))
        signals.push("архив или Office-файл");
    ...
    if (extension === ".1cd")
        signals.push("1C база");
    if (extension === ".dt")
        signals.push("1C выгрузка");
    if (extension === ".mdf" || extension === ".ldf")
        signals.push("SQL Server файлы данных");
    if (extension === ".sdf")
        signals.push("SQL Server Compact база");
    if (extension === ".dbf")
        signals.push("DBF/FoxPro таблица");
    if (extension === ".dbt" ||
        extension === ".fpt" ||
        extension === ".cdx" ||
        extension === ".idx" ||
    ...
    if (kind === "model")
        signals.push("Стоматологическая 3D-модель");
    if (kind === "image")
        signals.push("2D снимок или фото");
    return uniqueStrings(signals);
}
function detectMigrationProbeVendors(values) {
    const text = values.join(" ");
    return migrationProbeVendorMatchers
        .filter(([, pattern]) => pattern.test(text))
        .map(([label]) => label);
}
function migrationProbeSafeArtifact(filePath, kind, depth, signals) {
    let byteSize = null;
    let modifiedAt = null;
    try {
...
function incrementMigrationProbeCount(counts, kind) {
    if (kind === "database")
        counts.databases += 1;
    else if (kind === "dump")
        counts.dumps += 1;
...
async function inspectMigrationProbeFile(input) {
    ...
    if ((kind !== "unknown" || signals.length > 0) &&
...
function addMigrationProbeWarnings(warnings, flags) {
...
async function scanMigrationProbeDirectory(normalizedSourceRef, input, counts, formatSignals, artifactSamples, warnings, vendorInputs, initialLatestModifiedAt, initialScannedFolders, initialScannedFiles) {
    ...
    while (queue.length &&
        ...
        const orderedEntries = [...entries].sort((left, right) => migrationDiscoveryEntryPriority(right, current.folderPath) -
        ...
        for (const entry of orderedEntries) {
            ...
            if (entry.isDirectory()) {
                if (shouldSkipMigrationDiscoveryDirectory(entryName))
                    continue;
    ...
    if (queue.length)
        warnings.add(`Проверка источника остановлена после ${input.maxFolders} папок; выберите папку ближе к старой программе для более точной инвентаризации.`);
    return { latestModifiedAt, scannedFolders, scannedFiles };
}
async function buildMigrationLocalSourceProbe(input) {
    if (process.env.DENTE_ENABLE_LOCAL_MIGRATION_SCAN !== "true") {
        throw new Error("Чтение локальной файловой системы сервера отключено в облачном режиме. Доступ запрещен.");
    }
    const routeRef = resolveMigrationSourceRoute(input.sourceRef);
    ...
    const warnings = new Set();
    const formatSignals = new Set();
    const artifactSamples = [];
    const vendorInputs = routeRef.routeExpired
        ? [safeDisplayName]
        : [normalizedSourceRef];
    let sourceExists = !routeRef.routeExpired &&
        (isUrl || isBrowserLikeManifest || isWorkstationTrace);
    ...
    if (sourceExists &&
        ...
        !isWorkstationTrace &&
        sourceIsDirectory) {
        const scanResult = await scanMigrationProbeDirectory(normalizedSourceRef, input, counts, formatSignals, artifactSamples, warnings, vendorInputs, latestModifiedAt, scannedFolders, scannedFiles);
...
function migrationAutopilotPriority(score) {
    if (score >= 0.82)
        return "critical";
    if (score >= 0.64)
        return "high";
...
function migrationAutopilotOwner(candidate, probe) {
    if (probe?.adapters.some((adapter) => adapter.status === "blocked"))
        return "administrator";
    if (candidate.databaseFiles > 0 ||
...
function migrationAutopilotScore(candidate, probe) {
    const probeCount = (probe?.counts.databases ?? 0) +
    ...
    const score = candidate.confidence * 0.52 +
...
function migrationAutopilotRiskFlags(candidate, probe) {
    const flags = new Set();
    if (!probe)
        flags.add("probe_not_completed");
    if (probe?.sourceExists === false)
        flags.add("source_unavailable");
    if (probe?.adapters.some((adapter) => adapter.status === "needs_local_bridge"))
...
function migrationAutopilotReadiness(candidate, probe) {
    if (probe)
    ...
    return buildMigrationReadiness({
        ...
        counts: {
            ...
            models: 0,
            unknown: Math.max(0, candidate.matchedFiles -
        ...
        isBrowserManifest,
        isSmartPreviewSource,
        isWorkstationProfile: isWorkstationProfile ||
            /^workstation-signal:[a-f0-9]{8,12}$/i.test(sourceRef),
        isUrl: /^https?:\/\//i.test(sourceRef),
        nextAction: playbook.nextAction,
    });
}
function migrationAutopilotBridgeKit(candidate, probe, readiness) {
    if (probe)
        return probe.bridgeKit;
    const sourceRef = candidate.sourceRef;
    return buildMigrationBridgeKit({
        ...
        isWorkstationProfile: /^workstation-profile:[a-f0-9]{8,12}$/i.test(sourceRef) ||
            /^workstation-signal:[a-f0-9]{8,12}$/i.test(sourceRef),
        isUrl: /^https?:\/\//i.test(sourceRef),
    });
}
function migrationAutopilotRecommendedAction(candidate, probe) {
    const bestAdapter = probe?.adapters[0];
    if (bestAdapter?.nextAction)
        return bestAdapter.nextAction;
    if (candidate.databaseFiles > 0 ||
        candidate.dumpFiles > 0 ||
        [
            "firebird_database",
    ...
    if (candidate.dicomLikeFiles > 0 ||
        ...
        ["dicom_folder", "vendor_imaging_system", "pacs_dicom"].includes(candidate.sourceKind)) {
...
function migrationPacketStatusFromSources(sources) {
    if (!sources.length)
        return "empty";
...
function migrationPacketLaneStatusFromSubset(sources, fallback) {
    return sources.length ? migrationPacketStatusFromSources(sources) : fallback;
}
function migrationPacketScore(sources, clinicLookup) {
    ...
    const clinicScore = clinicLookup
        ? clinicLookup.suggestions.length
            ...
            : clinicLookup.publicLookupTargets.length
                ...
                : 0.18
        : 0;
    const score = sources.length && clinicLookup
...
function migrationPacketStatusFromBridgeStatus(status, readiness) {
    if (readiness.level === "blocked" || status === "blocked")
        return "blocked";
...
function buildMigrationHandoffChecklist(input) {
    ...
    const addItem = (item) => {
        ...
        if (items.some((existing) => `${existing.phase}:${existing.owner}:${existing.sourceFingerprint ?? "clinic"}:${existing.title}` ===
            key))
            return;
        items.push(item);
    };
    ...
    for (const source of input.sources.slice(0, 6)) {
        ...
        for (const action of source.bridgeKit.adminActions.slice(0, 2)) {
            addItem({
                ...
                blocking: source.readiness.blockers.length > 0 ||
        ...
        if (doctorAction) {
            addItem({
                ...
                status: source.readiness.level === "ready_for_preview"
                    ? "needs_admin"
                    : status,
...
function buildMigrationOperatorScript(input) {
    ...
    const topSourceIsSmartPreview = Boolean(topSource &&
        /^smart-preview:[a-f0-9]{8,12}$/i.test(topSource.candidate.sourceRef));
    const topDatabaseSource = sources.find((source) => [
        "firebird_database",
        "access_database",
        "sqlite_database",
        "sql_dump",
        "mis_database",
    ].includes(source.candidate.sourceKind)) ?? null;
    const topMediaSource = sources.find((source) => [
        "dicom_folder",
        "pacs_dicom",
        "vendor_imaging_system",
        "xray_image_archive",
    ...
    const previewSource = sources.find((source) => source.readiness.level === "ready_for_preview") ??
        topTableSource ??
        topDatabaseSource ??
        topMediaSource ??
        topSource;
...
function migrationSourceCanStartDryRunPreview(source) {
    ...
    const hasReadableMaterial = candidate.tableFiles +
        candidate.archiveFiles +
        candidate.dicomLikeFiles +
        candidate.imageFiles +
        candidate.matchedFiles >
        0;
    return (source.readiness.level === "ready_for_preview" ||
        source.bridgeKit.status === "ready" ||
        tableLikeSource ||
...
function buildMigrationDryRunSummary(input) {
    ...
    const adminBlockedSources = input.sources.filter((source) => source.owner === "administrator" &&
        (source.readiness.blockers.length > 0 ||
            ["blocked", "needs_bridge", "needs_export"].includes(source.readiness.level) ||
            source.bridgeKit.status !== "ready")).length;
    const doctorReviewRequiredSources = input.sources.filter((source) => {
        const parserTargets = new Set(source.bridgeKit.parserTargets);
        return (parserTargets.has("patients") ||
            parserTargets.has("imaging") ||
            parserTargets.has("dicom_series") ||
            source.riskFlags.includes("review_patient_media_matching"));
    }).length;
    const primaryStep = input.operatorScript.steps.find((step) => step.blocking && step.owner !== "doctor" && step.action !== "manual") ??
    ...
    const estimatedClinicDowntimeMinutes = input.sources.length
        ? Math.min(60, (input.totals.blocked > 0 ? 20 : 0) +
...
function buildMigrationOperatorPacket(input) {
    const sources = input.sources;
    const databaseSources = sources.filter((source) => source.candidate.databaseFiles > 0 ||
    ...
    const mediaSources = sources.filter((source) => source.candidate.dicomLikeFiles > 0 ||
    ...
    const tableSources = sources.filter((source) => source.candidate.tableFiles > 0 ||
        ["csv_export", "spreadsheet_export", "archive_export"].includes(source.candidate.sourceKind));
    ...
    const lanes = [
        {
            ...
            nextAction: input.clinicLookup?.nextAction ??
        ...
        },
        {
            ...
            score: databaseSources.length || tableSources.length
                ? Number(((databaseSources.length + tableSources.length) /
                    Math.max(1, sources.length)).toFixed(2))
                : 0,
        ...
        },
        {
            ...
            nextAction: sources[0]?.bridgeKit.nextAction ??
    ...
    ].filter((item) => Boolean(item && item.trim()))).slice(0, 6);
    ...
    return {
        ...
        score: migrationPacketScore(sources, input.clinicLookup),
        dataClasses: {
            clinicRequisites: Boolean(input.clinicLookup?.safeQuery ||
                input.clinicLookup?.suggestions.length ||
                input.clinicLookup?.publicLookupTargets.length),
...
function uniqueByMigrationCandidateKey(candidates, keyOf) {
    const unique = new Map();
...
function migrationSmartPreviewSourceRef(source, index) {
    const safeExistingRef = source.sourceRef &&
        /^(?:browser-local|smart-preview|workstation-profile|workstation-signal|migration-source):[a-f0-9]{8,12}$/i.test(source.sourceRef)
        ? source.sourceRef
        : null;
    if (safeExistingRef)
        return safeExistingRef;
...
function migrationCandidateFromSmartLegacySource(source, index) {
    ...
    const isDicom = source.kind === "dicom_folder" ||
    ...
    return {
        ...
        hasDicomDir: source.kind === "dicom_folder" ||
...
function clinicLookupInputFromSmartImport(suggestion) {
    const fields = suggestion?.fields;
    if (!fields)
        return null;
    const clinicText = (value) => value?.trim() || undefined;
    const payload = {
...
async function buildMigrationAutopilot(orgId, input) {
    if (process.env.DENTE_ENABLE_LOCAL_MIGRATION_SCAN !== "true") {
        throw new Error("Чтение локальной файловой системы сервера отключено в облачном режиме. Доступ запрещен.");
    }
    const warnings = new Set();
    const privacyWarnings = new Set([
    ...
    const candidateKey = (candidate) => `${candidate.sourceRef.toLowerCase()}|${candidate.sourceKind}`;
    ...
    const sortedCandidates = Array.from(candidatesBySource.values()).sort((left, right) => right.confidence - left.confidence ||
        right.matchedFiles - left.matchedFiles ||
        right.databaseFiles +
            right.dumpFiles +
    ...
    const candidates = uniqueByMigrationCandidateKey([
        ...sortedCandidates.filter((candidate) => knownCandidateKeys.has(candidateKey(candidate))),
        ...sortedCandidates,
    ], candidateKey).slice(0, input.maxCandidates);
    const probedCandidates = uniqueByMigrationCandidateKey([
        ...candidates.filter((candidate) => knownCandidateKeys.has(candidateKey(candidate))),
        ...candidates,
    ], candidateKey).slice(0, Math.min(input.maxProbeCandidates, candidates.length));
    const sources = await Promise.all(probedCandidates.map(async (candidate) => {
        let probe = null;
        try {
    ...
    let clinicLookup = null;
    const clinicLookupInput = input.clinic ??
...
function extractLegacySourceRef(value) {
    return (value.match(/\bbrowser-local:[a-f0-9]{8,12}\b/i)?.[0] ??
        ...
        value
            .match(/(?:[A-Za-zА-Яа-яЁё]:[\\/][^;|\n]+|\\\\[^;|\n]+|\/[^;|\n]+)(?:[\\/]DICOMDIR\b)?/i)?.[0]
            ?.trim() ??
        value.match(/\bDICOMDIR\b/i)?.[0] ??
        null);
}
function detectLegacySourceKind(value, sourceRef) {
    const text = `${value} ${sourceRef ?? ""}`.toLowerCase();
    if (/старая серверная база программы/.test(text))
        return "firebird_database";
    if (/старая настольная база/.test(text))
        return "access_database";
    if (/локальная база программы/.test(text))
        return "sqlite_database";
    if (/резервная копия старой базы/.test(text))
        return "sql_dump";
    if (/pacs|orthanc|dcm4chee|dicomweb|qido|wado|ae\s*title|dicom\s*server|пакс/.test(text))
        return "pacs_dicom";
    ...
    if (/\.sqlite\b|\.sqlite3\b|sqlite|(?:^|[\\/])[^\\/]+\.(?:db)\b/.test(text))
        return "sqlite_database";
    if (/\.sql\b|\.dump\b|\.bak\b|\.dt\b|\.mdf\b|\.ldf\b|\.sdf\b|postgres|postgresql|mysql|mssql|sql\s*server/.test(text))
        return "sql_dump";
    if (/\.xlsx\b|\.xlsm\b|\.xlsb\b|\.xls\b|\.ods\b|excel|таблиц/.test(text))
        return "spreadsheet_export";
    if (/\.csv\b|\.tsv\b|\.json\b|\.xml\b/.test(text))
        return "csv_export";
...
function legacySourceEvidence(value, sourceRef) {
    const evidence = new Set();
    if (sourceRef)
        evidence.add(`sourceRef=${sourceRef}`);
    if (/\.fdb|\.gdb|\.fbk|\.ib\b|\.ibk\b|\.gbk\b|firebird|interbase/i.test(value))
        evidence.add("Firebird/InterBase");
    if (/\.mdb|\.accdb|access/i.test(value))
        evidence.add("Access");
    if (/\.dbf|\.dbt|\.fpt|\.cdx|\.idx|\.ntx|\.ndx|\.mdx|dbase|foxpro|visual\s*foxpro|clipper|paradox/i.test(value))
        evidence.add("DBF/FoxPro/Clipper");
    if (/\.sqlite|\.sqlite3|sqlite|\.db\b/i.test(value))
        evidence.add("SQLite/DB файл");
    if (/\.1cd|\.dt|1c|1с/i.test(value))
        evidence.add("1C база/выгрузка");
    if (/\.sql|\.dump|\.bak|\.mdf|\.ldf|\.sdf|postgres|mysql|mssql|sql server/i.test(value))
        evidence.add("SQL резервная копия");
    if (/\.csv|\.tsv|\.xls|\.xlsx|\.xlsm|\.xlsb|\.ods|\.xml|\.json|excel/i.test(value))
        evidence.add("табличная выгрузка");
    if (/\.zip|\.7z|\.rar|\.tar|\.gz|архив/i.test(value))
        evidence.add("архив");
    ...
    if (imagingVendorPattern.test(value) ||
    ...
    if (legacyMisTextPattern.test(value))
        evidence.add("старая МИС");
    if (/\\\\|smb|network share|сетев/i.test(value))
        evidence.add("сетевая папка");
    return Array.from(evidence);
}
function safeLegacySourceAlias(kind, sourceRef) {
    if (!sourceRef)
        return null;
    if (/^(?:browser-local|workstation-profile|workstation-signal|migration-source):[a-f0-9]{8,12}$/i.test(sourceRef))
        return sourceRef;
    return `${legacySourceTitles[kind]} #${migrationFingerprint(sourceRef).toUpperCase()}`;
}
function safeLegacySourceEvidence(source) {
    const alias = source.safeSourceAlias ??
        (source.sourceRef
...
function buildLegacySources(lines) {
...
async function buildSmartImportPreview(orgId, input) {
...
function getLineClassificationRows(preview) {
...
function getPatientPreviewRows(preview) {
...
function getImagingPreviewRows(preview) {
...
function getClinicSuggestionRows(preview) {
...
function getPublicLookupRows(preview) {
...
function getLegacySourceRows(preview) {
...
function getParserNoteRows(preview) {
...
function buildSmartImportReportCsv(preview) {
    const header = [
        "section",
        "rowNumber",
        "status",
...
function smartImportSafeHandoffWarnings(count) {
    if (count <= 0)
        return "";
    return `${count} warning(s); details stay in internal preview`;
}
function smartImportSafeHandoffFingerprint(section, rowNumber, status, kind = "") {
    return migrationFingerprint(`${section}:${rowNumber}:${status}:${kind}`).toUpperCase();
}
function getSafeHandoffSummaryRow(preview) {
    return [
        "summary",
        1,
        "review",
        ...
        `lines ${preview.totalLines}; patient rows ${preview.patientPreview.totalRows}; imaging rows ${preview.imagingPreview.totalRows}; clinic fields ${preview.clinicSuggestion
            ? Object.keys(preview.clinicSuggestion.fields).length
            : 0}; legacy sources ${preview.legacySources.length}`,
        "",
...
function getSafeHandoffMigrationStepRows(preview) {
...
function getSafeHandoffPatientRows(preview) {
    return preview.patientPreview.rows.map((row) => [
        "patient_row",
        row.rowNumber,
        row.status,
...
function getSafeHandoffImagingRows(preview) {
    return preview.imagingPreview.rows.map((row) => {
...
function getSafeHandoffClinicSuggestionRows(preview) {
    if (!preview.clinicSuggestion)
        return [];
    return Object.entries(preview.clinicSuggestion.fields).map(([field, value]) => {
...
function getSafeHandoffPublicLookupRows(preview) {
    return preview.publicLookupTargets.map((target, index) => [
        "public_lookup",
        index + 1,
        "manual",
...
function getSafeHandoffLegacySourceRows(preview) {
    return preview.legacySources.map((source, index) => [
        "legacy_source",
        index + 1,
        source.automationLevel,
...
function getSafeHandoffParserNoteRows(preview) {
    return preview.parserNotes.map((note, index) => [
        "parser_note",
        index + 1,
        "info",
...
function getSafeHandoffPrivacyWarningRows(preview) {
...
function buildSmartImportSafeHandoffReportCsv(preview) {
    const rows = [
        [
            "section",
            "rowNumber",
...
function buildMigrationAutopilotReportCsv(plan) {
    const rows = [
        [
            "section",
            "order",
...
function safeSmartImportReportFilename(sourceName) {
...
function parseSmartImportPayload(schema, value, message) {
...
export async function registerSmartImportRoutes(app) {
    app.post("/api/imports/smart/preview", async (request, reply) => {
        if (!(await requireClinicalReadAccess(request, reply, "smart import preview")))
            return;
        const parsed = parseSmartImportPayload(smartImportRequestSchema, request.body, "Умный импорт не проверен: передайте непустой текст, таблицу или описание источника до 120000 символов.");
        if (!parsed.ok)
    ...
    app.post("/api/imports/smart/local-source-discovery", async (request, reply) => {
        if (!(await requireClinicalReadAccess(request, reply, "migration local source discovery")))
            return;
        const parsed = parseSmartImportPayload(migrationLocalSourceDiscoveryRequestSchema, request.body ?? {}, "Поиск старых источников не запущен: проверьте корни поиска и лимиты обхода.");
        if (!parsed.ok)
            return reply.code(400).send(parsed.response);
        const input = parsed.data;
    ...
    app.post("/api/imports/smart/local-source-workup", async (request, reply) => {
        if (!(await requireClinicalReadAccess(request, reply, "migration local source workup")))
            return;
        const parsed = parseSmartImportPayload(migrationLocalSourceWorkupRequestSchema, request.body, "План источника не построен: выберите источник из последнего поиска или безопасный код browser-local.");
        if (!parsed.ok)
            return reply.code(400).send(parsed.response);
        const input = parsed.data;
    ...
    app.post("/api/imports/smart/local-source-probe", async (request, reply) => {
        if (!(await requireClinicalReadAccess(request, reply, "migration local source probe")))
            return;
        const parsed = parseSmartImportPayload(migrationLocalSourceProbeRequestSchema, request.body, "Проверка источника не выполнена: выберите источник из последнего поиска или безопасный код browser-local.");
        if (!parsed.ok)
            return reply.code(400).send(parsed.response);
        const input = parsed.data;
    ...
    app.post("/api/imports/smart/migration-autopilot", async (request, reply) => {
        if (!(await requireClinicalReadAccess(request, reply, "migration autopilot")))
            return;
        const parsed = parseSmartImportPayload(migrationAutopilotRequestSchema, request.body ?? {}, "Автоплан миграции не построен: проверьте входные данные источников, клиники и лимиты поиска.");
        if (!parsed.ok)
            return reply.code(400).send(parsed.response);
        const input = parsed.data;
        const orgId = await resolveOrganizationId(request);
    ...
    app.post("/api/imports/smart/migration-autopilot/report.csv", async (request, reply) => {
        if (!(await requireClinicalReadAccess(request, reply, "migration autopilot report")))
            return;
        const parsed = parseSmartImportPayload(migrationAutopilotRequestSchema, request.body ?? {}, "Отчет миграции не создан: проверьте входные данные источников, клиники и лимиты поиска.");
        if (!parsed.ok)
            return reply.code(400).send(parsed.response);
        const input = parsed.data;
        const orgId = await resolveOrganizationId(request);
    ...
    app.post("/api/imports/smart/clinic-public-lookup", async (request, reply) => {
        if (!(await requireClinicalReadAccess(request, reply, "clinic public lookup")))
            return;
        const parsed = parseSmartImportPayload(clinicPublicLookupRequestSchema, request.body, "Поиск реквизитов не выполнен: передайте ИНН, ОГРН, КПП, название, адрес или лицензию клиники.");
        if (!parsed.ok)
            return reply.code(400).send(parsed.response);
        const input = parsed.data;
    ...
    app.post("/api/imports/smart/report.csv", async (request, reply) => {
        if (!(await requireClinicalReadAccess(request, reply, "smart import report")))
            return;
        const parsed = parseSmartImportPayload(smartImportRequestSchema, request.body, "Отчет умного импорта не создан: передайте непустой текст, таблицу или описание источника.");
        if (!parsed.ok)
            return reply.code(400).send(parsed.response);
        const input = parsed.data;
        const orgId = await resolveOrganizationId(request);
    ...
    app.post("/api/imports/smart/report.safe.csv", async (request, reply) => {
        if (!(await requireClinicalReadAccess(request, reply, "safe smart import handoff report")))
            return;
        const parsed = parseSmartImportPayload(smartImportRequestSchema, request.body, "Безопасный отчет умного импорта не создан: передайте непустой текст, таблицу или описание источника.");
        if (!parsed.ok)
            return reply.code(400).send(parsed.response);
        const input = parsed.data;
        const orgId = await resolveOrganizationId(request);
    ...
    app.post("/api/imports/smart/commit", async (request, reply) => {
        if (!(await requireClinicalMutationAccess(request, reply, "smart import commit")))
            return;
        const parsed = parseSmartImportPayload(smartImportRequestSchema, request.body, "Умный импорт не записан: повторно передайте ту же непустую выгрузку перед записью.");
        if (!parsed.ok)
            return reply.code(400).send(parsed.response);
        const input = parsed.data;
        const orgId = await resolveOrganizationId(request);
...
async function inspectMigrationDiscoveryFolder(item, input, queue, candidates, warnings) {
    let entries;
    try {
    ...
    const orderedEntries = [...entries].sort((left, right) => migrationDiscoveryEntryPriority(right, item.folderPath) -
    ...
    for (const entry of orderedEntries) {
        ...
        if (profilesForFile.length) {
            if (!firstProfileEvidencePath)
                firstProfileEvidencePath = fullPath;
    ...
    const profileMatches = Array.from(new Map([
        ...migrationWorkstationProfileMatches(`${item.folderPath} ${firstMatchPath}`),
        ...fileProfileMatches.values(),
    ].map((profile) => [profile.label, profile])).values());
    const matchedFiles = databaseFiles +
        dumpFiles +
        tableFiles +
        archiveFiles +
        dicomLikeFiles +
        imageFiles;
    const confidence = Math.min(1, hintScore +
        (databaseFiles ? 0.5 : 0) +
        (dumpFiles ? 0.42 : 0) +
        (tableFiles ? 0.28 : 0) +
        (archiveFiles ? 0.2 : 0) +
    ...
    const sourceKind = matchedFiles === 0 && hasGenericDataContainerHint && !profileMatches.length
        ? "unknown_legacy_source"
        : detectedSourceKind;
    const shouldUseFolderSource = sourceKind === "mis_database" && firstMatchPath
    ...
    const reasons = [];
    if (databaseFiles)
        reasons.push(`${databaseFiles} файлов старой базы`);
    if (dumpFiles)
        reasons.push(`${dumpFiles} файлов резервной копии`);
    if (tableFiles)
        reasons.push(`${tableFiles} табличных выгрузок`);
    if (archiveFiles)
        reasons.push(`${archiveFiles} архивов`);
    if (dicomLikeFiles)
        reasons.push(`${dicomLikeFiles} признаков КТ/снимков`);
    if (imageFiles)
        reasons.push(`${imageFiles} изображений`);
    if (hintScore > 0)
