...
const TOKEN = readFileSync('C:/Users/Admin/Documents/gitenv.txt', 'utf8').trim();
...
async function main() {
  ...
  for (const pr of prs) {
    ...
    // Attempt to merge
    try {
      execSync(`git merge --no-ff pr/${pr.number} -m "Merge PR #${pr.number}: ${pr.title.replace(/"/g, '\\"')}"`, { stdio: 'inherit' });
    } catch (e) {
      console.log(`\n🚨 Merge conflict detected for PR #${pr.number}! PAUSING SCRIPT.`);
      console.log("Please resolve the conflict manually, run 'git commit', and then re-run this script.");
    ...
    try {
      ...
      try { execSync(`git reset --hard HEAD~1`, { stdio: 'inherit' }); } catch(err) {}
      console.log(`Skipping PR #${pr.number} due to TS errors. Continuing to next PR...`);
      continue;
    }
