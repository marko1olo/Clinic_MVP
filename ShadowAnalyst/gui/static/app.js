...
document.addEventListener("DOMContentLoaded", () => {
	...
	const buildChunks = (items, maxLength, joiner) => {
		return items.reduce((acc, item) => {
			item = item.trim();
			if (!item) return acc;

			const last = acc[acc.length - 1];
			if (!last || (last + joiner + item).length > maxLength) {
				acc.push(item);
			} else {
				acc[acc.length - 1] = last + joiner + item;
			}
			return acc;
		}, []);
	};
	...
	const splitTextIntoSpeakableChunks = (text, maxLength) => {
		...
		const sentenceChunks = buildChunks(sentences, maxLength, " ");

		return sentenceChunks
			.flatMap((chunk) =>
				chunk.length > maxLength
					? buildChunks(chunk.split(/[,;]/), maxLength, ", ")
					: [chunk],
			)
			.filter((c) => c.length > 0);
	};
