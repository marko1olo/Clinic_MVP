...
async function buildClinicPublicLookup(input) {
	...
	const suggestions = uniqueClinicPublicLookupSuggestions(
		[...providerResult.suggestions, manualSuggestion].filter(Boolean),
	);
...
function migrationWorkupExtractableEntities(kind) {
	...
	if (kind === "xray_image_archive") return ["imaging", "patients"];
...
async function scanMigrationProbeDirectory(
	...
	warnings,
	vendorInputs,
	initialLatestModifiedAt,
	initialScannedFolders,
	initialScannedFiles,
) {
...
function smartImportSafeHandoffFingerprint(
	section,
	rowNumber,
	status,
	kind = "",
