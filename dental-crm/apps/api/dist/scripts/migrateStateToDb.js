 ...
 */
const tenantScopedTablesInDeletionOrder = [
    {
        name: "dente_telegram_outbox_delivery_receipts",
        table: schema.denteTelegramOutboxDeliveryReceipts,
    },
    {
        name: "dente_telegram_webhook_events",
        table: schema.denteTelegramWebhookEvents,
    },
...
function destructiveResetAuthorized() {
 ...
 */
async function countTenantRows(tx) {
    let total = 0;
    const byTable = [];
    for (const entry of tenantScopedTablesInDeletionOrder) {
        const rows = await tx
            .select({ found: sql `count(*)::int` })
            .from(entry.table);
 ...
 */
async function clearTenantData(tx, organizationId) {
    const { total, byTable } = await countTenantRows(tx);
    if (total === 0) {
        console.log(`Очистка: у арендатора ${organizationId} нет ни одной строки — удалять нечего.`);
        return;
    }
    console.log(`Очистка затронет ${total} строк арендатора ${organizationId}:`);
    for (const line of byTable)
        console.log(`   ${line}`);
    if (productionModeActive()) {
        throw new SeedRefusedError([
    ...
    console.log(`Разрешено переменной ${DESTRUCTIVE_RESET_ENV_NAME}=${DESTRUCTIVE_RESET_ENV_VALUE}. Удаляю...`);
    for (const entry of tenantScopedTablesInDeletionOrder) {
 ...
 */
function keepResolvable(rows, label, isResolvable) {
    const kept = rows.filter(isResolvable);
    const skipped = rows.length - kept.length;
    if (skipped > 0) {
        console.warn(`   пропущено ${skipped} из ${rows.length} (${label}): ссылка на отсутствующую строку`);
    }
    return kept;
}
async function seed() {
    ...
    if (!state) {
        throw new SeedRefusedError("Не удалось прочитать состояние JSON (см. DENTAL_STATE_FILE и .data/dental-crm-state.json).");
    }
    ...
    if (!organizationId || !UUID_PATTERN.test(organizationId)) {
        throw new SeedRefusedError(`В состоянии нет корректного clinicProfile.organizationId (получено: ${String(organizationId)}). ` +
    ...
    const clinicalRules = state.clinicalRules ?? [];
     ...
     */
    const staffRows = await Promise.all(staffMembers.map(async (staff) => {
    ...
    await withTenantCtx(organizationId, async (tx) => {
        ...
        const appointmentRows = keepResolvable(appointments, "appointments", (appointment) => (appointment.patientId === null ||
            ...
            (appointment.chairId === null || chairIds.has(appointment.chairId))).map((appointment) => ({
            id: appointment.id,
            organizationId,
        ...
        const visitRows = activeVisit && patientIds.has(activeVisit.patientId)
            ? [
                {
                    id: activeVisit.id,
                    organizationId,
                    ...
                    appointmentId: activeVisit.appointmentId &&
                        appointmentIds.has(activeVisit.appointmentId)
        ...
        if (activeVisit && visitRows.length === 0) {
            console.warn(`   активный приём ${activeVisit.id} пропущен: пациента ${activeVisit.patientId} нет в состоянии`);
        }
        ...
        const documentRows = keepResolvable(documents, "generated_documents", (document) => patientIds.has(document.patientId)).map((document) => ({
            id: document.id,
            organizationId,
             ...
             */
            visitId: document.visitId && visitIds.has(document.visitId)
                ? document.visitId
                : null,
            kind: document.kind,
        ...
        if (rulesWithSlugId.length > 0) {
            console.warn(`   у ${rulesWithSlugId.length} из ${clinicalRules.length} правил идентификатор не UUID ` +
        ...
        const paymentRows = keepResolvable(payments, "payments", (payment) => patientIds.has(payment.patientId)).map((payment) => ({
            id: payment.id,
            organizationId,
            patientId: payment.patientId,
            visitId: payment.visitId && visitIds.has(payment.visitId)
                ? payment.visitId
                : null,
            documentId: payment.documentId && documentIds.has(payment.documentId)
                ? payment.documentId
                : null,
