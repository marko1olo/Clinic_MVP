  ...
  try {
    ...
    if (response.success) {
      ...
      const mergeTable = async (
        ...
        // 1. Fetch all local records matching cloud record IDs in one query
        const recordIds = cloudRecords.map((r) => r.id);
        const localMatches = await db
          .select()
          .from(table)
          .where(inArray(table.id, recordIds));

        // 2. Map them by ID for O(1) lookups
