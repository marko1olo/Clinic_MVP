...
function extractTime(text) {
    ...
    if (m) {
        ...
        const min = m[2];
        if (text.includes("дня") && h < 12)
            h += 12;
        if (text.includes("вечера") && h < 12)
            h += 12;
...
function extractDate(text) {
    ...
    for (const [word, wd] of Object.entries(WEEKDAYS)) {
        if (text.includes(word))
            return { targetWeekday: wd };
    }
    return null;
}
function extractPatientName(text, actionRegexStr) {
...
function extractCost(text) {
    ...
    if (mWord) {
        let sum = 0;
        let hasHalf = false;
        for (const w of mWord[1].split(/\s+/)) {
            if (w.startsWith("полов"))
                hasHalf = true;
        ...
        if (sum > 0) {
            if (hasHalf)
                sum += 0.5;
            if (mWord[2].startsWith("тыс") ||
                mWord[1].includes("тысяч") ||
                mWord[1].includes("тыс")) {
                sum *= 1000;
            }
            return sum;
        }
...
function extractEmkSections(text, updates) {
    ...
    for (let i = 0; i < tokens.length; i++) {
        ...
        if (!token || token.length === 1)
            continue; // Skip empty tokens or single non-word delimiters
        ...
        if (isKeyword) {
            if (lowerToken.startsWith("жалоб"))
                matchedSection = "complaint";
            else if (lowerToken.startsWith("анамнез"))
                matchedSection = "anamnesis";
            else if (lowerToken.startsWith("объектив") ||
                lowerToken.startsWith("осмотр"))
                matchedSection = "objectiveStatus";
            else if (lowerToken.startsWith("диагноз"))
                matchedSection = "diagnosis";
            else if (lowerToken.startsWith("лечен") ||
...
function expandToothRanges(text) {
    const allTeeth = new Set();
    if (text.includes("все зубы") ||
        text.includes("обе челюсти") ||
        text.includes("санаци")) {
        for (let i = 11; i <= 18; i++)
            allTeeth.add(i.toString());
        for (let i = 21; i <= 28; i++)
            allTeeth.add(i.toString());
        for (let i = 31; i <= 38; i++)
            allTeeth.add(i.toString());
...
export function parseDictationLocally(transcript, context) {
    ...
    if (context === "schedule" || text.includes("пациент")) {
        if (text.match(/(отмени|удали).*(запись|прием)|отмени/)) {
            ...
            if (!pName)
                return null; // Force LLM fallback if we can't extract the name confidently
        ...
        if (text.match(/(запиши|записать|создай запись|запись для|новый пациент)/)) {
            ...
            if (!pName)
                return null; // Without a patient name, scheduling is impossible locally. Fallback to LLM.
            return {
                action: action,
                payload: {
                    patientName: pName,
    ...
    if (openCardMatch && (openCardMatch[1]?.length ?? 0) > 3) {
        return {
            action: "open_card",
            payload: { patientName: openCardMatch[1]?.trim() },
        };
    }
    ...
    if (context === "visit") {
        ...
        if (cost)
            emkUpdates.costRub = cost;
        extractEmkSections(text, emkUpdates);
        const hasStructuredEmk = !!(emkUpdates.complaint ||
                        ...
                        if (!hasStructuredEmk) {
                            if (state === "treatment") {
                                if (!emkUpdates.complaint)
                                    emkUpdates.complaint = clause.trim();
                                if (!emkUpdates.diagnosis)
                                    emkUpdates.diagnosis = keyword;
                            }
        ...
        if (!hasValidMatch && text.length > 10 && !hasStructuredEmk) {
            if (text.includes("жалоб"))
                emkUpdates.complaint = text;
            else if (text.includes("анамнез"))
                emkUpdates.anamnesis = text;
            else if (text.includes("диагноз"))
                emkUpdates.diagnosis = text;
            else
                return null; // If it's a completely generic string, let the LLM sort it out
            hasValidMatch = true;
        }
