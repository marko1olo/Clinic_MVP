...
function getComparisonUrls(cleanUrl) {
...
window.showToast = (message, type = "info") => {
	...
	if (type === "success") {
		iconSvg =
			'<svg viewBox="0 0 24 24" width="1em" height="1em" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path><polyline points="22 4 12 14.01 9 11.01"></polyline></svg>';
	} else if (type === "error") {
		iconSvg =
			'<svg viewBox="0 0 24 24" width="1em" height="1em" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><line x1="15" y1="9" x2="9" y2="15"></line><line x1="9" y1="9" x2="15" y2="15"></line></svg>';
	} else {
		iconSvg =
			'<svg viewBox="0 0 24 24" width="1em" height="1em" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><line x1="12" y1="16" x2="12" y2="12"></line><line x1="12" y1="8" x2="12.01" y2="8"></line></svg>';
	}
...
document.addEventListener("DOMContentLoaded", () => {
	...
	const _reportContent = document.getElementById("report-content");
	...
	const selectElevenlabsVoice = document.getElementById(
		"select-elevenlabs-voice",
	);
	const inputElevenlabsVoiceCustom = document.getElementById(
		"input-elevenlabs-voice-custom",
	);
	const elevenlabsApiKeyGroup = document.getElementById(
		"elevenlabs-api-key-group",
	);
	const elevenlabsVoiceIdGroup = document.getElementById(
		"elevenlabs-voice-id-group",
	);
	...
	btnSettings.addEventListener("click", async () => {
		try {
			...
			if (selectVoice)
				selectVoice.value = data.tts_voice || "ru-RU-DmitryNeural";
			...
			if (selectTtsProvider)
				selectTtsProvider.value = data.tts_provider || "edge";

			// Populate ElevenLabs keys textarea
			if (inputElevenlabsKeys) {
				const keysList =
					data.elevenlabs_api_keys ||
					(data.elevenlabs_api_key ? [data.elevenlabs_api_key] : []);
			...
			if (selectElevenlabsVoice) {
				...
				const preloadedValues = [
					"pNInz6obpgq54HWK483c",
					"21m00Tcm4TlvDq8ikWAM",
					"ErXwobaYiN019PkySvjV",
					"piTKgcLEGmPEe14mmc4w",
				];
	...
	btnSaveSettings.addEventListener("click", async () => {
		...
		const modelTierInput = document.getElementById("range-model-tier");
		const modelTier = modelTierInput ? parseInt(modelTierInput.value, 10) : 4;

		const googleKeys = inputGeminiKeys
			? inputGeminiKeys.value
					.split("\n")
					.map((k) => k.trim())
					.filter((k) => k.length > 0)
			: [];
		const groqKeys = inputGroqKeys
			? inputGroqKeys.value
					.split("\n")
					.map((k) => k.trim())
					.filter((k) => k.length > 0)
			: [];
		const elevenKeys = inputElevenlabsKeys
			? inputElevenlabsKeys.value
					.split("\n")
					.map((k) => k.trim())
					.filter((k) => k.length > 0)
			: [];
		...
		if (selectElevenlabsVoice) {
			if (selectElevenlabsVoice.value === "custom") {
				voiceId =
					(inputElevenlabsVoiceCustom
						? inputElevenlabsVoiceCustom.value.trim()
						: "") || "pNInz6obpgq54HWK483c";
			} else {
		...
		try {
			const res = await fetch("/api/settings", {
				...
				body: JSON.stringify({
					...
					comparison_slider: toggleSliderOption
						? toggleSliderOption.checked
						: true,
	...
	document.addEventListener("mousedown", (e) => {
		if (floatingToolsPanel && floatingToolsPanel.style.display === "flex") {
			if (
				!floatingToolsPanel.contains(e.target) &&
				!btnToggleTools.contains(e.target)
			) {
				floatingToolsPanel.style.display = "none";
			}
		}
		...
		if (currentImage) {
			try {
				...
				if (data.url) {
					...
					const showSlider =
						toggleAutoEnhance.checked &&
						(toggleSliderOption ? toggleSliderOption.checked : true);

					if (showSlider) {
						imgElement.style.display = "none";
						comparisonContainer.style.display = "flex";

						const { originalUrl, enhancedUrl } = getComparisonUrls(cleanUrl);
						...
						imgElement.src = `${cleanUrl}?t=${Date.now()}`;
					}
				}
			} catch (e) {
	...
	window.loadData = (imageUrl, reportText, summaryContent) => {
		...
		if (reportText && reportText.trim() !== "") {
			...
			fullReportText.innerHTML =
				'<div class="empty-state"><p>Снимок загружен. Нажмите "Анализировать снимок" для запуска ИИ.</p></div>';
	...
	window.showLoader = () => {
		...
		statusBadge.classList.remove("active");
		fullReportText.innerHTML =
			'<div class="empty-state"><p>Ожидайте ответ от ИИ...</p></div>';
		summaryCard.style.display = "none";
	};
	...
	const tryOnlineTTS = (summary, report) => {
		...
		if (cleanSummary) {
			const provider =
				selectTtsProvider && selectTtsProvider.value === "elevenlabs"
					? "elevenlabs"
					: "edge";
	...
	const fallbackToLocalTTS = (text) => {
		...
		const ruVoice = voices.find(
			(v) => v.lang.includes("ru") || v.lang.includes("RU"),
		);
		if (ruVoice) {
	...
	setInterval(async () => {
		...
		try {
			...
			const cleanLatestImage = data.latest_image
				? data.latest_image.split("?")[0]
				: "";
			...
			if (
				cleanCurrentImage &&
				cleanLatestImage &&
				(cleanCurrentImage === cleanLatestImage ||
					cleanCurrentImage.replace("_enhanced", "") ===
						cleanLatestImage.replace("_enhanced", ""))
			) {
				if (
					data.latest_report !== currentReport ||
					data.latest_summary !== currentSummary
				) {
			...
			if (data.recent_scans && incomingTray) {
				const hasNewScans =
					data.recent_scans.length > previousRecentScansLength;
				...
				const activeScan = data.recent_scans.find((s) => {
					...
					return (
						sClean === cleanCurrentImage ||
						sClean.replace("_enhanced", "") ===
							cleanCurrentImage.replace("_enhanced", "")
					);
				});
				if (activeScan) {
					if (
						activeScan.report !== currentReport ||
						activeScan.summary !== currentSummary
					) {
			...
			if (data.is_processing) {
				...
				const cleanProcessingImage = data.processing_image
					? data.processing_image.split("?")[0]
					: "";

				const isCurrentProcessing =
					cleanProcessingImage &&
					(cleanCurrentImage === cleanProcessingImage ||
						cleanCurrentImage.replace("_enhanced", "") ===
							cleanProcessingImage.replace("_enhanced", ""));
	...
	document.addEventListener("paste", (e) => {
		...
		for (let i = 0; i < items.length; i++) {
			...
			if (item.kind === "file" && item.type.startsWith("image/")) {
				...
				const file = new File([blob], `clipboard_${Date.now()}.png`, {
					type: blob.type,
				});
				uploadFile(file);
				break;
	...
	if (btnSaveHistory) {
		btnSaveHistory.addEventListener("click", async () => {
			...
			const ageVal = patientAge.value ? parseInt(patientAge.value, 10) : null;
			...
			const payload = {
				...
				enhanced_image: currentImage.includes("_enhanced")
					? currentImage
					: currentImage.replace(/\.([a-zA-Z0-9]+)$/, "_enhanced.$1"),
				brightness: bVal,
				contrast: cVal,
				inverted: invVal,
				scale: scale,
			...
			try {
				...
				console.error(e);
				window.showToast(
					`Сбой подключения при сохранении: ${e.message}`,
					"error",
				);
			} finally {
	...
	const loadScan = (scan) => {
		...
		fetch("/api/active_scan", {
			...
			console.error("Failed to sync active scan to backend", err),
		);

		// Restore adjustments
		brightnessSlider.value =
			scan.brightness !== undefined ? scan.brightness : 100;
		...
		const sliderPos =
			scan.slider_position !== undefined ? scan.slider_position : 50.0;
		...
		try {
			...
			if (historyList) {
				...
				scans.forEach((scan) => {
					...
					else if (scan.patient_gender === "Женский")
						card.classList.add("female");
					else card.classList.add("other");

					const genderLabel =
						scan.patient_gender === "Мужской"
							? "М"
							: scan.patient_gender === "Женский"
								? "Ж"
								: "-";
					const genderClass =
						scan.patient_gender === "Мужской"
							...
							: scan.patient_gender === "Женский"
								? "female"
								: "other";
					const ageText = scan.patient_age
						? `${scan.patient_age} лет`
						: "возраст не указан";
					...
					delBtn.addEventListener("click", async (e) => {
						...
						if (confirm(`Удалить запись пациента ${scan.patient_name}?`)) {
							try {
								const delRes = await fetch(`/api/history/${scan.id}`, {
									method: "DELETE",
								});
	...
	document.addEventListener("mousedown", (e) => {
		if (historyDrawer?.classList.contains("active")) {
			if (
				...
				!btnOpenHistory.contains(e.target)
			) {
				historyDrawer.classList.remove("active");
			}
		}
