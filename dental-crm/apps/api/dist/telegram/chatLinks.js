...
function chatLinkClinicVisibility(clinicId) {
	...
	return or(
		eq(denteTelegramChatLinks.clinicId, scoped),
		isNull(denteTelegramChatLinks.clinicId),
	);
}
...
async function persistableClinicId(organizationId, clinicId) {
	...
	const [existing] = await db
		...
		.from(clinics)
		.where(
			and(eq(clinics.id, clinicId), eq(clinics.organizationId, organizationId)),
		)
		.limit(1);
...
export async function buildDenteTelegramChatLinkList(input) {
	...
	const { totals, filtered, rows } = await withTenantCtx(
		input.organizationId,
		async (tx) => {
			...
			const pageRows = await tx
				...
				.orderBy(
					desc(denteTelegramChatLinks.lastUpdateAt),
					desc(denteTelegramChatLinks.id),
				)
				.limit(limit)
				.offset(start);
