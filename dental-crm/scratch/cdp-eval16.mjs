...
async function run() {
  ...
  ws.on('open', () => {
    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
          send('Runtime.evaluate', {
             expression: 'try { import("/src/packages/shared/src/index.ts").then(shared => { console.log("SHARED IMPORTED"); try { shared.dashboardSchema.parse(window.testPayload); console.log("ZOD_OK"); } catch(e) { console.log("ZOD_ERR:", JSON.stringify(e)); } }).catch(e => console.log("SHARED_ERR:", e.message)); } catch(e) { console.log("TRY_ERR:", e.message) }'
          });
        }, 1000);
    }, 4000);
  });
