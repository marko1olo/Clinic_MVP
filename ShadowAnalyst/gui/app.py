...
def check_connection_loop():
    ...
    while True:
        ...
        try:
            ...
            if gemini_use_proxy:
                print(
                    '[VPN Monitor] Gemini API доступен напрямую. Переключаем на direct.')
            gemini_use_proxy = False
        except Exception:
            ...
            if ensure_tunnel():
                if check_tcp_socks5('generativelanguage.googleapis.com', 443):
                    if not gemini_use_proxy:
                        print(
                            '[VPN Monitor] Gemini напрямую недоступен. Переключаем на прокси.')
        ...
        try:
            ...
            if groq_use_proxy:
                print(
                    '[VPN Monitor] Groq API доступен напрямую. Переключаем на direct.')
            groq_use_proxy = False
        except Exception:
            # Если прямой доступ не удался, пробуем через SSH-туннель
            if ensure_tunnel():
                if check_tcp_socks5('api.groq.com', 443):
                    if not groq_use_proxy:
                        print(
                            '[VPN Monitor] Groq напрямую недоступен. Переключаем на прокси.')
                    groq_use_proxy = True
...
def groq_chat(api_key: str, **kwargs):
    ...
    try:
        ...
        if not groq_use_proxy:
            print(
                f"[VPN] Сбой прямого запроса Groq ({e}). Пробуем через SSH SOCKS5...")
            if ensure_tunnel():
...
def gemini_chat(api_key: str, **kwargs):
    ...
    try:
        ...
        if not gemini_use_proxy:
            print(
                f"[VPN] Сбой прямого запроса Gemini ({e}). Пробуем через SSH SOCKS5...")
            if ensure_tunnel():
...
def load_dotenv_manually():
    possible_env_paths = [
        ...
        # C:\Clinic_MVP\ShadowAnalyst\.env
        os.path.abspath(os.path.join(BASE_DIR, '..', '.env')),
        # C:\Clinic_MVP\ShadowAnalyst\gui\.env
    ...
    for path in possible_env_paths:
        if os.path.exists(path):
            print(
                f"[CONFIG] Found .env file at {path}. Loading environment variables.")
            try:
...
possible_config_paths = [
    CONFIG_FILE,
    # C:\Clinic_MVP\config.json
    os.path.abspath(os.path.join(BASE_DIR, '..', '..', 'config.json')),
    # C:\Clinic_MVP\ShadowAnalyst\config.json
    os.path.abspath(os.path.join(BASE_DIR, '..', 'config.json')),
    # C:\Clinic_MVP\ShadowAnalyst\gui\config.json
...
DEFAULT_CONFIG = {
    ...
    # Default tier is 2 (Qwen 3.6 + Llama 4) as recommended by user
    'model_tier': 2,
...
env_groq = [key.strip() for key in os.getenv(
    'GROQ_API_KEYS', '').split(',') if key.strip()]
config['groq_api_keys'] = merge_unique_keys(
    config.get('groq_api_keys', []), env_groq)

env_google = [key.strip() for key in os.getenv(
    'GOOGLE_API_KEYS', '').split(',') if key.strip()]
config['google_api_keys'] = merge_unique_keys(
    config.get('google_api_keys', []), env_google)
...
GROQ_VISION_MODEL = config.get(
    'groq_vision_model', DEFAULT_CONFIG['groq_vision_model'])
GOOGLE_API_KEYS = config.get(
    'google_api_keys', DEFAULT_CONFIG.get('google_api_keys', []))
GEMINI_VISION_MODEL = config.get('gemini_vision_model', DEFAULT_CONFIG.get(
    'gemini_vision_model', 'gemini-3.5-flash'))
...
TOPIC_XRAY_RESULT = config.get(
    'mqtt_topic_xray', DEFAULT_CONFIG['mqtt_topic_xray'])
...
app_state = {
    ...
    'elevenlabs_api_keys': config.get(
        'elevenlabs_api_keys', DEFAULT_CONFIG.get(
            'elevenlabs_api_keys', [
                'sk_7ec26dd2067a1110f4cd27a2d2ea18e9f536a7256d9065e4',
                'sk_f891c9ef32d99fa018749f081d00f84d092ea2120bf5db1a',
                'sk_f71e9d767c1b151745f78117aad2a9678c84ea23881e5544',
...
async def get_tts(text: str, provider: str = None):
    ...
    if provider == 'elevenlabs':
        ...
        if not keys:
            single_key = config.get('elevenlabs_api_key') or app_state.get(
                'elevenlabs_api_key')
            keys = [single_key] if single_key else []

        voice_id = config.get('elevenlabs_voice_id') or app_state.get(
            'elevenlabs_voice_id') or 'pNInz6obpgq54HWK483c'
        ...
        for api_key in keys:
            ...
            try:
                ...
                if r.status_code == 200:
                    ...
                    print(
                        f"ElevenLabs API failed with key {api_key[:8]}... (status={r.status_code}): {r.text}")
            except Exception as e:
                print(
                    f"ElevenLabs TTS synthesis failed with key {api_key[:8]}...: {e}")
...
def trigger_analysis():
    ...
    threading.Thread(target=process_analysis_only,
...
def run_ai_analysis(file_path, patient_info=None):
    ...
    for idx, (model_name, provider) in enumerate(models_with_providers):
        ...
        if not keys:
            print(
                f"Skipping model {model_name} ({provider}) - no API keys configured.")
            continue
        random.shuffle(keys)
        success = False
        for api_key in keys:
            try:
                ...
                if response.choices and len(response.choices) > 0:
                    ...
                    if val:
                        import re
                        val_cleaned = re.sub(
                            r'(?i)<think>.*?(?:</think>|$)', '', val, flags=re.DOTALL).strip()
                        ...
                        if len(val_cleaned) >= 80:
                            ...
                            success = True
                            print(
                                f"[AI Pass 1] Success with model {model_name} ({len(val_cleaned)} chars)")
                            break
                        else:
                            print(
                                f"[AI Pass 1] Model {model_name} returned cut-off or too short response ({len(val_cleaned)} chars). Trying next model.")
                    else:
                        print(
                            f"Key {api_key[:10]}... returned empty content for model {model_name}.")
            except Exception as e:
                print(
                    f"Error using key {api_key[:10]}... with model {model_name} ({provider}): {e}")
                last_err = e
                err_str = str(e).lower()
                if '429' in err_str or 'rate_limit' in err_str or 'rate limit' in err_str:
                    print(
                        f"[AI Pass 1] Rate limit (429) hit for model {model_name} with key {api_key[:10]}... trying next key.")
                    continue
                continue
        if success:
            break
    ...
    if model_tier == 3:
        ...
        critic_models = models_with_providers[first_model_idx +
                                              1:] + models_with_providers[:first_model_idx + 1]
    ...
    for model_name, provider in critic_models:
        ...
        if not keys:
            print(
                f"Skipping critic model {model_name} ({provider}) - no API keys configured.")
            continue
        random.shuffle(keys)
        success = False
        for api_key in keys:
                ...
                if response.choices and len(response.choices) > 0:
                    ...
                    if val:
                        ...
                        if len(val_cleaned) >= 100:
                            ...
                            success = True
                            print(
                                f"[AI Pass 2] Success with critic model {model_name} ({len(val_cleaned)} chars)")
                            break
                        else:
                            print(
                                f"[AI Pass 2] Critic model {model_name} returned too short response ({len(val_cleaned)} chars). Trying next model.")
                    else:
                        print(
                            f"Key {api_key[:10]}... returned empty content for model {model_name} in critic.")
            except Exception as e:
                print(
                    f"Error using key {api_key[:10]}... with model {model_name} in critic ({provider}): {e}")
                last_err_critic = e
                err_str = str(e).lower()
                if '429' in err_str or 'rate_limit' in err_str or 'rate limit' in err_str:
                    print(
                        f"[AI Pass 2] Rate limit (429) hit for model {model_name} in critic with key {api_key[:10]}... trying next key.")
                    continue
                continue
        if success:
            break
    ...
    summary_match = re.search(
        r'<summary>(.*?)</summary>', final_output, re.DOTALL | re.IGNORECASE)
    report_match = re.search(r'<report>(.*?)</report>',
    ...
    if not summary_match and not report_match:
        ...
        ) if summary_match else 'Снимок проанализирован ИИ-ассистентом.'
        report_text = report_match.group(
            1).strip() if report_match else first_report
...
def send_to_mqtt(image_path, report_text, filename, patient_name=None):
    ...
    try:
        ...
        client.disconnect()
        print(
            f"MQTT: Successfully published {filename} to {TOPIC_XRAY_RESULT}")
    except Exception as e:
...
def detect_gender_ru(name: str) -> str:
    ...
    male_names = ['никита', 'илья', 'данила', 'даниил', 'савва', 'миша', 'гриша', 'саша',
                  'петя', 'ваня', 'дима', 'леша', 'коля', 'юра', 'вова', 'толя', 'женя', 'сережа']
    female_names = ['маша', 'даша', 'лена', 'оля', 'света', 'наташа',
                    'катя', 'ира', 'таня', 'аня', 'юля', 'вера', 'надя', 'люба']
...
def parse_patient_from_file(file_path: str) -> dict:
    ...
    if file_path.lower().endswith('.dcm'):
        try:
            ...
            dcm_name = str(getattr(ds, 'PatientName', '')
                           ).replace('^', ' ').strip()
            if dcm_name:
...
def process_analysis_only(filename):
    ...
    enhanced_file_path = os.path.join(
        STATIC_DIR, 'uploads', f"{base_name}_enhanced{ext}")
    ...
    pat_name = 'Не указан'
    threading.Thread(target=send_to_mqtt, args=(
        file_path, report, orig_name, pat_name), daemon=True).start()
...
def process_new_xray(file_path):
    ...
    try:
        ...
        if not run_cv2_enhancement(static_img_path, enhanced_path):
            app_state[
                'error_message'] = f"Не удалось прочитать или улучшить изображение: {filename}"
            app_state['is_processing'] = False
            app_state['processing_image'] = None
            return
        ...
        threading.Timer(
            10.0, lambda: processing_files.discard(filename)).start()
...
def start_watchdog():
    ...
    for path in detected_paths:
        try:
            ...
            print(
                f"[Watchdog] Monitoring auto-detected directory (recursive): {path}")
        except Exception as e:
...
if __name__ == '__main__':
    ...
    threading.Thread(target=setup_tray, args=(
        global_window,), daemon=True).start()
