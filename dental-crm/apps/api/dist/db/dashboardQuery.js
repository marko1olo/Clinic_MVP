...
import { assertCriticalSlicesAvailable, hydrateDomainStateFromDb, } from "./domainStateHydration.js";
...
export async function getDashboardFromDb(organizationId) {
    ...
    if (report.warnings.length > 0) {
        console.warn(`[DashboardQuery] Клиника ${organizationId}: ${report.warnings.length} предупреждений гидратации:`, report.warnings);
    }
    ...
    if (!parsed.success) {
        console.error(`[DashboardQuery] Клиника ${organizationId}: сводка НЕ соответствует контракту dashboardSchema.`, "Ошибки валидации:", JSON.stringify(parsed.error.errors, null, 2), "Счётчики гидратации:", report.counts, "Пропущено строк:", report.skipped);
        // БЫЛО: возвращала невалидное всё равно. СТАЛО: бросаем — невалидную
        // сводку отдавать нельзя, клиент получит несходящиеся расчёты.
        throw new Error(`Сводка клиники ${organizationId} не прошла проверку контракта: ${parsed.error.errors.length} нарушений. ` +
            "Первое: " +
