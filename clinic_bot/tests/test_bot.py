...
class TestBotCmdStart(unittest.IsolatedAsyncioTestCase):
    ...
    @patch('bot.db.get_user_role')
    @patch('bot.db.add_user')
    async def test_cmd_start_db_error(self, mock_add_user, mock_get_user_role):
        # Arrange
        mock_get_user_role.side_effect = Exception("Database connection failed")
        ...
        mock_add_user.assert_not_called()
        self.mock_message.answer.assert_not_called()
