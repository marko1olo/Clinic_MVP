...
async function purgeAuditEvents(orgIds:string[]){await withSuperuserBypass(async tx=>{try{await tx.execute(sql`ALTER TABLE audit_events DISABLE TRIGGER ALL`);await tx.delete(auditEvents).where(inArray(auditEvents.organizationId,orgIds))}finally{await tx.execute(sql`ALTER TABLE audit_events ENABLE TRIGGER ALL`)}})}
