...
async def cmd_start(message: Message):
    ...
    if not role:
        await asyncio.to_thread(
            db.add_user, chat_id, "guest", message.from_user.full_name
        )  # По умолчанию гость для безопасности
        role = "guest"
...
async def cmd_status(message: Message):
    ...
    if now - _status_cache["timestamp"] > CACHE_TTL:
        _status_cache["doctors"] = len(
            await asyncio.to_thread(db.get_users_by_role, "doctor")
        )
        _status_cache["admins"] = len(
            await asyncio.to_thread(db.get_users_by_role, "admin")
        )
...
async def broadcast_photo(
    photo_bytes: bytes, caption: str, report_text: str, role: str = "doctor"
):
    ...
    max_len = 4000
    report_chunks = [
        report_text[i : i + max_len] for i in range(0, len(report_text), max_len)
    ]
    ...
    async def _send_to_user(chat_id):
        try:
            ...
            await bot.send_photo(
                chat_id, photo=input_file, caption=caption, parse_mode="Markdown"
            )
...
def handle_xray_result(topic, payload, loop):
    ...
    patient_str = (
        ...
        if patient_name and patient_name != "Неизвестен"
        else "Пациент: неизвестен (нет записи)"
    )
    ...
    if image_b64:
        ...
        asyncio.run_coroutine_threadsafe(
            broadcast_photo(photo_bytes, caption, report, role="doctor"), loop
        )
...
def handle_alert_admin(topic, payload, loop):
    if isinstance(payload, dict):
        payload_text = payload.get("text", str(payload))
    else:
...
def start_mqtt(loop: asyncio.AbstractEventLoop):
    ...
    client.on_disconnect = lambda c, ud, d, rc, p: log.warning(
