...
function detectKind(fileName, mimeType, buffer, rawText) {
    const ext = extensionOf(fileName);
    if (isLegacyDatabaseExtension(ext))
        return "legacy_database";
    if (isLegacyDumpExtension(ext))
        return "legacy_dump";
...
function legacyFileFormatLabel(fileName, kind) {
    const ext = extensionOf(fileName);
    if (ext === "fdb" || ext === "gdb")
        return "база Firebird/InterBase";
    if (ext === "mdb" || ext === "accdb")
        return "база Access";
    if (ext === "sqlite" || ext === "sqlite3" || ext === "db")
        return "база SQLite";
    if (ext === "dbf")
        return "таблица DBF/FoxPro";
    if (ext === "sql")
        return "SQL-дамп";
...
function stripXmlTags(xml) {
    return normalizeText(xmlDecode(xml
        .replace(/<w:tab\/>/g, "\t")
        .replace(/<\/w:tc>/g, "\t")
        .replace(/<\/w:p>/g, "\n")
        .replace(/<\/a:p>/g, "\n")
...
function stripHtml(value) {
    return normalizeText(xmlDecode(value
...
function zipText(entries, pathPattern) {
    return entries
...
function extractCellValue(cellXml, shared) {
...
function extractPdfHexText(source) {
    const output = [];
...
function extractPdf(buffer) {
    ...
    const literalText = Array.from(new Set([
    ...
    if (!text)
        warnings.push("pdf_text_not_extracted_may_be_scanned");
    return {
        text,
        tableCount: 0,
        warnings,
...
function collectSignals(kind, text, warnings) {
    const signals = [];
    const normalized = text.toLowerCase();
    if (kind === "zip")
        signals.push("архив");
    if (kind === "image")
        signals.push("изображение");
    if (kind === "pdf")
        signals.push("PDF");
    if (kind === "legacy_database")
        signals.push("старая база");
    if (kind === "legacy_dump")
        signals.push("резервная копия старой базы");
    if (warnings.includes("pdf_text_not_extracted_may_be_scanned"))
        signals.push("PDF может быть сканом");
    if (looksTabular(text))
        signals.push("похоже на таблицу");
    if (/[А-Яа-яЁё]/.test(text))
        signals.push("русский текст");
...
function routesFor(kind, text, target) {
...
export function extractDocument(input) {
    ...
    if (!tableCount && looksTabular(text))
        tableCount = 1;
