...
3. Сохраняйте все номера зубов (FDI), дозировки анестетиков и названия материалов.`,
	...
	if (typeof content !== "string") {
		throw new Error(
			"ИИ-генератор черновика вернул пустой или некорректный ответ.",
		);
	}
	let parsed;
	try {
	...
	return {
		complaint:
			typeof parsed.complaint === "string" ? parsed.complaint.trim() : null,
		anamnesis:
			typeof parsed.anamnesis === "string" ? parsed.anamnesis.trim() : null,
		objectiveStatus:
			typeof parsed.objectiveStatus === "string"
				? parsed.objectiveStatus.trim()
				: null,
		diagnosis:
			typeof parsed.diagnosis === "string" ? parsed.diagnosis.trim() : null,
		treatmentPlan:
			typeof parsed.treatmentPlan === "string"
				? parsed.treatmentPlan.trim()
				: null,
		_rawToothStates:
			typeof parsed.toothStates === "object" && parsed.toothStates !== null
				? parsed.toothStates
				: null,
	};
}
...
async function callOpenAiCompatibleVisitDraftWithKeyRotation(input) {
	...
	try {
		...
		console.warn(
			`[AI Draft Primary Fallback Triggered] Сбой основного провайдера: ${primaryError instanceof Error ? primaryError.message : primaryError}`,
		);
	}
	...
	for (const fallback of DENTAL_AI_CASCADING_MODELS) {
		...
		try {
			...
			if (!keyCandidate) {
				console.warn(
					`[AI Draft Cascade] Нет доступных ключей для провайдера ${fallback.provider}`,
				);
				continue;
			}
			...
			console.warn(
				`[AI Draft Cascade] Модель ${fallback.model} (${fallback.provider}) завершилась ошибкой: ${fallbackError instanceof Error ? fallbackError.message : fallbackError}`,
			);
		}
	}
	throw new Error(
		"Сбой ИИ-генератора черновика: все модели из каскада фоллбеков завершились ошибкой или лимиты исчерпаны.",
	);
}
...
export async function buildVisitDraftFromTranscript(
	...
	const baseline = buildRuleBasedVisitDraftFromTranscript(
		transcript,
		specialty,
		{
	...
	if (transcript.trim().length > config.maxTranscriptChars) {
		baseline.warnings.push(
			"ИИ-генерация черновика пропущена: текст длиннее настроенного лимита.",
		);
		return baseline;
	}
	try {
		...
		if (neural._rawToothStates && finalQuality) {
			...
			if (Object.keys(parsedStates).length > 0) {
				...
				finalQuality.detectedToothCodes = Array.from(
					new Set([
		...
		return {
			...
			warnings: [
				...
				...(baseline.warnings || []).filter(
					(w) => !w.includes("черновик собран по профилю специальности"),
				),
			],
		};
	} catch (error) {
		...
		baseline.warnings.push(
			`ИИ-генерация черновика не выполнена, применен локальный разбор: ${message}`,
		);
		return baseline;
	}
}
