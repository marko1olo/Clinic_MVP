...
class Store:
    ...
    def touch_dialog(
        ...
        chat_id: str,
        *,
        patient_message_at: datetime | None = None,
        our_message_at: datetime | None = None,
    ) -> None:
        ...
        """
        patient = (
            _iso(patient_message_at, "patient_message_at")
            if patient_message_at
            else None
        )
        ...
        with self._write() as conn:
            conn.execute(
                ...
                """,
                {
                    ...
                    "patient": patient,
                    "our": our,
                    "p_inc": 1 if patient else 0,
                    "o_inc": 1 if our else 0,
                },
            )
    ...
    def _set_until(self, chat_id: str, column: str, until: datetime | None) -> None:
        ...
        with self._write() as conn:
            ...
            conn.execute(
                f"UPDATE dialogs SET {column} = ? WHERE chat_id = ?", (stamp, chat_id)
            )
    ...
    def capture_phone(self, chat_id: str, phone_hash: str) -> None:
        ...
        with self._write() as conn:
            self._ensure_dialog(conn, chat_id, _iso(hours.now(), "now"))
            conn.execute(
                "UPDATE dialogs SET phone_hash = ? WHERE chat_id = ?",
                (phone_hash, chat_id),
            )
    ...
    def mark_followup_sent(self, chat_id: str, at: datetime) -> None:
        ...
        with self._write() as conn:
            ...
            conn.execute(
                ...
                "last_followup_at = ? WHERE chat_id = ?",
                (stamp, chat_id),
            )
    ...
    def resolve_draft(
        ...
        *,
        ...
        if action not in DRAFT_ACTIONS:
            raise ValueError(
                f"неизвестное решение {action!r}, " f"допустимы {sorted(DRAFT_ACTIONS)}"
            )
    ...
    def link_draft_message(self, draft_id: int, tg_message_id: int) -> None:
        ...
        with self._write() as conn:
            ...
            taken = conn.execute(
                "SELECT id FROM drafts WHERE tg_message_id = ?", (tg_message_id,)
            ).fetchall()
            if taken:
                raise ValueError(
                    f"сообщение {tg_message_id} уже занято черновиком "
                    f"{taken[0]['id']}"
                )
            conn.execute(
                "UPDATE drafts SET tg_message_id = ? WHERE id = ?",
                (tg_message_id, draft_id),
            )

    def draft_by_tg_message(self, tg_message_id: int) -> DraftRow | None:
        rows = self._read(
            "SELECT * FROM drafts WHERE tg_message_id = ?", (tg_message_id,)
        )
        return _draft(rows[0]) if rows else None
    ...
    def audit(
        self, event: str, *, chat_id: str | None = None, payload: dict | None = None
    ) -> None:
    ...
    def pending_inbox(self, limit: int = 20) -> list[InboxRow]:
        ...
        rows = self._read(
            ...
            "ORDER BY harvested_at, position, rowid LIMIT ?",
            (limit,),
        )
        return [_inbox(row) for row in rows]

    def chat_history(
        self, chat_id: str, *, before_position: int | None = None, limit: int = 40
    ) -> list[InboxRow]:
    ...
    def patient_texts(self, chat_id: str, limit: int = 20) -> tuple[str, ...]:
        ...
        rows = self._read(
            ...
            "ORDER BY position DESC, rowid DESC LIMIT ?",
            (chat_id, limit),
        )
        return tuple(row["text"] for row in rows)

    def mark_inbox_processed(
        self, external_id: str, at: datetime | None = None
    ) -> bool:
        ...
        with self._write() as conn:
            cursor = conn.execute(
                ...
                "WHERE external_id = ? AND processed_at IS NULL",
                (stamp, external_id),
            )
            return cursor.rowcount == 1
    ...
    def queue_outbox(
        ...
        *,
        ...
        if kind not in OUTBOX_KINDS:
            raise ValueError(
                f"неизвестный вид отправки {kind!r}, "
        ...
        with self._write() as conn:
            ...
            existing = conn.execute(
                "SELECT id FROM outbox WHERE draft_id = ?", (draft_id,)
            ).fetchall()
        if (
            not existing
        ):  # pragma: no cover - вставка без draft_id конфликтовать не может
    ...
    def mark_outbox_accounted(self, outbox_id: int) -> bool:
        with self._write() as conn:
            cursor = conn.execute(
                ...
                (_iso(hours.now(), "now"), outbox_id),
            )
            return cursor.rowcount == 1

    def account_outbox_batch(self, rows: list[OutboxRow]) -> int:
        """Учесть пачку отправленных сообщений за одну транзакцию.

        Выполняет логику touch_dialog, mark_followup_sent, audit и mark_outbox_accounted
        сразу для списка отправленных сообщений, используя executemany для скорости.
        """
        if not rows:
            return 0
        ...
        for row in rows:
            ...
            sent_stamp = _iso(row.sent_at, "row.sent_at")

            # 1. Параметры для диалога (из touch_dialog)
            dialog_params.append(
                {
                    "chat_id": row.chat_id,
                    "first": sent_stamp,
                    "patient": None,
            ...
            if row.kind == "followup":
                followup_params.append((sent_stamp, row.chat_id))

            # 3. Аудит
            payload = {"outbox_id": row.id, "kind": row.kind, "draft_id": row.draft_id}
            blob = json.dumps(payload, ensure_ascii=False, sort_keys=True, default=str)
            audit_params.append((now_stamp, "sent", row.chat_id, blob))

            # 4. Outbox accounting
            outbox_ids.append((now_stamp, row.id))
        ...
        with self._write() as conn:
            ...
            conn.executemany(
                "UPDATE outbox SET accounted_at = ? WHERE id = ? AND status = 'sent' AND accounted_at IS NULL",
                outbox_ids,
            )
