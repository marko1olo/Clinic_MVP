...
async function seed() {
	...
	await withTenantCtx(organizationId, async (tx) => {
		...
		console.log(
			`Организация: ${clinicProfile?.clinicName || "(без названия)"}`,
		);
		...
		const documentRows = keepResolvable(
			documents,
			"generated_documents",
			(document) => patientIds.has(document.patientId),
		).map((document) => ({
			id: document.id,
			organizationId,
		...
		const rulesWithSlugId = clinicalRules.filter(
			(rule) => !UUID_PATTERN.test(rule.id),
		);
		...
		const paymentRows = keepResolvable(payments, "payments", (payment) =>
			patientIds.has(payment.patientId),
		).map((payment) => ({
			id: payment.id,
			organizationId,
			patientId: payment.patientId,
			visitId:
				payment.visitId && visitIds.has(payment.visitId)
					? payment.visitId
					: null,
			documentId:
				payment.documentId && documentIds.has(payment.documentId)
					? payment.documentId
					: null,
	...
	console.log("");
	console.log(
		"Сид завершён. Схему создаёт 'npm run db:migrate', её надо запускать первой.",
	);
}
seed()
	.then(async () => {
