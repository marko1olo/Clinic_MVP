...
function pushMissingPreflightIssue(issues, xmlText, needle, code, message) {
    if (!xmlText.includes(needle)) {
        issues.push({ code, message });
    }
}
function validateSingleTagPair(issues, xmlText, openNeedle, closeNeedle, code, name) {
...
function validateKnd1151156XmlDraft(xmlText, expected) {
    const issues = [];
    pushMissingPreflightIssue(issues, xmlText, '<?xml version="1.0" encoding="UTF-8"?>', "xml-declaration", "нет декларации XML UTF-8");
    pushMissingPreflightIssue(issues, xmlText, `<Файл ИдФайл="${xml(expected.fileName)}"`, "file-id", "ИдФайл не совпадает с именем выгрузки");
    pushMissingPreflightIssue(issues, xmlText, `ВерсФорм="${FNS_MEDICAL_EXPENSE_XML_VERSION}"`, "format-version", `ВерсФорм должна быть ${FNS_MEDICAL_EXPENSE_XML_VERSION}`);
    pushMissingPreflightIssue(issues, xmlText, `<Документ КНД="${FNS_MEDICAL_EXPENSE_XML_KND}"`, "document-knd", `Документ/@КНД должен быть ${FNS_MEDICAL_EXPENSE_XML_KND}`);
    ...
    pushMissingPreflightIssue(issues, xmlText, `ПрПациент="${expected.samePatientFlag}"`, "same-patient-flag", "флаг налогоплательщик/пациент не совпадает с платежом");
...
function compactDocumentNumber(document) {
...
function isoToRuDate(value) {
    ...
    if (explicit)
        return `${explicit[3]}.${explicit[2]}.${explicit[1]}`;
...
function identityDocumentKindCode(value) {
    ...
    if (normalized.includes("свидетельств") && normalized.includes("рожд"))
        return "03";
    if (normalized.includes("военн"))
        return "07";
    if (normalized.includes("вид на жительство"))
        return "12";
    if (normalized.includes("иностран"))
        return "10";
    if (normalized.includes("паспорт"))
        return "21";
    return "91";
}
function identityDocumentNumber(value) {
...
function identityDocumentXml(value) {
...
function payerPersonXml(payment) {
    ...
    if (!payerName || !identifier || !payerBirthDate)
        return null;
...
function patientPersonXml(patient) {
...
function signerXml(profile) {
    const signer = nameParts(profile.signatoryName || profile.legalName || profile.clinicName);
    return signer ? `<Подписант ПрПодп="1">${fioXml(signer)}</Подписант>` : null;
}
function requireTaxOfficeCode(value) {
...
export function buildKnd1151156Xml(document, patient, context) {
    ...
    if (!document.issuedAt) {
        return {
            ok: false,
            statusCode: 409,
            error: "У документа нет даты выдачи, а в XML для налоговой она обязательна: по ней налоговая " +
