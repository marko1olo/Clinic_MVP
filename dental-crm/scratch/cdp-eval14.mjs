...
async function run() {
  ...
  ws.on('open', () => {
    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
          send('Runtime.evaluate', {
             expression: 'try { import("/src/useAppLogic.tsx").then(logic => { console.log("LOGIC IMPORTED"); }).catch(e => console.log("IMPORT_ERR:", e.message)); } catch(e) { console.log("TRY_ERR:", e.message) }'
          });
        }, 1000);
    }, 4000);
  });
