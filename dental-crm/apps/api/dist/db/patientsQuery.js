...
import { buildPatientLedgers, MoneyPrecisionError, patientAccountBalanceKopecks, QuantityContractError, rublesFromKopecks, } from "../money/patientDebt.js";
 ...
 */
async function patientAccountBalancesRub(organizationId, patientIds) {
    ...
    const chargesByPatient = new Map();
    for (const row of chargeRows) {
        const bucket = chargesByPatient.get(row.patientId);
        if (bucket)
            bucket.push(row);
        else
            chargesByPatient.set(row.patientId, [row]);
    }
    ...
    for (const row of paymentRows) {
        ...
        if (bucket)
            ...
            paymentsByPatient.set(row.patientId, [row]);
    }
    for (const patientId of patientIds) {
        ...
        try {
            ...
            if (error instanceof MoneyPrecisionError ||
                error instanceof QuantityContractError) {
                console.error(`[patientsQuery] Сальдо пациента ${patientId} (клиника ${organizationId}) не рассчитано: ${error.message} ` +
...
export function rowToPatient(p, balanceRub = null) {
    ...
    try {
        ...
        if (input.familyGroupId !== undefined) {
            ...
            const [current] = await db
                ...
                .limit(1);
            if (current?.familyGroupId &&
                input.familyGroupId !== null &&
                input.familyGroupId !== current.familyGroupId) {
                throw new Error("Пациент уже состоит в другой семейной группе. Сначала отвяжите его (familyGroupId: null), затем привяжите к новой.");
            }
            if (input.familyGroupId !== null) {
                const [family] = await db
                    .select({ id: schema.familyGroups.id })
                    .from(schema.familyGroups)
                    .where(and(eq(schema.familyGroups.id, input.familyGroupId), eq(schema.familyGroups.organizationId, organizationId)))
                    .limit(1);
                if (!family) {
...
export async function updatePatientAdministrativeProfileInDb(organizationId, patientId, input) {
    ...
    try {
        ...
        if (!current)
            return null;
        const existingProfile = current.administrativeProfile &&
        ...
        const [updated] = await db
            .update(schema.patients)
            .set({
            administrativeProfile: mergedProfile,
            updatedAt: new Date(),
        })
