 ...
 */
async function readDicomIdentity(filePath) {
    let handle = null;
    try {
...
async function fileCarriesRequestedUids(filePath, studyUid, seriesUid, instanceUid) {
    const identity = await readDicomIdentity(filePath);
    if (!identity)
        return false;
    return (identity.studyUid === studyUid &&
 ...
 */
function sampleDicomOwnerOrganizationId() {
    const configured = process.env[SAMPLE_DICOM_ORGANIZATION_ID_ENV]?.trim();
    if (!configured || !UUID_SHAPE.test(configured))
        return null;
    return configured;
}
/**
 ...
 */
async function resolveInstanceFilePath(organizationId, studyUid, seriesUid, instanceUid) {
    ...
    eq(schema.imagingInstances.organizationId, organizationId), eq(schema.imagingSeries.organizationId, organizationId), eq(schema.imagingStudies.organizationId, organizationId), eq(schema.imagingStudies.dicomStudyUid, studyUid), eq(schema.imagingSeries.dicomSeriesUid, seriesUid), eq(schema.imagingInstances.dicomSopInstanceUid, instanceUid)))
        .limit(1);
    if (instanceRow?.storagePath)
        return path.resolve(instanceRow.storagePath);
    const [studyRow] = await db
        .select({ storagePath: schema.imagingStudies.storagePath })
        .from(schema.imagingStudies)
        .where(and(eq(schema.imagingStudies.organizationId, organizationId), eq(schema.imagingStudies.dicomStudyUid, studyUid), isNotNull(schema.imagingStudies.storagePath)))
        .limit(1);
    ...
    if (sampleOwnerOrganizationId !== null &&
...
export async function registerDicomwebRoutes(app) {
    ...
    { config: { tenantTxSelfManaged: true } }, async (request, reply) => {
        ...
        try {
            ...
            request.log.error({ err: statError, organizationId, studyUid, seriesUid, instanceUid }, "[dicomweb] Файл объекта числится в базе, но не читается с диска");
            return reply.code(404).send({
