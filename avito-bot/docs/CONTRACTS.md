...
- Все деньги — целые рубли (`int`). Все моменты времени — `datetime` c таймзоной
  `Europe/Samara`, берётся через `brain/gate/hours.py: tz()`.
...
- **Наши собственные номера не считаются ПД** и не скрабятся: `+7 846 922-99-26`
  и `+7 927 712-99-26` берутся из `clinic-facts.json`, не из константы в коде.
