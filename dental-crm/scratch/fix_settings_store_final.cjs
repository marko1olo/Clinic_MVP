...
let storeCode = fs.readFileSync(path.join(__dirname, '..', 'apps', 'web', 'src', 'store', 'settingsStore.ts'), 'utf8');
...
// 3. Update initialUiPreferences usages
storeCode = storeCode.replace(/initialUiPreferences\./g, 'loadUiPreferences().');
...
fs.writeFileSync(path.join(__dirname, '..', 'apps', 'web', 'src', 'store', 'settingsStore.ts'), storeCode);
