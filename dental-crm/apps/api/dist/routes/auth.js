...
export async function registerAuthRoutes(app) {
	...
	app.post("/api/auth/clinic/set-password", async (request, reply) => {
		...
		if (!targetOrganizationId) {
			return reply
				.code(400)
				.send({ error: "ValidationError", message: "Не указана организация." });
		}
		if (
			isOrgAdmin &&
			body.organizationId &&
	...
	app.post("/api/auth/login", async (request, reply) => {
		...
		const isDemoUserLogin =
			demoLoginAllowed() &&
			(loginEmail === "doctor@clinic.com" || loginEmail === "admin@clinic.ru");
		let user = null;
		try {
		...
		const [userOrg] = await withTenantCtx(user.organizationId, async (tx) =>
			tx
