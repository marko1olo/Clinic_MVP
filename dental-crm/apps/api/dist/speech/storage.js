...
const durableRecordingJobKind = "voice_transcription";
...
function trimSpeechTranscriptionChunkRetention() {
    ...
    for (const chunk of speechTranscriptionChunks) {
        ...
        const overCap = !retainedByOrganization
...
function mergeDurableAndCachedChunks(storedChunks, cachedChunks) {
    ...
    for (const chunk of cachedChunks) {
        ...
        merged.set(chunk.chunkIndex, !stored || shouldReplaceSpeechTranscriptionChunk(stored, chunk)
            ? chunk
            : stored);
    }
    return Array.from(merged.values()).sort((left, right) => left.chunkIndex - right.chunkIndex ||
        left.createdAt.localeCompare(right.createdAt));
}
/**
...
function withoutDurableFailureWarnings(chunks) {
    return chunks.map((chunk) => chunk.warnings.some((warning) => warning.startsWith(durableWriteFailureWarningPrefix))
        ? {
            ...chunk,
            warnings: chunk.warnings.filter((warning) => !warning.startsWith(durableWriteFailureWarningPrefix)),
        }
        : chunk);
}
...
export function speechDurableRestoreState() {
    return {
        ...
        nextRetryAt: speechRestoreRetryAtMs > 0
...
async function restoreSpeechTranscriptionChunks() {
  ...
  `));
    ...
    for (const row of restored.rows ?? []) {
        ...
        if (cachedChunkCount + admitted.length > chunkBudget ||
