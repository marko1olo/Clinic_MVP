...
function selectedProvider() {
	...
	return (
		...
		(rawProvider in providerLabels ? rawProvider : "none")
	);
}
...
export class SpeechAsyncJobTimeoutError extends Error {
	...
	constructor(input) {
		super(
			`${input.providerLabel}: задание распознавания не завершилось за ${Math.round(input.waitedMs / 1000)} сек. (опросов ${input.pollCount}).`,
		);
...
function normalizeSpeechChunkTimings(input) {
	...
	if (
		...
		input.warnings.push(
			`Длительность аудиофрагментов нормализована до ${Math.round(minChunkMs / 1000)}-${Math.round(maxChunkMs / 1000)} сек.; рекомендовано ${Math.round(recommendedChunkMs / 1000)} сек.`,
		);
	}
...
export function getSpeechGatewayStatus() {
	...
	const maxChunkBytes = numberFromEnv(
		"DENTAL_SPEECH_MAX_CHUNK_BYTES",
		6_000_000,
	);
...
function decodeBase64Audio(value, maxChunkBytes) {
	...
	if (buffer.byteLength > maxChunkBytes) {
		throw new SpeechChunkPayloadError(
			`Аудиофрагмент слишком большой для текущих настроек (${Math.ceil(buffer.byteLength / 1024 / 1024)} МБ из ${Math.ceil(maxChunkBytes / 1024 / 1024)} МБ). Запишите короче или дождитесь отправки очереди.`,
		);
	}
	return buffer;
}
	...
	try {
		while (pollCount < policy.maxAttempts) {
			...
			try {
				...
				console.warn(
					`[SpeechGateway] ${providerLabel}: опрос задания N ${pollCount} не прошёл (${speechProviderFailureReason(pollError)}); задание живо, ожидание продолжается, запас неудачных опросов ${consecutivePollFailures}/${policy.failureTolerance}.`,
				);
				continue;
			}
