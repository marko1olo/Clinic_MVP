...
export async function registerClinicalRoutes(app) {
    ...
    app.post("/api/clinical/phase-completions", async (request, reply) => {
        ...
        if (!orgId)
            return;
        const body = (request.body && typeof request.body === "object" ? request.body : {});
        const patientId = typeof body.patientId === "string" && UUID_PATTERN.test(body.patientId)
            ? body.patientId
            : null;
        ...
        const toothCodesValid = toothCodesRaw === undefined ||
            (Array.isArray(toothCodesRaw) &&
                toothCodesRaw.every((c) => typeof c === "string"));
        const notesValid = body.notes === undefined ||
            body.notes === null ||
