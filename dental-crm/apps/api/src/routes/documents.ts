...
export async function signedMedicalSourceVisitsAreValid(
  ...
  const appointmentIdsToFetch = Array.from(new Set(visits.map((v) => v.appointmentId).filter((id): id is string => id !== null)));
  const appointments = appointmentIdsToFetch.length > 0 ? await getAppointmentsByIdsInDb(document.organizationId, appointmentIdsToFetch) : [];
