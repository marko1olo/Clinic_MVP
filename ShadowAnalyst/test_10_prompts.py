...
API_KEYS = [key.strip() for key in os.getenv(
    "GROQ_API_KEYS", "").split(",") if key.strip()]
_GROQ_CLIENTS = {key: AsyncOpenAI(
    api_key=key, base_url="https://api.groq.com/openai/v1", max_retries=0) for key in API_KEYS}
...
def style_1_neon(draw, img, x1, y1, x2, y2, name):
    ...
    draw_text_with_bg(draw, f" 1. Неон: {name} ", x1, max(
        0, y1 - 45), (0, 255, 255))
...
def style_4_ellipse(draw, img, x1, y1, x2, y2, name):
    ...
    draw_text_with_bg(draw, f"4. Овал: {name}", x1, max(
        0, y1 - 45), (50, 255, 50))
...
def style_5_dashed(draw, img, x1, y1, x2, y2, name):
    ...
    for x in range(x1, x2, step * 2):
        nx = min(x + step, x2)
        draw.line([(x, y1), (nx, y1)], fill=color, width=4)
        draw.line([(x, y2), (nx, y2)], fill=color, width=4)
    for y in range(y1, y2, step * 2):
        ny = min(y + step, y2)
        draw.line([(x1, y), (x1, ny)], fill=color, width=4)
        draw.line([(x2, y), (x2, ny)], fill=color, width=4)
...
def style_6_overlay(draw, img, x1, y1, x2, y2, name):
    ...
    d.rectangle([x1, y1, x2, y2], fill=(255, 0, 0, 60),
                outline=(255, 0, 0, 200), width=3)
...
def style_7_police(draw, img, x1, y1, x2, y2, name):
    ...
    draw_text_with_bg(
        draw, f"7. Police: {name}", x1 + 2, max(0, y1 - 45), "yellow", bg_color=(0, 0, 0, 255))
