...
import { evaluateClinicalRulesInDb, createClinicalRuleInDb, updateClinicalRuleInDb, deleteClinicalRuleInDb } from "../db/clinicalQuery.js";
...
export async function registerClinicalRoutes(app) {
    ...
    app.post("/api/clinical/phase-completions", async (request, reply) => {
        if (!(await requireClinicalMutationAccess(request, reply, "clinical phase completion")))
            return;
        const orgId = requireOrganizationId(request, reply);
        if (!orgId)
            return;
        ...
        const toothCodesValid = toothCodesRaw === undefined || (Array.isArray(toothCodesRaw) && toothCodesRaw.every((c) => typeof c === "string"));
