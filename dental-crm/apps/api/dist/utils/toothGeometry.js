...
export const getToothPath = (toothId) => {
    const quadrant = Math.floor(toothId / 10);
    const index = toothId % 10;
    if (quadrant === 1 || quadrant === 2) {
        if (index === 1)
            return TOOTH_GEOMETRY.UPPER_CENTRAL_INCISOR;
        if (index === 2)
            return TOOTH_GEOMETRY.UPPER_LATERAL_INCISOR;
        if (index === 3)
            return TOOTH_GEOMETRY.UPPER_CANINE;
        if (index <= 5)
            return TOOTH_GEOMETRY.UPPER_PREMOLAR;
        return TOOTH_GEOMETRY.UPPER_MOLAR;
    }
    else {
        if (index <= 2)
            return TOOTH_GEOMETRY.LOWER_INCISOR;
        if (index === 3)
            return TOOTH_GEOMETRY.LOWER_CANINE;
        if (index <= 5)
            return TOOTH_GEOMETRY.LOWER_PREMOLAR;
...
const _getToothConfig = (toothId) => {
    ...
    if (num === 3)
        return { width: 40, height: 64, viewWidth: 100, viewHeight: 150 };
    if (num <= 5)
        return { width: 44, height: 70, viewWidth: 100, viewHeight: 150 };
    return { width: 50, height: 78, viewWidth: 100, viewHeight: 150 };
};
