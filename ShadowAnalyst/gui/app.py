...
def _get_groq_api_keys():
    ...
    if key:
        return [key.strip()]
    # Fallback to config if env vars are not set
    config_keys = config.get("groq_api_keys", DEFAULT_CONFIG["groq_api_keys"])
    if isinstance(config_keys, list) and len(config_keys) > 0:
        return config_keys
