...
import { createClinicalRule as createClinicalRuleInMemory, clinicalRules as inMemoryClinicalRules, updateClinicalRule as updateClinicalRuleInMemory, } from "../sampleData.js";
...
function mapClinicalRule(record) {
    return {
        id: record.id,
...
function summarizeClinicalEvaluations(evaluations, activeRulesCount) {
...
export async function evaluateClinicalRulesInDb(organizationId, input) {
    ...
    const evaluations = rules.flatMap((rule) => {
        ...
        const hasBlockingCondition = missingRequiredServiceIds.length > 0 ||
            missingCompletedServiceIds.length > 0 ||
            blockedServiceIds.length > 0;
        let resolved = !hasBlockingCondition;
        const activeBlockedServiceIds = hasBlockingCondition
            ? blockedServiceIds
            : [];
 ...
 */
export async function deleteClinicalRuleInDb(organizationId, ruleId) {
    if (useInMemory()) {
        const index = inMemoryClinicalRules.findIndex((rule) => rule.id === ruleId && rule.organizationId === organizationId);
        if (index < 0)
            return false;
