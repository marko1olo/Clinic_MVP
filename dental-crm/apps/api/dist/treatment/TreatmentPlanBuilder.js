...
export class TreatmentPlanBuilder {
	 ...
	 */
	addItem(service, quantity, toothCode = null, discountRub = 0) {
		...
		if (toothCode && ["18", "28", "38", "48"].includes(toothCode)) {
			if (
				service.title.toLowerCase().includes("удаление") &&
				!service.title.toLowerCase().includes("сложное")
			) {
				console.warn(
					`[CLINICAL WARNING] Удаление зуба мудрости (${toothCode}) обычно требует сложного удаления. Выбран обычный прайс: ${service.title}`,
				);
	...
	validateTooth(toothCode, category, serviceTitle) {
		...
		if (tooth.status === "missing") {
			...
			throw new Error(
				`ОШИБКА ЛОГИКИ: Зуб ${toothCode} отсутствует. Действие '${serviceTitle}' невозможно.`,
			);
		}
	 ...
	 */
	convertToAct(itemIds) {
		const itemsToConvert = this.items.filter(
			(i) => itemIds.includes(i.id) && i.status !== "completed",
		);
