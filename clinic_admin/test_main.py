...
class TestMain(unittest.TestCase):
    ...
    def test_api_current_appointment_authenticated(self):
        ...
        os.environ['ADMIN_PASSWORD'] = 'admin'
        response = self.client.get(
            '/api/current_appointment', auth=('admin', 'admin'))
        self.assertEqual(response.status_code, 200)
        data = response.json()
    ...
    def test_add_appointment_authenticated(self):
        ...
        response_patient = self.client.post(
            '/patients/add', data={'name': 'Test Patient', 'phone': '1234567890'}, auth=('admin', 'admin'), follow_redirects=False)
        ...
        response = self.client.post(
            '/appointments/add', data=form_data, auth=('admin', 'admin'), follow_redirects=False)
        ...
        conn = get_connection()
        c = conn.cursor()
        c.execute('SELECT * FROM appointments WHERE patient_id = ? AND doctor = ?',
                  (patient_id, 'Dr. Smith'))
