  ...
  app.post('/api/auth/register', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    return reply.code(201).send({ ok: true, staffToken: token, organizationId: org.id, userId: user.id });
  });
  ...
  app.post('/api/auth/login', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    return reply.send({ ok: true, staffToken, user: { id: user.id, fullName: user.fullName, role: user.role, email: user.email } });
  });
  ...
  app.post('/api/auth/invites/create', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    return reply.send({ ok: true, inviteLink: \/auth/accept-invite?token=\\ });
  });
