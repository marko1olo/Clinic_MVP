...
export async function registerCommunicationRoutes(app) {
    app.post("/api/communications/tasks/complete", async (request, reply) => {
         ...
         */
        const organizationId = await requireResolvedOrganizationId(request, reply, "communication task complete");
        if (!organizationId)
            return;
        try {
    ...
    app.get("/api/settings/message-templates", async (request, reply) => {
        ...
        if (!orgId)
            return;
        const templates = await import("../db/messageTemplateCatalogsQuery.js").then((m) => m.getMessageTemplateCatalogs(orgId));
    ...
    app.post("/api/settings/message-templates", async (request, reply) => {
        ...
        const template = await import("../db/messageTemplateCatalogsQuery.js").then((m) => m.createMessageTemplateCatalog(orgId, parsedInput.data));
