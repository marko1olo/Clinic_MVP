    ...
    static async processOsteointegrationRecalls() {
        ...
        for (const item of readyForCrown) {
            ...
            if (new Date() >= healingDate) {
                // Trigger Recall
                await RecallScheduler.triggerRecall(item.patientId, item.toothNumber, item.planName, item.patientFullName, item.organizationId);
            }
        }
    }
    static async triggerRecall(patientId, toothNumber, planName, patientFullName, organizationId) {
