  ...
  ws.on('open', () => {
    ...
    setTimeout(() => {
        send('Runtime.evaluate', {
            expression: 'fetch("http://127.0.0.1:4100/api/dashboard").then(r => r.json()).then(d => { const dstr = JSON.stringify(d); console.log("DASHBOARD LEN:", dstr.length); console.log("DASH_DATA:", dstr.substring(0, 500)); })'
        });
    }, 4000);
  });
  ws.on('message', data => {
