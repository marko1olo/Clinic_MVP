 ...
 */
export async function captureElementAsImage(element: HTMLElement): Promise<{ dataUrl: string; width: number; height: number }> {
...
export async function generateSurgicalReportPdf({
  ...
  if (!containerElement) throw new Error("No container element provided for Surgical Report");

  const { dataUrl, width, height } = await captureElementAsImage(containerElement);
...
export async function generateTreatmentPlanPdf({
  ...
  if (odontogramElement) {
    const { dataUrl, width, height } = await captureElementAsImage(odontogramElement);
  ...
  phases.forEach((phase, index) => {
    ...
    if (phase.discount && phase.discount > 0) {
      ...
      pdf.text(discountText, pageWidth - 15 - pdf.getTextWidth(discountText), yCursor);
      pdf.setTextColor(0, 0, 0);
...
export type PdfDocumentType = 'surgical_report' | 'treatment_plan';
...
export async function generateClinicalPdf({
  ...
  filename,
  quality = 2
}: GenerateClinicalPdfOptions): Promise<void> {
  ...
  // Set rendering class to avoid clipping or scrollbars
  ...
  try {
    ...
    const pdf = new jsPDF({
      orientation: documentType === 'surgical_report' ? 'landscape' : 'portrait',
      unit: 'mm',
      format: 'a4'
    });
    ...
    const finalImgHeight = Math.min(imgHeight, pageHeight - 20);
