...
# State
app_state = {"is_processing": False, "latest_image": "", "latest_report": ""}
...
def process_new_xray(file_path):
    ...
    try:
        ...
        try:
            ...
            resp = requests.get(CRM_API_URL, timeout=3, auth=(admin_user, admin_pass))
        ...
        threading.Thread(
            target=send_to_mqtt,
            args=(file_path, report, filename, pat_name),
            daemon=True,
        ).start()
...
class XRayHandler(FileSystemEventHandler):
    ...
    def handle_event(self, event):
        ...
        if not event.src_path.lower().endswith(
            (".jpg", ".jpeg", ".png", ".bmp", ".webp", ".dcm")
        ):
            return

        # Start processing in a background thread to avoid blocking Watchdog
        threading.Thread(
            target=process_new_xray, args=(event.src_path,), daemon=True
        ).start()
