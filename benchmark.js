...
function runBenchmark() {
  ...
  const treatmentLineTotal = (item) => Math.max(0, item.unitPriceRub * item.quantity - item.discountRub);

  // Unoptimized
  const startUnoptimized = process.hrtime.bigint();
  const taxDeductionEligibleRubUnoptimized = activePlanItems.reduce((total, item) => {
  ...
  const taxDeductionEligibleRubOptimized = activePlanItems.reduce((total, item) => {
