...
export const issuedArchiveIntegrityError = "Архивная копия выданного документа отсутствует или не прошла проверку целостности.";
...
export async function readValidPdfFile(pdfPath) {
    try {
...
export async function renderIssuedHtmlToPdf(html) {
...
export function normalizedTaxpayerInn(value) {
...
export function annualTaxpayerScopesOverlap(left, right) {
...
export async function findIssuedDuplicateTaxCertificate(document, payments) {
    ...
    for (const candidate of allDocuments) {
        if (candidate.patientId !== document.patientId)
            continue;
        if (candidate.taxYear !== document.taxYear)
            continue;
        if (candidate.status !== "issued")
            continue;
        if (candidate.kind !== document.kind)
            continue;
        if (candidate.organizationId !== document.organizationId)
            continue;
...
export function taxSnapshotDocument(document, snapshot) {
    if (!snapshot)
        return document;
    return {
        ...document,
...
export function frozenTaxXmlClinicProfile(document, fallbackClinicProfile) {
    return document.taxXmlSourceSnapshot?.clinicProfile ?? fallbackClinicProfile;
}
export function frozenTaxXmlPayments(document, fallbackPayments) {
...
export function documentChainDateIsBlankOrValid(value) {
    const normalized = (value ?? "").trim();
...
export function medicalRecordExtractPeriodIsChronological(payload) {
    return documentChainDateRangeIsChronological(payload.periodStart, payload.periodEnd);
}
export function medicalRecordExtractDatesAreValid(payload) {
    return (documentChainDateIsBlankOrValid(payload.periodStart) &&
...
export function medicalRecordCopyRequestDatesAreValid(payload) {
    return (documentChainDateIsBlankOrValid(payload.periodStart) &&
        documentChainDateIsBlankOrValid(payload.periodEnd) &&
        documentChainDateIsBlankOrValid(payload.requestedAt) &&
        documentChainDateRangeIsChronological(payload.periodStart, payload.periodEnd));
}
export function outpatientMedicalCard025uDatesAreValid(payload) {
    const dates = [
        payload.openedAt,
        payload.periodStart,
        payload.periodEnd,
...
export function medicalDocumentReleaseReceiptDatesAreValid(payload) {
    ...
    if (!documentChainDateIsBlankOrValid(payload.periodStart) ||
        !documentChainDateIsBlankOrValid(payload.periodEnd) ||
...
export function releaseSourceSnapshotSha256(document, scope) {
    return createHash("sha256")
...
export async function buildMedicalDocumentReleaseJournalEntry(document, issuedAt, signatureAttestation) {
...
export function taxCertificateExpectedApplicationForm(document) {
    if (document.kind === "tax_deduction_certificate")
        return "knd_1151156";
    if (document.kind === "tax_deduction_registry")
        return "knd_1151156";
    if (document.kind === "legacy_tax_deduction_certificate")
        return "legacy_2021_2023";
    return null;
}
export function normalizeTaxApplicationRelationship(value) {
    const normalized = normalizedDocumentChainValue(value);
    if (!normalized)
        return null;
    if (["self", "patient", "пациент", "сам", "сама", "лично"].includes(normalized))
        return "self";
    if (["spouse", "супруг", "супруга", "муж", "жена"].includes(normalized))
        return "spouse";
    if (["parent", "родитель", "мать", "отец", "мама", "папа"].includes(normalized))
        return "parent";
...
export async function hasIssuedTaxApplicationForCertificate(document) {
    ...
    return allDocuments.some((candidate) => {
        ...
        if (candidate.status !== "issued")
            return false;
        if (candidate.kind !== "tax_deduction_application")
            return false;
        if (candidate.organizationId !== document.organizationId)
            return false;
        if (candidate.patientId !== document.patientId)
            return false;
        if (application.requestedTaxYear !== document.taxYear)
            return false;
...
export async function findIssuedMedicalCopyRequestForRelease(document) {
    ...
    return (allDocuments.find((candidate) => {
        const request = candidate.payload?.medicalRecordCopyRequest;
        if (!request)
            return false;
        return (candidate.id === sourceRequestDocumentId &&
...
export async function hasIssuedMedicalCopyRequestForRelease(document) {
    return Boolean(await findIssuedMedicalCopyRequestForRelease(document));
}
export async function completedWorksActMatchesIssuedContract(document) {
    const allDocuments = await getDocumentsByPatientId(document.organizationId, document.patientId);
    ...
    return allDocuments.some((candidate) => {
        ...
        if (!contract)
            return false;
        if (candidate.id !== act.linkedContractDocumentId)
            return false;
        if (candidate.status !== "issued")
            return false;
        if (candidate.kind !== "paid_medical_services_contract")
            return false;
        if (candidate.organizationId !== document.organizationId)
            return false;
        if (candidate.patientId !== document.patientId)
            return false;
        if (candidate.visitId !== document.visitId)
            return false;
        return normalizedDocumentChainValue(act.contractNumber).includes(normalizedDocumentChainValue(contract.contractNumber));
    });
}
export async function medicalRecordExtractVisitDate(organizationId, visitId) {
...
export async function signedMedicalSourceVisitsAreValid(sourceVisitIds, document, periodStartRaw, periodEndRaw) {
    ...
    const appointmentIdsToFetch = new Set();
    ...
    for (const visitId of sourceVisitIds) {
        ...
        const visitDate = (comparableDocumentChainDate(appointment?.startsAt ? (typeof appointment.startsAt === "string" ? appointment.startsAt : appointment.startsAt.toISOString()) : null) ??
        ...
        if (visitDate === null)
            return false;
        if (periodStart !== null && visitDate < periodStart)
            return false;
        if (periodEnd !== null && visitDate > periodEnd)
            return false;
    }
    return true;
}
export async function medicalRecordExtractSourcesAreValid(payload, document) {
    return await signedMedicalSourceVisitsAreValid(payload.sourceVisitIds, document, payload.periodStart, payload.periodEnd);
}
export async function outpatientMedicalCard025uSourcesAreValid(payload, document) {
    if (!payload.sourceVisitIds.length || !payload.specialistVisitRecords.length)
        return false;
    const sourceIds = new Set(payload.sourceVisitIds);
    if (payload.specialistVisitRecords.some((record) => !sourceIds.has(record.sourceVisitId)))
        return false;
    return await signedMedicalSourceVisitsAreValid(payload.sourceVisitIds, document, payload.periodStart, payload.periodEnd);
}
