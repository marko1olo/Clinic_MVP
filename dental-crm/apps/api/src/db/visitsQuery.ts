...
export async function getVisitsByIdsInDb(organizationId: string, ids: string[]) {
  if (ids.length === 0) return [];
  return await db.select().from(schema.visits).where(and(eq(schema.visits.organizationId, organizationId), inArray(schema.visits.id, ids)));
}
