...
function drawData(modules, reserved, codewords) {
	const bits = [];
...
function modulesToSvg(modules) {
	...
	for (let y = 0; y < QR_SIZE; y += 1) {
		for (let x = 0; x < QR_SIZE; x += 1) {
			if (modules[y]?.[x])
				pathParts.push(
					`M${x + quietZone} ${y + quietZone}h1v1H${x + quietZone}z`,
				);
		}
	}
...
export function createTelegramQrSvg(payload) {
