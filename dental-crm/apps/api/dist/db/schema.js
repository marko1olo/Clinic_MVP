...
import { boolean, check, date, foreignKey, index, integer, jsonb, numeric, pgEnum, pgTable, real, text, timestamp, unique, uniqueIndex, uuid, varchar, } from "drizzle-orm/pg-core";
...
export const treatmentPlanScenarioStrategy = pgEnum("treatment_plan_scenario_strategy", ["urgent", "standard", "optimal", "phased", "maintenance"]);
export const treatmentPlanScenarioPriority = pgEnum("treatment_plan_scenario_priority", ["budget", "balanced", "clinical"]);
...
export const denteTelegramWebhookStatus = pgEnum("dente_telegram_webhook_status", ["processing", "processed", "duplicate", "ignored", "rejected"]);
 ...
 */
export const DEFAULT_CLINIC_MODE = "one_chair";
...
export const patients = pgTable("patients", {
    ...
    email: text("email"),
    notes: text("notes"),
    administrativeProfile: jsonb("administrative_profile").$type(),
...
export const visits = pgTable("visits", {
    ...
    return {
        visitPatientOrganizationUnique: unique("visits_id_patient_organization_unique").on(table.id, table.patientId, table.organizationId),
    };
});
...
export const clinicalRules = pgTable("clinical_rules", {
    ...
    ownerRole: text("owner_role").$type().notNull(),
...
export const payments = pgTable("payments", {
    ...
    fiscalReceipt: jsonb("fiscal_receipt").$type(),
...
export const generatedDocuments = pgTable("generated_documents", {
    ...
    taxXmlSourceSnapshot: jsonb("tax_xml_source_snapshot").$type(),
    taxXmlSnapshot: jsonb("tax_xml_snapshot").$type(),
    signatureAttestation: jsonb("signature_attestation").$type(),
    voidAttestation: jsonb("void_attestation").$type(),
    releaseJournalEntry: jsonb("release_journal_entry").$type(),
...
export const denteTelegramBotConfigs = pgTable("dente_telegram_bot_configs", {
    ...
    visualCardUrls: jsonb("visual_card_urls").$type(),
    ...
    patientLinkTokenTtlMinutes: integer("patient_link_token_ttl_minutes")
        .notNull()
        .default(120),
    appointmentReminderLeadTimesHoursJson: text("appointment_reminder_lead_times_hours_json")
        .notNull()
        .default("[24]"),
    ...
    return {
        denteTelegramBotConfigUnique: unique("dente_telegram_bot_configs_org_clinic_config_unique").on(table.organizationId, table.clinicId, table.botConfigId),
    };
});
...
export const communicationSettings = pgTable("communication_settings", {
    ...
    appointmentReminderLeadHoursJson: text("appointment_reminder_lead_hours_json")
        .notNull()
        .default("[24]"),
    /** Окно поиска, чтобы перезапуск не разослал напоминания о вчерашних приёмах. */
    appointmentReminderWindowMinutes: integer("appointment_reminder_window_minutes")
        .notNull()
        .default(90),
    createdAt: timestamp("created_at", { withTimezone: true })
        .notNull()
...
export const migrationQuarantineResolution = pgEnum("migration_quarantine_resolution", ["open", "resolved_imported", "resolved_merged", "discarded"]);
...
export const migrationRuns = pgTable("migration_runs", {
    ...
    skippedRows: integer("skipped_rows").notNull().default(0),
    mappingJson: jsonb("mapping_json").$type(),
...
export const migrationStagingRecords = pgTable("migration_staging_records", {
    ...
    rawJson: jsonb("raw_json").$type().notNull(),
    rawHash: text("raw_hash").notNull(),
    naturalKey: text("natural_key"),
    normalizedJson: jsonb("normalized_json").$type(),
    lineageJson: jsonb("lineage_json").$type(),
...
export const migrationEntityLinks = pgTable("migration_entity_links", {
    ...
    return {
        entityLinkSourceUnique: unique("migration_entity_links_source_unique").on(table.organizationId, table.entityKind, table.sourceSystem, table.sourceEntityId),
...
export const portalOtpCodes = pgTable("portal_otp_codes", {
    ...
    return {
        idxPortalOtpPatient: index("portal_otp_codes_patient_idx").on(table.organizationId, table.patientId, table.createdAt),
