...
function installmentRows(document, context) {
	...
	if (paidKopecks > 0) {
		rows.push(
			`<tr><td>Оплачено по сохраненным платежам</td><td>${escapeHtml(rub(rublesFromKopecks(paidKopecks)))}</td><td>оплачено</td></tr>`,
		);
	}
	if (remainingKopecks > 0) {
		...
		rows.push(
			`<tr><td>Следующий платеж до ближайшего визита</td><td>${escapeHtml(rub(rublesFromKopecks(firstPartKopecks)))}</td><td>план</td></tr>`,
		);
		if (secondPartKopecks > 0) {
			rows.push(
				`<tr><td>Финальный платеж до выдачи акта</td><td>${escapeHtml(rub(rublesFromKopecks(secondPartKopecks)))}</td><td>план</td></tr>`,
			);
		}
	}
	return rows.length
...
function paymentReceiptRows(payments) {
	...
	return payments
		.map(
			(payment) =>
				`<tr><td>${escapeHtml(paymentDateLabel(payment))}</td><td>${escapeHtml(paymentMethodForDocument(payment))}</td><td>${escapeHtml(rub(payment.amountRub))}</td><td>${escapeHtml(paymentReceiptLabel(payment))}</td><td>${escapeHtml(
					compactParts([
...
function treatmentPlan(document) {
	...
	if (payload) {
		const stageRows = payload.plannedStages
			.map(
				(stage) =>
					`<tr>${cell(stage.stageName)}${cell(stage.plannedServices)}${cell(stage.plannedTiming)}${cell(stage.clinicalNotes?.trim() || "по клинической ситуации")}${cell(rub(stage.estimatedAmountRub ?? null))}</tr>`,
			)
			.join("");
...
function outpatient025uXrayDoseTable(rows, emptyText) {
    ...
    ${rows
			.map(
				(item) =>
					`<tr>${cell(item.date)}${cell(item.study)}${cell(item.area)}${cell(item.dose)}${cell(item.sourceDocument)}</tr>`,
			)
			.join("")}
  </table>`;
}
function outpatientMedicalCard025u(document, patient) {
	...
	const registrationType = outpatient025uCode(
		payload.registrationUrbanRuralCode,
		{ 1: "город", 2: "село" },
	);
...
function render025uPatientInfo(
	payload,
	patient,
	sexLabel,
	registrationType,
