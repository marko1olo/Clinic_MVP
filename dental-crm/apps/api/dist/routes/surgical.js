...
function extractHUZones(samples) {
	...
	return {
		...
		cancellousHU: avgArr(
			samples.slice(k, n - k).length > 0 ? samples.slice(k, n - k) : samples,
		),
...
export default async function registerSurgicalRoutes(fastify) {
	...
	fastify.post("/api/surgical/drill-protocol/save", async (req, reply) => {
		...
		if (!body.patientId || !body.organizationId || !body.huSamples?.length) {
			return reply
				.code(400)
				.send({ error: "patientId, organizationId, huSamples required" });
		}
		...
		const steps = buildDrillSteps(
			mischClass,
			body.implantSystem,
			body.implantDiameterMm,
			body.implantLengthMm,
		);
		const [protocol] = await db
			.insert(drillProtocols)
			.values({
	...
	fastify.get("/api/surgical/drill-protocol/:id", async (req, reply) => {
		...
		const [protocol] = await db
			.select()
			.from(drillProtocols)
			.where(eq(drillProtocols.id, id))
			.limit(1);
		if (!protocol) return reply.code(404).send({ error: "Not found" });
		return reply.send({
			protocol: { ...protocol, steps: JSON.parse(protocol.protocolJson) },
		});
	});
	...
	fastify.get("/api/surgical/drill-protocols", async (req, reply) => {
		...
		if (patientId && toothFdi) {
			rows = await db
				...
				.where(
					and(
						eq(drillProtocols.patientId, patientId),
						eq(drillProtocols.toothFdi, parseInt(toothFdi)),
					),
				);
		} else if (patientId) {
			rows = await db
				.select()
				.from(drillProtocols)
				.where(eq(drillProtocols.patientId, patientId));
		} else if (toothFdi) {
			rows = await db
				.select()
				.from(drillProtocols)
				.where(eq(drillProtocols.toothFdi, parseInt(toothFdi)));
		} else {
			rows = await db.select().from(drillProtocols);
		}
		return reply.send({
			protocols: rows.map((r) => ({ ...r, steps: JSON.parse(r.protocolJson) })),
		});
	});
	// POST /api/surgical/drill-protocol/:id/confirm
	fastify.post(
		"/api/surgical/drill-protocol/:id/confirm",
		async (req, reply) => {
			const { id } = req.params;
			const [updated] = await db
				.update(drillProtocols)
	...
	fastify.post("/api/commissions", async (req, reply) => {
		...
		if (
			...
			return reply.code(400).send({
				error: "organizationId, userId, specialty, serviceCategory required",
			});
		}
		const [row] = await db
	...
	fastify.post("/api/commissions/calculate", async (req, reply) => {
		...
		const [config] = await db
			...
			.where(
				and(
					eq(doctorCommissions.organizationId, body.organizationId),
					eq(doctorCommissions.userId, body.userId),
					eq(doctorCommissions.serviceCategory, body.serviceCategory),
					eq(doctorCommissions.isActive, true),
				),
			)
			.limit(1);
		if (!config)
			return reply.code(404).send({
				error: "Commission config not found for this doctor+category",
			});
		const deductedMaterial =
			body.materialCostRub * (config.materialCostDeductionPct / 100);
		...
		const clinicMargin =
			body.totalRevenue - doctorEarnings - body.materialCostRub;
		return reply.send({
			...
			clinicMarginPct:
				body.totalRevenue > 0
					? Math.round((clinicMargin / body.totalRevenue) * 100)
					: 0,
		});
	});
	...
	fastify.post("/api/scheduler/reserve", async (req, reply) => {
		...
		if (!body.organizationId || !body.patientId) {
			return reply
				.code(400)
				.send({ error: "organizationId, patientId required" });
		}
	...
	fastify.get("/api/scheduler/reservations", async (req, reply) => {
		...
		if (!organizationId)
			return reply.code(400).send({ error: "organizationId required" });
		const conditions = [
			eq(schedulerReservations.organizationId, organizationId),
		];
		if (phase)
	...
	fastify.get("/api/scheduler/recall-due", async (req, reply) => {
		...
		const overdue = await db
			...
			.where(
				and(
					eq(schedulerReservations.organizationId, organizationId),
					eq(schedulerReservations.phase, 2),
	...
	fastify.post("/api/scheduler/recall-trigger/:id", async (req, reply) => {
		const { id } = req.params;
		const [updated] = await db
			.update(schedulerReservations)
			.set({ recallTriggeredAt: new Date(), status: "patient_notified" })
			.where(eq(schedulerReservations.id, id))
			.returning();
		if (!updated) return reply.code(404).send({ error: "Not found" });
...
function buildDrillSteps(misch, system, diameter, length) {
	...
	if (misch === "D1") {
		...
		steps.push({
			step: 3,
			drillType: `Profile ${diameter - 0.5}mm`,
			diameterMm: diameter - 0.5,
			depthMm: length,
		...
		steps.push({
			step: 3,
			drillType: `Profile ${diameter - 0.2}mm`,
			diameterMm: diameter - 0.2,
			depthMm: length,
		...
		steps.push({
			step: 2,
			drillType: "Twist 2.0mm",
			diameterMm: 2.0,
			depthMm: length,
			rpmRange: "1200 RPM",
			torqueNcm: "25 Ncm",
			irrigation: false,
			note: `Underdrill D4: max primary stability`,
		});
		steps.push({
			step: 3,
			drillType: `Under-profile ${eff}mm`,
			diameterMm: eff,
			depthMm: length,
			rpmRange: "1000 RPM",
			torqueNcm: "30 Ncm",
