...
async def _send_message_task(chat_id: int, text: str):
    try:
        await bot.send_message(chat_id, text, parse_mode="Markdown")
    except Exception as e:
        log.error(f"Failed to send to {chat_id}: {e}")

async def _send_photo_task(chat_id: int, photo_bytes: bytes, caption: str, report_text: str):
    try:
...
async def broadcast(text: str, role: str = 'admin'):
    ...
    if not users:
        ...
        return
    tasks = [_send_message_task(chat_id, text) for chat_id in users]
    if tasks:
...
async def broadcast_photo(photo_bytes: bytes, caption: str, report_text: str, role: str = 'doctor'):
    ...
    tasks = [_send_photo_task(chat_id, photo_bytes, caption, report_text) for chat_id in users]
    if tasks:
        await asyncio.gather(*tasks)
