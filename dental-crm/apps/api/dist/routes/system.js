...
function timestampForDownloadName(value = new Date()) {
...
function bridgeRemoteProbeAllowed() {
...
function healthUrl(rawUrl, defaultHealthPath, deriveHealthFromConfiguredPath = false) {
...
function localBridgeUrlWarning(error) {
...
function localBridgeProbeWarning(error) {
...
async function probeBridge(definition, allowRemoteBridgeProbe) {
...
async function buildLocalBridgeReadiness() {
...
function readyBridge(readiness, kind) {
    return readiness.bridges.find((bridge) => bridge.kind === kind && bridge.status === "ready") ?? null;
}
function envConfigured(names) {
    return names.some((name) => Boolean(process.env[name]?.trim()));
}
function numberedEnvConfigured(prefixes, max = 20) {
    for (const prefix of prefixes) {
        for (let index = 1; index <= max; index += 1) {
            if (process.env[`${prefix}_${index}`]?.trim())
                return true;
        }
    }
    return false;
}
function groqVisionConfigured() {
    return (envConfigured(["GROQ_API_KEY", "GROQ_API_KEYS"]) ||
...
function buildVisitDictationPlan(readiness) {
    ...
    const primaryPath = localBridge ? "local_bridge" : serverSttAvailable ? "server_gateway" : "browser_local";
    const warnings = [
...
function buildDocumentOcrPlan(readiness) {
    const ocr = readyBridge(readiness, "ocr_vision");
    const groqReady = groqVisionConfigured();
    const primaryPath = ocr ? "local_bridge" : groqReady ? "cloud_provider" : "manual_review";
    return {
        ...
        steps: [
            ...
            planStep(2, ocr ? "Запустить локальный OCR-модуль" : groqReady ? "Использовать серверное распознавание изображений" : "Отметить, что нужен OCR", "system", primaryPath, true, false, ocr
...
function buildPricePhotoPlan(readiness) {
    const ocr = readyBridge(readiness, "ocr_vision");
    const groqReady = groqVisionConfigured();
    const primaryPath = groqReady ? "cloud_provider" : ocr ? "local_bridge" : "manual_review";
    return {
        ...
        steps: [
            ...
            planStep(2, groqReady ? "Классифицировать через серверное распознавание" : ocr ? "Считать текст локальным OCR" : "Использовать детерминированный разбор таблицы", "system", primaryPath, true, false, groqReady
...
function buildCbctMprPlan(readiness) {
    const dicom = readyBridge(readiness, "dicom_cbct");
    const ohif = readyBridge(readiness, "ohif_viewer");
    const primaryPath = dicom ? "local_bridge" : ohif ? "external_viewer" : "metadata_preview";
    return {
        scenario: "cbct_mpr",
        ...
        steps: [
            ...
            planStep(2, dicom ? "Использовать локальный КТ-обработчик" : ohif ? "Открыть внешний просмотр" : "Остаться в режиме метаданных", "system", primaryPath, true, false, dicom
...
function buildImagingImportPlan(readiness) {
    const dicom = readyBridge(readiness, "dicom_cbct");
    const ocr = readyBridge(readiness, "ocr_vision");
    const primaryPath = dicom ? "local_bridge" : "metadata_preview";
    return {
        ...
        steps: [
            ...
            planStep(2, dicom ? "Передать тяжелый архив снимков обработчику" : "Использовать встроенный разбор метаданных", "system", primaryPath, true, false, dicom
            ...
            planStep(3, ocr ? "OCR этикеток при необходимости" : "OCR этикеток остается необязательным", "system", ocr ? "local_bridge" : "manual_review", true, false, ocr ? "Локальный OCR может читать экспортированные этикетки или бумажные ссылки." : "Ручное сопоставление пациента остается доступным."),
...
export async function registerSystemRoutes(app) {
    app.get("/api/system/persistence/verify", async (request, reply) => {
        if (!(await requireClinicalReadAccess(request, reply, "persistence verify")))
            return;
        return await getPersistentStateIntegrityReport();
    });
    app.get("/api/system/local-bridges/readiness", async (request, reply) => {
        if (!(await requireClinicalReadAccess(request, reply, "local bridge readiness")))
            return;
        return buildLocalBridgeReadiness();
    });
    app.get("/api/system/local-bridges/use-plans", async (request, reply) => {
        if (!(await requireClinicalReadAccess(request, reply, "local bridge use plans")))
            return;
    ...
    app.get("/api/system/persistence/export", async (request, reply) => {
        ...
        if (auditOrganizationId) {
            ...
            request.log.warn({ url: request.url, reason: "no-verified-organization" }, "[SYSTEM_EXPORT_UNAUDITED] Выгрузка состояния выполнена по ключу администратора без токена клиники: " +
