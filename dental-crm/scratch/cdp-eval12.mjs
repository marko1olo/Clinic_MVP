  ...
  ws.on('open', () => {
    ...
    setTimeout(() => {
        send('Runtime.evaluate', {
            expression: 'fetch("http://127.0.0.1:4100/api/dashboard").then(r => r.json()).then(d => { const dstr = JSON.stringify(d); console.log("DASHBOARD LEN:", dstr.length); console.log(dstr.substring(0, 1000)); })'
        });
    }, 4000);
  });
  ws.on('message', data => {
